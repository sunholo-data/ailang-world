...
## Iteration 38 — 2026-07-29 — `w-effect-journal` **MJ.B LANDED** (PR #27 → squash `3ef5510`, dev CI green both jobs SHA-addressed on the merge commit, judge sonnet PASS 86/100 zero-blocking) — and the iteration's spine is that **the standard way to undo a mutation silently deletes the milestone it was testing**: `git checkout <file>` reverts to HEAD, not to the uncommitted baseline this mission builds every sprint on, and the only thing that caught it was the sha256 the discipline already required for a different reason
