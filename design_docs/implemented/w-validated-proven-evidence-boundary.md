...
| M26 **timeout ordering** | `host/evidence/validator.go`: change `NewValidator`'s ordering refusal `if window > 0 && cfg.ObjectReadTimeout <= window` (window is the reader's reported `BusyTimeout()`; exact spelling fixed at implementation — the neutered comparison is the ordering pin itself) to `if false && (window > 0 && cfg.ObjectReadTimeout <= window)` | `TestConstructorPinsBusyTimeoutBelowObjectReadTimeout` (AC18), plus the isolated fake-based `TestConstructorNamesActuallyUsedUnorderedTimeouts` | Correct code: constructing a validator over the REAL file-backed store with a POSITIVE `ObjectReadTimeout` of 1 s — merely <= the store's live 2000 ms busy window — is refused with the dedicated `ErrUnorderedTimeouts` naming both values. Under the mutant the unordered validator constructs; AC16's non-positive constructor refusal is a DIFFERENT, WEAKER guard and stays green on this stimulus (1 s is positive), which is exactly what makes the two arms distinguishable rather than one check wearing two names. Failure: `NewValidator accepted ObjectReadTimeout 1s <= busy_timeout 2s; want ErrUnorderedTimeouts`. The fake-based constructor test kills the branch in isolation; the real-store arm additionally proves the production store reports the live window and the full composition obeys it (§6.1). |
...
| M30 **mandatory wait-bound metadata** | `host/evidence/validator.go`: change the negative/unknown `BusyTimeout()` refusal to treat that value as zero | `TestReaderWaitBoundsCannotBeLostThroughWrapper` (AC22), plus the isolated fake-based `TestConstructorRefusesUnknownBusyTimeout` | Correct code refuses a reader whose `BusyTimeout()` is unknown/negative with `ErrInvalidValidatorConfig`, while the real-store wrapper control forwards the positive 2000 ms value and reaches the existing ordering refusal for `ObjectReadTimeout` = 1 s. Under the mutant the fake-based constructor test accepts the unknown bound in isolation; failure: `NewValidator accepted unknown BusyTimeout: <nil>`. The real-store wrapper is integration evidence that adaptation forwards production metadata, not a prerequisite for killing the constructor branch. |
...
- **M26** — the fake-based `TestConstructorNamesActuallyUsedUnorderedTimeouts` kills the ordering
  mutant in isolation by supplying the two runtime values. The real-store AC18 arm is still needed
  as integration evidence that the production store reports its cached positive window and that
  the composed validator observes it; it is not required for the branch kill.
...
- **M30** — the fake-based `TestConstructorRefusesUnknownBusyTimeout` kills the unknown-bound mutant
  in isolation. The wrapper-around-real-store arm adds integration evidence: it forwards the
  store's actual cached window and proves interface adaptation cannot silently erase the ordering
  check; the real store is not required for the branch kill.
...
V56 was run first-party for PE.F on 2026-08-22 from the milestone worktree. Each mutant was
restored from the same sha256-verified `/tmp` backup before the next arm or control ran.
...
| V33 | Round-5 sweep (R1): before this revision the document referenced the free resolver symbol at exactly **five** lines — 78, 90, 201, 295, 398 — two more than the three the revision brief listed, which is why the sweep and not the list is authoritative. After the revision, every remaining occurrence names the symbol only to record that it is dropped (header, §2.2, §3.2, §10.5, and this log); none specifies it as a live API surface. | `grep -n 'GradeOfValidated' design_docs/implemented/w-validated-proven-evidence-boundary.md` run before the edits and again after; counts via `grep -c` on the same pattern and path | Before: `doc_count=5`, lines 78/90/201/295/398 (the line-78/201/295 subset matching the brief's `sed -n '78p;201p;295p'` spot-check). After the R1/R2/R3 edits and before these two rows landed: 5 hits at lines 9/82/221/705/710, each a drop-notice; final re-run after this table row is recorded beneath the table. |
...
| V56 | PE.F refutes the premise that M26 and M30 require the real store to be killed. Each constructor branch reds in isolation with the existing fake reader; the real-store tests remain valuable integration evidence but are not prerequisites for either branch kill. | M26 mutant: change `if window > 0 && cfg.ObjectReadTimeout <= window` to `if false && (window > 0 && cfg.ObjectReadTimeout <= window)`, then run `GOTOOLCHAIN=go1.25.6 AILANG_BIN=/tmp/ailang-v0300/ailang go test ./host/evidence -run '^TestConstructorNamesActuallyUsedUnorderedTimeouts$' -count=1`; restore, then run the same command pristine. M30 mutant: replace the `window < 0` return with `window = 0`, then run `GOTOOLCHAIN=go1.25.6 AILANG_BIN=/tmp/ailang-v0300/ailang go test ./host/evidence -run '^TestConstructorRefusesUnknownBusyTimeout$' -count=1`; restore, then run the same command pristine. | M26 mutant **RED rc=1**: `authority_test.go:553: ordering refusal did not name runtime values: <nil>`; pristine control **GREEN rc=0**. M30 mutant **RED rc=1**: `authority_test.go:544: NewValidator accepted unknown BusyTimeout: <nil>`; pristine control **GREEN rc=0**. No real store occurs in either named test. |
