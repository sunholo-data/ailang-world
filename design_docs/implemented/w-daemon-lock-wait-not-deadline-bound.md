...
- Status: **implemented** (iter-189; sprint `1eeab0c` M1, `0cac9fb` M2, `f1ccd13` judge-survivor pin) · Date: 2026-09-25 · Designer: claude-opus-5-5 (iter-189) · Base commit: `a5090f0` · Owning queue row: **22** · Scope class: daemon robustness (clause-2) · Verify profile: go-host (no `.ail` change)
...
## §14 Implementation record (iteration 189)

- **Plan:** `55ba69c` (planner `opus`). It was prototyped first: 19/19 mutations killed after DV3 added one message assertion. DV1 is a LOC count correction. DV2 adds a third false comment at `daemon.go:124-127` to M2.
- **M1 `1eeab0c`:** `checkReadOrdering`, `ErrUnorderedTimeouts`, the refusal in `New` right after `store.Open`, and `lock_wait_order_test.go` (3 tests, 5 subtests). **M2 `0cac9fb`:** comment-only honesty edits in `writer_lock.go`, `handlers.go` and `daemon.go`. The LIMITATION tag and residual (i) are byte-identical (shasum `43a5caf6…` on both trees).
- **Executor** `opus`: 19/19 plan mutations killed on the committed tree. Non-vacuity was measured against M1's own call block: removing it fails `/equal` and `/above`.
- **Controller re-derivation:** `go vet` clean, `go test ./...` **21 ok / 0 FAIL**, and `verify_ail.sh` PASS (11 identities, 40 named tests). The R1 retune (busy_timeout 15 s) was reproduced first-party and is killed by AC1.
- **Evaluator** `sonnet`, in its own worktree: **PASS 97/100, zero blocking.** It ran 5 mutations of its own. E3 survived (the operator-facing `StartupError.Detail` was never asserted). The controller reproduced E3 and closed it with a test-only pin, `f1ccd13`. That pin is E3's sole killer, measured with the mutation applied and `daemon.go` then restored byte-identical.
- **Left open, by design:** F1, the pre-existing overclaim at `daemon.go:505-507` ("the same proven bound applies" for the projection's `MaxWait`). Runtime deadline enforcement and the 500-vs-503 classification of a lock-blocked read also stay open. Both belong to the successor row the controller files for O1.
- **Round-2 judge** `sonnet` (resumed with its own transcript, on `f1ccd13`): **PASS 99/100.** It confirmed the diff is exactly 5 test lines. E3 is now KILLED at `lock_wait_order_test.go:80`, and E1, E2, E4 and E5 are still killed. The full suite is 21 ok / 0 FAIL, and 8 `--- PASS` lines were counted. F1 stays open and out of scope.
