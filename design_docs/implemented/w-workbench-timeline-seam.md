...
- Revision 2 (quorum r2: gemini-3-1-pro PASS, oc-glm-5-2 PASS, gpt6-astra REJECT on one mutation row; narrow-refinement carve-out, controller-applied, reviewer's verbatim fix): H11 becomes `selected.Edges = edges[:0]` because deleting the assignment leaves `edges` unused, which Go rejects at compile time; AC8 now records compilation separately and a build failure never counts as a kill. See §11.
...
- **AC8 — mutation drill.** Apply each row of §7 on its own. `go test ./host/workbench ./host/daemon -count=1` reds, and the named test is among the reds. Restore with `git diff --quiet -- host/` rc=0 before the next row. Record the per-row table (mutant landed y/n, **compiled y/n**, red set, restored) in the sprint's Verification Log. Record successful compilation separately from the named test failure; build failures must not count as kills.
...
| H11 | workbench.go · Selected block | `selected.Edges = edges` → `selected.Edges = edges[:0]` (keeps `edges` used, so the mutant compiles; revision 2, gpt6-astra r2 verbatim) | `TestWorkbenchSelectedEntry/stored-edge-links` |
...
| r2 | gemini-3-1-pro | pass | — | — |
| r2 | oc-glm-5-2 | pass | — | — |
| r2 | gpt6-astra | reject | H11 (delete `selected.Edges = edges`) leaves `edges` unused → compile error, so the named test can never be the kill; contradicts §7's every-mutation-compiles premise | **Upheld; applied verbatim by the controller under the narrow-refinement carve-out** (direction undisputed, concrete reviewer-authored fix): H11 → `selected.Edges = edges[:0]`; AC8 records compiled y/n and build failures never count as kills |
