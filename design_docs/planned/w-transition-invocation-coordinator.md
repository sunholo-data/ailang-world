...
**Estimate**: ~2d as the row says. Seven milestones of ≤ ~150 production code lines each, sized from the prototype's measured code lines (V27). **First landable slice = M1–M4c, with no production caller** (quorum r2 carve-out, see "Round 2 carve-out"). **M5 and M6 are BLOCKED on row 23's policy tranche** (`D-WORLD-37` = A: context-aware receipt lookup, connection acquisition and pre-durable transaction operations, plus an enforced bound for any remaining non-cancellable durable work). Until then `/a2a/` keeps refusing with the constant `-32603`.
...
| 1a | **in-flight guard (quorum r2, gemini-3-1-pro, verbatim fix — see "Round 2 carve-out" below)**: "Add an in-memory flight tracker (e.g., `inFlight sync.Map`) to `Coordinator` keyed by `InvocationID`. In Step 1 (before 1b), check if the ID is in flight. If it is, either return an immediate 'in-flight' error to prevent duplicate execution, or store a `chan struct{}` that allows the concurrent retry to block (bounded by `ctx`) and then safely re-evaluate Step 1b once the original request completes. Release the ID in a `defer`." | in-memory, per `Coordinator` | **R17** `*InFlightError` (the immediate-refusal branch; the planner chooses between the two branches the fix names and records the choice) |
...
### Round 2 carve-out — the reviewers' fixes, applied verbatim (controller, iteration 196)

Quorum round 2 blocked 2/0 on one surface, the durable/concurrency tail. Both objections carried a concrete reviewer-authored `proposed_fix` and neither disputed the design direction, so the controller applied the narrow-refinement carve-out: the fixes below are the reviewers' own text. Where they change a statement elsewhere in this doc, **this section wins**.

**(1) Concurrent same-task-id retries (gemini-3-1-pro).** The earlier claim that `*store.DuplicateInvocationError` is "unreachable after step 1b" is **withdrawn**. The reviewer's objection is correct: between `GetReceipt` (step 1b) and `AppendIntent` (step 10) there is a window of up to the capsule's 60 s bound, and a retry of the same `TaskID` inside it sees `Absent`, executes a second time, and races the original to step 10. The controller adds that the same window also invalidates R15's inference in the concurrent case: a retry can read the original's intent after `AppendIntent` and before `Commit`, and would then say "not committed" for a commit about to land. **Fix, verbatim:** *"Add an in-memory flight tracker (e.g., `inFlight sync.Map`) to `Coordinator` keyed by `InvocationID`. In Step 1 (before 1b), check if the ID is in flight. If it is, either return an immediate 'in-flight' error to prevent duplicate execution, or store a `chan struct{}` that allows the concurrent retry to block (bounded by `ctx`) and then safely re-evaluate Step 1b once the original request completes. Release the ID in a `defer`."* This is step 1a above (R17). The tracker is per-process, which is sufficient on this store because it is single-process-writer (writer lock, row 107 F8). The guard and its release each get a named mutation and a concurrency test in M4c; the error-mapping row for `DuplicateInvocationError` stays as a defensive mapping, no longer described as unreachable.

**(2) The durable tail violates the bounded-waits gate (gpt6-astra).** Revision 1 weakened the deadline *claim*; the reviewer is right that it did not change the *blocking behaviour*, and that reconciliation does not bound anything because the resend enters the same unbounded `GetReceipt` wait. **Fix, verbatim:** *"Make bounded durable operations a prerequisite for M5, not a deferred residual. Add: 'M1–M4c may land without a production caller. M5 remains blocked on row 23 providing context-aware receipt lookup, connection acquisition, and pre-durable transaction operations, plus an enforced bound for any remaining non-cancellable durable work. Uncertain outcomes remain explicitly unconfirmed and are reconciled without re-execution.' Require tests that hold the sole connection past the invocation deadline and inject stalls in each durable operation, demonstrating bounded request completion and no accumulating blocked workers. Preserve the rule that a confirmed successful commit is never converted into failure by a subsequent context check."*

Consequences for this doc, stated once:
- **M1–M4c may land without a production caller.** Nothing in `host/daemon` or `host/projection` constructs a `Coordinator` until M5. `/a2a/` keeps refusing with the constant `-32603` until then.
- **M5 and M6 are BLOCKED on row 23's policy tranche** (`D-WORLD-37` = A), which must provide context-aware receipt lookup, connection acquisition and pre-durable transaction operations, plus an enforced bound for any remaining non-cancellable durable work. R-106-11 is therefore a **prerequisite of M5**, not a deferred residual.
- M5's acceptance criteria gain the reviewer's tests: hold the sole connection past the invocation deadline, and inject stalls in each durable operation, demonstrating bounded request completion and no accumulating blocked workers.
- Unchanged and still binding: a confirmed successful commit is never converted into a failure by a subsequent context check (`TestCommitBoundary/deadline_during_commit_reports_success`, MUT-POSTCOMMIT-CTX).
...
**First landable slice: M1–M4c, with no production caller** (quorum r2 carve-out). **The prototype already implements M1–M4c** (see Prototype); M4c additionally gains the step-1a in-flight guard (R17). **M5 and M6 are BLOCKED on row 23's policy tranche** (R-106-11 is a prerequisite of M5, not a deferred residual). The clause-6 value therefore appears only once row 23's tranche and M5 land.
...
### Round 2 (iteration 196): BLOCKED 2/0 (gemini-3-1-pro, gpt6-astra present; glm and kimi ABSENT on the Ollama weekly limit) → narrow-refinement carve-out

Artifact `.ailang/state/mission-quorum/w-transition-invocation-coordinator-2026-09-26T18-18-39Z.json`. Both objections landed on one surface (the durable/concurrency tail), each carried a concrete reviewer-authored `proposed_fix`, and neither disputed the design direction (one is a completeness fix, the other a scope-cut of M5 behind row 23). The controller applied both fixes verbatim in the section "Round 2 carve-out" and in the Estimate, milestone-slice, error-mapping and R-106-11 text, and routed to the planner without a third round. Objections verbatim:

> **gemini-3-1-pro:** The claim in the error-mapping table that `*store.DuplicateInvocationError` is 'unreachable after step 1b' is false due to a TOCTOU race. There is a concurrency window of up to 60 seconds (the capsule execution bound) between `GetReceipt` in Step 1b and `AppendIntent` in Step 10. If a client times out and retries the same `TaskID` while the original request is still executing, the retry will see an 'Absent' receipt at Step 1b, bypass reconciliation, perform a redundant 60-second capsule execution, and then race the original request to Step 10. The loser of this race will trigger the allegedly unreachable `DuplicateInvocationError`, causing the retry to return a `-32602 task id already used` error instead of the intended reconciled result.

> **gpt6-astra:** The production invocation path explicitly violates the bounded-waits hard gate. V30 establishes that GetReceipt, AppendIntent, and Commit can wait indefinitely for the single database connection, and transaction execution has no bound. M5 nevertheless enables this path before row 23 lands. Acknowledging the violation as R-106-11 and offering resend reconciliation does not resolve it; the resend itself enters the same unbounded GetReceipt wait.

*Controller measurement:* both premises TRUE on reading the doc's own step table (steps 1b and 10 are separate store calls with a capsule run between them; V30 already measured the durable waits as unbounded). The concurrency window additionally invalidates R15's "not committed" inference for a concurrent retry, recorded in the carve-out section.
