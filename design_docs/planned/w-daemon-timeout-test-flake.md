...
| MU-DEADLINE-DETACH | `readCtx` body: `d.readDeadline` → `readDeadline` (the 10 s package constant; production one-shot, `handlers.go`; compiles — both identifiers in scope, pinned by D16: the constant at `daemon.go:128`, the field-consuming `readCtx` body at `handlers.go:269-271`) | the same `-run` → exactly 1 `--- FAIL:`, the "returned a LIVE context" Fatal (field −1 ns ignored, 10 s future deadline → live ctx). | `-skip` the killer: **multiply-killed, and the red set is the assertion** — `TestDaemonReadDeadline/real-store-expired-deadline` and `TestTimeoutStatusMirrorsSketch` red deterministically (every route 200: the M2 failure shape at 100% instead of <1%), **and so does `TestDaemonReadDeadline/blocking-store`** — CORRECTED BY MEASUREMENT, iteration 94 (planner-found, controller-reproduced independently): that subtest parks a getter behind a 50 ms field deadline, so a `readCtx` that ignores the field never releases it and its own 2 s watchdog fires. The red is the mutant's own phenotype, not an unexplained red. `normal-deadline-answers-200` stays green. **The measured red set is therefore exactly 4 `--- FAIL` lines** — `TestDaemonReadDeadline` (parent, 2.01 s), its two subtests above, and `TestTimeoutStatusMirrorsSketch` — with `normal-deadline-answers-200` PASS. Any red OUTSIDE those four is unexplained and fails the arm. (The earlier wording named two and added "any red outside that set is unexplained and fails the arm", which would have scored a CORRECT mutant as a failed arm — a red set is a count taken in a scope, and this one was reasoned rather than run.) |
