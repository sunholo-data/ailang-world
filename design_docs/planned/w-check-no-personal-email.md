...
- Status: **planned** · Date: 2026-09-14 (revised once 2026-09-14 — quorum r1: gpt5-6-sol reject on AC-M1-3/AC-M1-9, gemini-3-1-pro reject on D3's conditional and D1's undefined filter; all four measured by the controller and applied; artifact w-check-no-personal-email-2026-09-14T09-39-09Z.json). **Quorum r2 BLOCKED and CLOSED under the narrow-refinement carve-out** (artifact `w-check-no-personal-email-2026-09-14T09-43-05Z.json`, both reviewers present, `absent_reviewers` empty): `gpt5-6-sol` reject — the doc claimed a *tracked-file* contract while the instrument scans a narrower surface; its own fix offered two options, and the controller MEASURED which applies (a full-tree scan reads 7 non-address false positives today, V15), so option 2 — state the exact narrow contract, quote its authoritative source, enumerate and justify every exclusion — is applied VERBATIM in §1, D2 and §9. `gemini-3-1-pro` reject — that `SCOPE_RE` drops "five root-level mission logs": REFUTED by measurement (V14: zero root-level `*mission*.md` files exist; all six are `design_docs/`-prefixed — the r1 text's F7 shorthand omitted the prefix and invited the misread), so its regex fix is NOT applied and the six paths are now spelled out in full. Neither objection disputes direction; the widening question is a NEW queue row, not a revision. · Designer: pi:ollama/deepseek-v4-flash:0731-cloud (iter-175; rotation entry codex:gpt-6-astra was ration-blocked this fire) · Base commit: a295291bea069d793960e08f055e501c6f9661f4 · Owning queue row: 74 · Scope class: HARNESS — enforcement of the attended-ledger privacy contract · Verify profile: ailang-code

## §1 Problem

Row 74's premise is that the shared mission-control skill's ATTENDED LEDGER EDITS contract (Mark, attended 2026-09-02) names `make check-no-personal-email` as the enforcement behind its provenance rule — while that target existed **nowhere** in `sunholo-data/ailang-world`. **The contract, stated exactly (V16):** the rule is about an author's ADDRESS being written into a ledger evidence cell — *"Writing the author's ADDRESS into an evidence cell publishes it — ledger rows are pasted verbatim into the public bookkeeping issue by every report … `make check-no-personal-email` fails the build if one comes back"* (resolved shared skill, `resources/gate-0-preflight.md:169-172`). "One" is an address in the surface the LOOP writes, and the fleet's own implementation says so in its header: *"SCOPE IS DELIBERATELY NARROW: the loop-written surface only"* (`scripts/check_no_personal_email.sh:11-16` at `f0d44915d`). This design implements THAT contract — the loop-written tracked surface — and nowhere claims to police every tracked file; the exclusions are enumerated and justified in D2. The history is concrete and public:
...
**Why "passes today" is not a gate**: the single in-scope hit is a live personal address sitting in a tracked, loop-written script today, and no CI step would notice it. The contract is enforceable or it is decoration. "It happens to be clean except one address" is not enforcement — the instrument and its wiring do not exist. This item makes the contract real: an address-pattern check over the loop-written tracked surface (the exact narrow contract above — NOT every tracked file), with the functional defaults handled, wired into CI, and proven non-vacuous with a mutation arm that plants an address and asserts the gate REDs.
...
- **Exclusion list (verbatim, anchored)**:
  ...
  ```
  **EVERY clause is anchored to the whole extracted token** (`$` on each domain suffix, `^` on the `noreply` local part) — see D5 arms H, I and J. *(Revised after evaluator round 1: the r2-quorum text anchored only the reserved-TLD clause; the judge reproduced four attacker-shaped addresses — `attacker-noreply@<personal domain>`, `x@users.noreply.github.com.<personal domain>`, `y@gserviceaccount.com.<personal domain>`, `z@sentry.io.<personal domain>` — reading ✓ rc=0 through the four unanchored clauses. The principle D5 already states, "a privacy gate must not whitelist a substring", now applies to all six.)*
...
**[controller-measured 2026-09-14 at a295291]** F7: World's in-scope tracked files under the fleet's regex are exactly six mission docs, ALL under `design_docs/` — `design_docs/world-mission.md`, `design_docs/world-mission-log.md`, `design_docs/world-mission-log-archive.md`, `design_docs/world-mission-status-archive.md`, `design_docs/world-mission-dashboard.md`, `design_docs/world-mission-index.md` — plus 28 files under `scripts/` = **34 (at base, at a295291)**. There are **zero** root-level `*mission*.md` files (V14: `git ls-files | grep -cE '^[^/]*mission[^/]*\.md$'` → 0, control: the `design_docs/`-prefixed form lists the six), so the `design_docs/` prefix in the regex drops nothing. World has **NO `.claude/skills/`** directory (the mission-control skill resolves through a symlink into the V1 checkout), so that regex alternative matches nothing here.
...
**Exclusions — enumerated and justified, never implied by the regex** (quorum r2, `gpt5-6-sol`'s fix applied verbatim). Everything tracked that is NOT matched by `SCOPE_RE` is out of scope, and each class is out for a stated reason:

| Excluded class | Why | Measured today (V15) |
|---|---|---|
| `design_docs/planned/**`, `design_docs/implemented/**`, `design_docs/verification/**`, root `sprint_*.json` | sprint artifacts written by designers/planners/executors, not the ledger/report surface the contract names; a full-tree scan reads **7 non-address false positives** here (`git@github.com` ×3 — SSH remote strings; `toolchain@v0.0.1-go1.2x.y.darwin` ×4 — Go toolchain module ids), so widening needs its own exclusion set and its own arms | 7 hits, 0 personal |
| Go sources, `.ail` modules, `.github/workflows/*.yml`, `go.mod`/`go.sum`, `docs/**` | code and configuration, not loop-written prose; an address there is functional (the fleet's precedent excludes `cmd/ailang/*.go` for the same reason) | 0 hits |
| `README.md`, `CLAUDE.md`, governance/contact files | deliberate public contact points — changing them is a governance decision, not a lint (fleet precedent header, verbatim) | 0 hits |

The one personal address in the entire tracked tree today is the `scripts/mission_answer.sh:69` default D3 removes (V15: 8 hits total = 1 personal + 7 false positives). **Widening the surface to `design_docs/**` is a follow-up queue row**, filed by the controller with V15 as its evidence — it is a scope decision with its own exclusions to rule on, exactly as the fleet's header warns (*"Widening this without ruling on those first would just fail the build"*).
...
- **I — every OTHER clause is anchored too** (evaluator r1 blocking finding, fixed in-iteration): four tokens in one mission doc — a local part merely ENDING in `noreply` on a personal domain, and personal domains that merely CONTAIN `users.noreply.github.com`, `gserviceaccount.com`, `sentry.io` — read `rc=1 hit=4`.
- **J — the anchors still ADMIT every legitimate machine identity** measured on both loop-written surfaces: `noreply@anthropic.com`, `sa-…@<project>.iam.gserviceaccount.com`, `alerts@sentry.io`, `151556158+sunholo-voight-kampff@users.noreply.github.com` → `rc=0 hit=0 ok=1`. An anchor that is too tight reds the real surface; J is what makes I safe to ship.
...
- **AC-M1-4** `bash scripts/test_check_no_personal_email.sh` → `10 passed, 0 failed` (arms A–J; the suite prints F last because it needs a second throwaway repo).
...
| M10 | revert the four newly-anchored clauses to the fleet's substring form (`users\.noreply\.github\.com\|noreply@\|gserviceaccount\.com\|@sentry\.io`) | arm I RED alone (`rc=0 hit=0 ok=1`, 9 passed / 1 failed) — controller-measured | yes — sole killer |
| M11 | delete the `^noreply@` clause (an over-tight anchor set) | arm J RED alone (`noreply@anthropic.com` becomes a hit, 9/1) — controller-measured | yes — sole killer |
...
| V14 | `[controller-measured 2026-09-14 at a295291]` (quorum r2, refutes `gemini-3-1-pro`) | `git ls-files \| grep -cE '^[^/]*mission[^/]*\.md$'` ; control `git ls-files \| grep -E '^design_docs/[^/]*mission[^/]*\.md$'` | **0** root-level mission files; the control lists exactly the six `design_docs/`-prefixed docs — the regex drops nothing |
| V15 | `[controller-measured 2026-09-14 at a295291]` (quorum r2, decides `gpt5-6-sol`'s option) | full-tree scan: `git ls-files` (357 files) × `PAT` × the anchored exclusion list | **8** hits: `scripts/mission_answer.sh` (the one personal address, D3) + **7 false positives** — `git@github.com` ×3 (two `design_docs/planned/w-daemon-timeout-test-flake*` docs, one `sprint_w-daemon-timeout-test-flake.json`), `toolchain@v0.0.1-go1.25.6.darwin`/`…go1.27.0.darwin` ×4 (four `design_docs/implemented/w-defork-pin-redirects-work-repo*` / `w-setup-go-pin-unguarded-sprint-plan.md`) |
| V16 | `[controller-measured 2026-09-14]` (quorum r2, the authoritative contract) | `sed -n 169,172p "$(readlink -f ~/.claude/skills/mission-control)/resources/gate-0-preflight.md"`; `git -C <fleet> show origin/dev:scripts/check_no_personal_email.sh \| sed -n 11,16p` | the skill: *"Writing the author's ADDRESS into an evidence cell publishes it … `make check-no-personal-email` fails the build if one comes back"*; the fleet script: *"SCOPE IS DELIBERATELY NARROW: the loop-written surface only … Widening this without ruling on those first would just fail the build."* |
...
- **Scope is deliberately narrow, and the doc says so exactly**: the gate implements the loop-written-surface contract (§1, V16), not a tracked-file contract. Design docs under `planned/`/`implemented/` are NOT scanned — they are sprint artifacts, and a scan of them reads 7 non-address false positives today (D2's exclusion table, V15). Widening is a follow-up queue row with its own exclusion decisions, not this one. The gate is scoped to the six `design_docs/*mission*.md` docs + `scripts/` (36 files on the sprint tree, 34 at base).
...
- **The deviation from fleet parity — every exclusion clause anchored** (D1/D5, arms H/I/J) could surprise a future maintainer expecting byte-identical behaviour. It is documented in the script header and here, and pinned by three arms; the fleet's copy still carries the substring form and is told so in §10.
- **Declared residuals of the anchoring, both measured by the r1 judge:** (a) `x@sub.example.com` is a HIT — the reserved-TLD clause admits `@example.com` exactly, not its subdomains; this over-flags in the SAFE direction and is left as is. (c) the exclusion is CASE-SENSITIVE — `alerts@SENTRY.IO` or `you@EXAMPLE.COM` written in upper case are HITS (the r2 judge's only new finding; reproduced by the controller; over-flags in the safe direction, and no upper-cased machine identity exists on either surface, so it is left as is). (b) `noreply@<personal-domain>` is ADMITTED — the gate trusts the `noreply` local-part convention (a mailbox nobody reads is not a person), exactly as the fleet does; the r1 bypass was the local part merely ENDING in `noreply`, which `^noreply@` now rejects.
- **Three judge-invented mutations survive the suite with no observable effect** (`LC_ALL=C` removed; `sort -u` → `sort`; `[ -f "$f" ]` dropped): all three are defensive/locale code with no fixture that can distinguish them on this surface, recorded as untested rather than load-bearing.
