...
- **Declared residuals of the anchoring, both measured by the r1 judge:** (a) `x@sub.example.com` is a HIT — the reserved-TLD clause admits `@example.com` exactly, not its subdomains; this over-flags in the SAFE direction and is left as is. (b) `noreply@<personal-domain>` is ADMITTED — the gate trusts the `noreply` local-part convention (a mailbox nobody reads is not a person), exactly as the fleet does; the r1 bypass was the local part merely ENDING in `noreply`, which `^noreply@` now rejects.
