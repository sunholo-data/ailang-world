...
| AC9 | `git diff --shortstat origin/dev -- host/` and `git diff --shortstat origin/dev -- host/ ':!*_test.go'` | doc bound ≤ 320 total is **expected RED** (§0 P4) [548 total; 111 non-test]. Record both. The controller dispositions it, and tests are never cut to meet it. |
