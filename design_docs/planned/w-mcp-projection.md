...
- [ ] **AC15 — toolchain floor (P6.T):** **ALL FOUR** `ci.yml` pin sites read `1.26.6` —
  `GOTOOLCHAIN: go1.26.6` at `:21`/`:102` AND `actions/setup-go@v5`'s `go-version: '1.26.6'` at
  `:28`/`:109` — measured as `grep -c '1\.26\.6' .github/workflows/ci.yml` = **4** against a base
  of 0, and `grep -c '1\.25\.6' .github/workflows/ci.yml` = **0** against a base of 4; plus
  `go 1.26.6` in `go.mod`. (*Corrected 2026-08-26 during the P6.T sprint: this row still read
  "BOTH `ci.yml` sites" after the round-5 carve-out corrected the milestone body and both files
  tables to four — the planner caught the unsynced row. An acceptance criterion that under-counts
  the sites it gates is the round-5 objection one level down.*) The `verify_go.sh` deny-list still enumerates exactly
...
|---|---|---|
| AC15 `MUT-TOOLCHAIN-REGRESS` | set ONE site back to `go1.25.6`, one arm per site | **THREE ARMS, and the third is expected to SURVIVE — record it, do not hide it.** *Arm 1* (`go.mod`): `verify_go.sh` rc=1, `FATAL: active toolchain go1.26.4 miscompiles…`; landed-proof is `go env GOVERSION` moving `go1.26.6 → go1.26.4`, the exact variable `verify_go.sh:216` reads. *Arm 2* (`ci.yml:21`, then `:102`): rc=1 with the floor message `go: go.mod requires go >= 1.26.6 (running go 1.25.6; GOTOOLCHAIN=go1.25.6)` verbatim, the detector reading the pin OUT of the mutated file; landed-proof is the occurrence count `4 → 3`, a query against the file's own view rather than its bytes. *Arm 3* (`ci.yml:28`/`:109`, the `setup-go` sites): **SURVIVED — no command locally or in CI turns it red.** A regression there keeps CI green and silently pays a second toolchain download per job. That is an honest gap in this milestone's guard, not a passed check; it is recorded rather than papered over, and guarding it is a separate queue row. (*Mechanism corrected 2026-08-26 during the sprint: this row previously said "dependency resolution REDs", which was true only while P6.D's `require …@v0.33.2` line was in scope. The round-5 carve-out deferred that line, so the red now comes from the module's own `go` directive — same text, different and still-live cause.*) |
...
| N7 [R] | **REWRITTEN A THIRD TIME, 2026-08-26 — and the recurrence is the finding.** Round 5 blocked because this row's search was narrowed to one file; the fix recorded a command with a `':!design_docs'` pathspec it did not publish; and the sprint EVALUATOR then found that the pathspec itself hid a genuinely **executable** pin. Three fixes, three differently-drawn boundaries, same defect — **the scope of an exclusion IS part of the claim**, and this row is the worked example. Current reading, post-`P6.T`, sweeping the WHOLE repo with NO exclusion and classifying every hit rather than filtering any away: `ci.yml` and `go.mod` now carry **0** occurrences of `1.25.6` (they were the four+one sites `P6.T` moved). Exactly **three** tracked scripts still contain the literal, and only **ONE is live**: (a) **LIVE — `design_docs/verification/w-race-gate-blindspot/run.sh:25`**, `KNOWN_GOOD="go1.25.6 go1.24.9"`, an executable script (`test -x` YES) whose probe list is consulted at run time — so the instrument that certifies "known-good" **never probes the version the repo now actually uses**; tracked as a follow-up queue row (sprint-plan `Q1`/`unguarded_pins`), deliberately NOT widened into `P6.T`, whose merge criterion is zero other diffs. (b) INERT — `scripts/verify_go.sh:222`, the literal inside an `echo` advice string in an error message. (c) INERT — `scripts/verify_world_package.sh:226`, a comment. | `git grep -ln '1\.25\.6' -- '*.sh' '*.yml' '*.yaml' 'go.mod'` -> **3** files, each then classified by `test -x` and by reading its line; **no pathspec exclusion of any kind**. Candidate config files the round-5 reviewer named, plus eleven more the evaluator added (`.golangci.yml`/`.yaml`, `Makefile`, `Dockerfile`, `.tool-versions`, `.mise.toml`, `mise.toml`, `renovate.json`, dependabot, `go.work`/`.sum`, `.goreleaser.yml`/`.yaml`): **all ABSENT** (`test -e` on each). Fresh-literal negative control -> **0**. | Confirmed, and the earlier unqualified *"no other tracked file carries a live pin"* is **WITHDRAWN as false** — one does, named above |
