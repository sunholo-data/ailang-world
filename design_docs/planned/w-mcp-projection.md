...
| **Commit-boundary contract** | **UNVERIFIED — PREREQUISITE (round-2 objection, `gpt5-6-sol`)** | No landed broker/coordinator/store API exposes an atomic "not-started versus committed" contract, a stable invocation/idempotency ID, or a queryable durable receipt. Decision 6's commit-boundary wording and AC13's before/after tests depend on exactly those semantics, so **P6.B is blocked until the clause-3 broker lands them and this row cites the exact API + tests**. The controller did not invent a mechanism here |
