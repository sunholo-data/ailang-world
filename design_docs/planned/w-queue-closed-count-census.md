...
- `scripts/testdata/queue_census_rowstarts_8b15153.md` — **new** (M1). Frozen snapshot of the
  ...
  `git show 8b15153:design_docs/world-mission.md | awk '/^## Queue/{h=NR; print substr($0,1,120); next} h && (/^[0-9]+\. (\*\*|\[)/ || /^\*\*\[/) {print substr($0,1,120)}' | iconv -c -f UTF-8 -t UTF-8`
  → 91 lines, **10,034** bytes, sha256 `9424152cdbc9194ce19cd27fc6685bc50876d223641f542b8866d15aa03395b0`
  (V18, V23): heading + preamble + 89 first lines truncated to 120 bytes. **Two parts of this
  command are load-bearing and were bought by a CI red** (V23): it reads the charter **from the
  named commit** (`git show 8b15153:…`), so the fixture matches its own filename rather than
  whatever the working tree happens to hold; and it pipes through **`iconv -c -f UTF-8 -t UTF-8`**,
  because `substr($0,1,120)` counts BYTES and cuts multi-byte characters in half. The
...
| V18 | Snapshot fixture size when generated by the named command | the §3 command `\| wc -lc` (all 89 row starts match `^[0-9]+\. (\*\*\|\[)` — `awk 'NR>1610 && /^[0-9]+\. (\*\*\|\[)/' … \| wc -l` → `89`) | `91   10034`; sha256 `9424152c…` |
| V23 | **The pre-`iconv` fixture was byte-invalid and CI proved it, at the cost of one red** — `substr($0,1,120)` counts BYTES, so it cut a 3-byte `—` in half on exactly two lines, and mawk on `ubuntu-latest` did not enumerate those two rows while BWK awk on the rig did | on the pre-`iconv` file: `sed -n '67p;86p' <fixture> \| tail -c 12 \| od -An -tx1c`; the CI job's own output; on the post-`iconv` file: `/usr/bin/grep -c -axv '.*' <fixture>`; `diff <pre> <post>` | pre: both lines end `e2 80` (a truncated `—`); CI `Queue census instrument suite` → `not ok snapshot rc=2: ✗ numbering gap: expected 1..89, missing 65 76` and `58 passed, 10 failed`, while the same file on the rig read `68 passed, 0 failed`; post: `0` invalid lines, and `diff` shows **exactly** the two dropped 2-byte tails and nothing else; the fixture's own census is unchanged at `41 closed / 89 rows (tagged-open 4, untagged 44)` | **The instrument was never wrong — the FIXTURE was, and the numbering-gap floor (F4) is what caught it, loudly, naming both missing rows.** Rows 65 and 76 are the only two truncated lines and were exactly the two reported missing (2 of 89, and precisely those 2) |
...
- **CI portability was asserted by CI itself, and it FIRED on the first push** — see V23. The
  residual below was written as a theoretical one; it turned out to be live, and the defect was in
  the *fixture's bytes*, not in the awk program. What generalises: **`substr` counts bytes, a
  generated fixture inherits whatever encoding accident the generator commits, and two awks
  disagree about invalid bytes** — so a fixture must be pinned against the transformations between
  its generator and its parser, not merely "checked in as produced". The anti-vacuity floor is what
  made this cheap: F4 named both missing rows instead of quietly reporting `87 rows`.
