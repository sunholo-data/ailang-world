...
| 10 | A2A agent card + session-scoped projection | CARD LANDED iter-187; invocation DEFERRED | `implemented/w-a2a-session-projection.md`, charter row 40 | Card + fail-closed `/a2a/` shipped; invocation needs a transition-invocation coordinator and a production registry publisher (new charter rows) |
