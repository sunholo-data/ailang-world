...
## STATUS 2026-09-01 (iteration 142) — **ROW 52 PARKED ON `D-WORLD-30`: THE ROW CALLED THE DEFECT "MEASURED NON-EXPLOITABLE"; IT IS EXPLOITABLE IN *BOTH* DIRECTIONS, AND TWO FULL-STRENGTH QUORUM ROUNDS THEN KILLED TWO SUCCESSIVE LOCATORS.** Design doc written, revised once and BANKED at `design_docs/planned/w-wiring-test-step-scoping-imprecise-under-key-reorder.md` (627 lines, 19 verification rows); no code was routed, so no planner, executor or evaluator ran. **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives — `#107` since the `2026-08-27T18:20:00Z` watermark (4 comments, none allowlisted), BOTH watermarks read and the OLDER taken, via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH. Decision ledger **16 rows, 2 OPEN** before this iteration's append; read AFTER Gate 1's fetch, and `dev` was already == `origin/dev`, so the shared skill's Gate-0 ordering defect could not bite. No rotation owed (`#107` created `2026-08-31T09:26:51Z`, AFTER this Monday's 07:00 **local** boundary; 4 comments against the 80 ceiling) and no weekly sweep owed (iteration 141 ran it on this same rotation). Inbox **20 unread; ONE addressed to this mission** — V1 iter-311 on the `mission-world` channel, answered below with a measurement; the rest is V1/eval-suite/package-agent traffic and rows this mission itself posted at iter-141. **GATE 1:** `dev` == `origin/dev` == `7077e455`; tree clean; **0** open PRs; **0** stale worktrees; CI **GREEN** on `7077e455`, `checks=2`, both `success`, SHA-addressed with `total_count` as the known-positive control and the parent control resolving at **2**; skill drift **CLOSED for the twelfth consecutive iteration** (`readlink -f` → `/Users/voightkampff/dev/sunholo-data/ailang/.claude/skills/mission-control`, `cmp` against `origin/dev` IDENTICAL, run against the RESOLVED target, never the relative path). **GATE 2 — THE ROW'S OWN HEADLINE CLAIM IS REFUTED, IN BOTH DIRECTIONS.** Row 52 is an evaluator's inherited claim from iter-133 recording the scoping bug as *"measured non-exploitable … every reordering the judge tried widened the scanned range; none narrowed it."* **ARM A (widening):** `continue-on-error: true` on the PREVIOUS, unrelated `go build + test gate` step plus a key reorder of the miscompile step gives **rc=1 at `ci.yml:166`** — and `166` IS the unrelated step's own flag line, with `ruby -ryaml` (the system's own view, not bytes) reporting `step[5] coe=true`, `step[6] coe=nil`, step count unchanged at 10. A **FALSE POSITIVE** violating the boundary the test's own comment cites from quorum round-2 R1. **ARM B (narrowing):** the flag live on the miscompile step plus a line whose trimmed text begins `- name:` inside that step's own `run: |` block gives **rc=0 `--- PASS`** — the gate is **BLIND** to the exact flag it exists to forbid, because `TrimSpace` discards indentation so nesting depth is invisible. Both mutants LANDED by sha256 (`aed8e186…`→`28731ce9…`/`66bad3af…`), `go vet` rc=0 each, restored byte-identical, pristine control green either side, porcelain 0. **SCOPE CORRECTED:** the step-scoping defect has **ONE** call site, not the row's "three places" (`HasPrefix(strings.TrimSpace` → 3 hits, two of them the same function's start/end scans, the third a SQLite pragma; same-scope control **6**), and exactly **three** Go files read `ci.yml`, the other two doing whole-file counting with their own controls. Row 41's V18 re-verified: `actionlint` is in **no** repo gate (control firing at 1) though the binary sits on the rig — a rig fact, not a repo fact. **QUORUM: TWO ROUNDS, BOTH BLOCKED AT FULL STRENGTH** — `.synthesis.absent_reviewers` **`[]`** in both, cross-checked by `[.reviewers[]|select(.present==false)]` and `has("synthesis")`, read at the NESTED path V1 iter-311 corrected us to rather than the top-level one that returns `null`. R1 ($0.1035, reject/reject/**pass**/controller-reject): `gpt5-6-sol`'s census objection — DEFECT confirmed (prose said four files, enumeration listed three), CONCLUSION **REFUTED** (the enumeration is complete at three; the revision was told to fix the word and NOT hunt a fourth). `gemini-3-1-pro` said the doc's relative indentation rule is spoofable; **I implemented the doc's own locator verbatim and ran the attack — `start=177 (indent 12) InvA=PASS InvB=PASS coe=0` → GREEN, ARM B's fail-open fully recreated. CONFIRMED.** My own two objections were measured too: both addition-shaped mutants were outcome-identical across arms, so the drill had ZERO discriminating addition coverage while quoting the rule that *only an addition proves it LOOKS*; and *"key order no longer matters"* was FALSE for the legitimate `- run:`-as-dash-key layout (**`FATAL start not found`**). Before routing the revision I derived and measured the fix myself — an ABSOLUTE step column from the enclosing `steps:` key — so the designer got a direction, not three objections: six arms all anchoring at `stepCol=6` on the true dash, ARM A green, ARM B red, gemini attack red. R2 ($0.1489, after ONE revision = the Fable diet's one-DOC ceiling, met not exceeded): **BLOCKED, all three rejecting, and two landed on the exact hole I named in my own note while voting pass** — the `stepCol` anchor is itself an indentation-blind backward search, so a decoy `steps:` inside the `run:` scalar hijacks it. **Measured: the doc's derivation gives `stepCol=12 start=179 coe=0` → GREEN/blind, a shallowest-`steps:` derivation on the identical file gives `stepCol=6 start=173 coe=1` → RED/caught.** `oc-glm-5-2` caught a falsified citation (V12 is cited for `indentOf` being unallocated and its greps never search that name). **WHY THIS PARKS RATHER THAN TAKING THE CARVE-OUT:** the one-revision-one-requorum allowance is spent, and read from the artifact's `proposed_fix` fields rather than the summaries, the reviewers **disagree with each other** — `gemini-3-1-pro` says keep the line scan and pin `stepCol == 6`; `gpt5-6-sol` says replace it with semantic YAML traversal and, verbatim, *"if introducing a YAML dependency is not acceptable, park or widen the item"*. That disputes the DIRECTION, so the carve-out's own precondition fails, and choosing between two reviewers is controller judgment over a contested direction (Standing rule 2). The doc had pre-registered this exact contingency in its Milestones. A shallowest-anchor fix demonstrably survives, but it is **controller-invented**, which the carve-out forbids by name — so it becomes arm A of `D-WORLD-30` instead. **THE RETRO FRICTION IS MINE AND IT IS NEW: a hedged `pass` is syntactically indistinguishable from a clean one.** I voted `pass` in a note that named the `steps:` derivation as spoofable and *"deserves a reviewer eye"*; two reviewers then rejected on precisely it, costing a full round ($0.1489) to learn what I had already written. The skill has extensive machinery for measuring a claim before *forwarding* it and none saying **a controller verdict must be the strongest reading of the controller's own note**. Instance **1** of a ≥2 bar, pre-registered here, not a skill edit. **CROSS-MISSION ASK ANSWERED WITH A MEASUREMENT:** V1 iter-311 asked whether World is pin-rooted. `~/.ailang-driver-pin/world` EXISTS but is a worktree of **`sunholo-data/ailang`** and does **not** contain `design_docs/world-mission.md` (control: it DOES contain `v1-mission.md`); its HEAD `da96b98a5` is five days stale against that repo's `origin/dev` `a223e7274`; and the driver log's own provenance line reads `workdir: /Users/voightkampff/dev/sunholo-data/ailang-world` — the SOURCE CLONE. **World is pinned for the driver and skill and genuinely UNPINNED for the tree whose ledger Gate 0 reads**, which is V1's "genuinely unpinned" branch: the ordering edit stands with the scope V1 asked for. **ROUTING:** controller `claude:claude-opus-5`; designer **`claude:claude-fable-5`** (rotation's next entry after `pi:ollama/deepseek-v4-flash:0731-cloud`), probed rc=0 via `claude-sub`, AUTHORED (470 lines) and REVISED (470→627) rc=0 with a non-empty diff both times; rotation advanced. `metered=$0.2524` of $5 (R1 $0.1035 + R2 $0.1489); every non-quorum lane is a quota bucket. Also measured en route and written into the banked doc: **`go build ./host/verifygate/` is rc=1 on pristine `dev`** ("no non-test Go files" — test-only package), so an AC of that shape is broken at base; `go vet ./host/verifygate/` is rc=0 and is the correct compile gate. **NEXT:** rows **53**, **54**, **55**, **56**, **57**, **58**, **59**, then **39**; rows **48**, **50** and **52** wait on `D-WORLD-28`, `D-WORLD-29` and `D-WORLD-30` — this iteration's three asks, one of them new.

## STATUS 2026-08-31 (iteration 141) — **ROW 51 IS LANDED, AND THE NUMBER THAT JUSTIFIES IT IS THE JUDGE'S *BEFORE* MEASUREMENT: 7 OF 9 CORE ARMS WALKED STRAIGHT THROUGH THE OLD TEST, SO 6 REAL MUTANTS WERE INVISIBLE UNTIL TODAY.** PR [#108](https://github.com/sunholo-data/ailang-world/pull/108) → squash [`d195717`](https://github.com/sunholo-data/ailang-world/commit/d195717). **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives — `#107` since the `2026-08-27T18:20:00Z` watermark (3 comments, none allowlisted) and, per the rotation-week catch, the predecessor `#89` re-read from the EPOCH, whose 3 directives all predate the watermark and were consumed by iter-139; run via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH. Decision ledger **16 rows, 2 OPEN** (`D-WORLD-28`, `D-WORLD-29`) — and I again read it AFTER Gate 1's fetch rather than before, because the shared skill's Gate-0 step 6 ordering is the defect iterations 139 and 140 recorded; `dev` was already == `origin/dev`, so this time the ordering could not have bitten and the inversion cost nothing. Weekly external-issue sweep re-run: **0 orphans of 1 enumerated**, per-issue counts printed, positive control firing at 3 on a known-tracked issue, negative control fired on a fresh literal deliberately not published here. Inbox **27 unread, none actionable for this mission** — V1 controlplane traffic, two `interface_hash` notes addressed to package agents, a `pkg:sunholo/external_backend` task completion, and rows this mission itself posted at iters 139–140. **GATE 1:** `dev` == `origin/dev` == `2c698da`; tree clean; **0** open PRs; **0** stale worktrees; skill drift **CLOSED for the eleventh consecutive iteration** (`readlink` → `/Users/voightkampff/dev/sunholo-data/ailang/.claude/skills/mission-control`, `cmp` against `origin/dev` IDENTICAL, run against the RESOLVED symlink target and not the relative path); CI **GREEN** on `2c698da`, `checks=2`, both `success`, SHA-addressed with `total_count` as the known-positive control. **GATE 2 — THE PICK WAS REPRODUCED IN BOTH DIRECTIONS BEFORE ANY ROUTING.** Row 51 is an evaluator's inherited claim from iter-132, so it was measured rather than forwarded: a fabricated 7th coupled-site row added to the `scripts/verify_ail.sh` home ONLY leaves the test **rc=0 `--- PASS`** (sha `5a1bbe89…`→`4738e3fa…`, block row count 6→7, `bash -n` rc=0) — and so does the SYMMETRIC arm the row does not record, the same row added to `coding-standards.md` §S8 ONLY (`b710a510…`→`63208d62…`, S8 count 6→7). Both restored byte-identical, pristine control green either side, porcelain 0. **QUORUM: R1 BLOCKED (3 external rejects + controller reject, `absent_reviewers` EMPTY, $0.1368) → ONE designer revision → R2 BLOCKED at FULL STRENGTH ($0.1642 + $0.0417 to restore `oc-glm-5-2`, absent on `invalid`, whose raw text already read `reject`) → CLEARED UNDER THE NARROW-REFINEMENT CARVE-OUT.** R1's decisive finding was the controller's own and it was simulated, not asserted: the designer's comparator checked ONE direction plus length equality, which is **not set equality** — script gains a new site while S8 gains a DUPLICATE of an existing one → both length 7, every S8 site present → **GREEN** with the sets genuinely different; and it made the doc's own AC4 unsatisfiable, since with arm A1 landed only the cardinality branch can fire and that message names counts, never the site. Both closed by a reverse-direction loop plus a per-home duplicate `t.Fatalf`. `gpt5-6-sol` separately proved **N2 impossible as written** (floor neutered + I1 landed reds on six membership Errorfs) — measured true, and the arm was replaced by a PAIR (both enumerators neutered, floor absent → GREEN; the same mutant with the floor present → `instrument failure` Fatalf), because a single arm cannot isolate the floor. **WHY THIS TOOK THE CARVE-OUT WHERE ITERATION 140 PARKED:** all three surviving R2 objections are completeness / verification-publication / drill-attribution defects and **not one disputes the design DIRECTION** — the exact inverse of iteration 140, where the surviving objection attacked the direction AND the ratified queue row behind it. `gpt5-6-sol` asked for a reuse audit that had genuinely never been run: run verbatim, the three proposed symbols are **0** repo-wide against a same-scope known-positive control of **9** `regexp.MustCompile` in `host/`, the closest analogue is an unexported `jobLine` YAML matcher in another test function, there is exactly ONE markdown-pipe split in the whole repo (another package's unrelated test), and `go.mod` has ONE direct dependency and zero parsing libraries — CONCLUSION REFUTED, audit PUBLISHED as §Conflict Surface. `gemini-3-1-pro` was CONFIRMED: arms D1/D2 duplicated a KNOWN row, which the pre-existing per-row `strings.Count(...) == 1` loop intercepts, so the arm redded for the legacy reason; its verbatim fix (duplicate a FABRICATED 7th row) was applied and AC13 gained an attribution assertion (`grep -c 'want exactly 1'` must be **0**). `oc-glm-5-2` was CONFIRMED as a doc gap and its underlying claim measured TRUE, so V13 now publishes the greps. Per this mission's **iteration-98 guardrail** — a carve-out fix enters the doc after the round that reviewed it, so the last eyes on it are the author's — **every applied fix acquired its own AC and its own named mutation**: AC14/C1, AC15/C2, AC13's attribution clause. **THE PLANNER (`opus`, lane `opus fail-closed:env-pin`) RAN ALL FIFTEEN ARMS BEFORE WRITING THE PLAN AND REFUTED SIX THINGS, ONE OF THEM MINE.** Doc-side: C1 is a **no-op as written** (Go redeclaration is package-scoped, so a collision planted in `host/broker` leaves both vets rc=0 — it must sit in `host/verifygate`); C1's landed-gate counts are wrong; A4's effect gate must NOT move, violating the drill's own must-MOVE rule; R2's named §S8 row 3 is confounded as the sole home of the `REQUIRED_VERIFIED` sharedNeedle; AC8 needs a dual per-row-plus-floor verdict. Controller-side: **`scripts/verify_go.sh` returned rc=0 with an EMPTY failing set**, against the rc=1/one-test base I had handed it under a VERIFIED-BY-ME label and iteration 140's rc=1/four-tests. Four runs in ~90 minutes, two red, two green, three different failing sets — **the gate is FLAKY on this rig, not deterministically red**, and my Gate-2 red was itself on a DIFFERENT mechanism (`exec_started=false forked=false`) from the one row 58 names. Row **58 is AMENDED** accordingly; the rig-cost mechanism stands, the *property* does not. The planner also caught its own first N2a/N2b attempt silently failing to land and reporting both arms backwards — the landed-proof is what caught it. **EXECUTOR `codex:gpt-5.6-sol`, rc=0, 15/15 arms matching prediction, ZERO git writes.** It correctly REFUSED to run `verify_go.sh` because that gate reaches `host/daemon/daemon.go:634 net.Listen("tcp", …)` and a `workspace-write` loopback denial is indistinguishable from a regression. Both protected files end byte-identical (`5a1bbe89…`, `b710a510…`), re-asserted after every arm; §S8 was never edited. Milestone commits reconstructed from cumulative `.snap/` snapshots and proved faithful by sha256 (`d9bd7a81…` both sides), with the package suite green at BOTH boundaries. **EVALUATOR `sonnet` 97/100 PASS, ZERO BLOCKING** in its own worktree (generator≠judge four ways: pi/deepseek designer, opus planner, codex executor, sonnet judge, opus controller). It measured the counterfactual — reverting only the assertion block, leaving the helper compiled — and **7 of 9 core arms SURVIVED the old test**; the two that died died on the pre-existing per-row loop. Excluding A3 (expected-PASS in both states), **6 real previously-invisible mutants** are newly caught. It then invented five arms nobody had run. **ITS ONE NON-BLOCKING FINDING IS THE ITERATION'S MOST DURABLE ARTIFACT, AND I REPRODUCED IT RATHER THAN INHERITING THE LABEL:** wrap the whole new assertion block in `if false { … }`, land arm A1 — mutant LANDS (`d9bd7a81…`→`ad86884d…`), block count moves 6→7, `go vet` rc=0, test returns **rc=0 `--- PASS`**, and **AC2's own `grep -c` still reads 3**, its passing value. The shipped code is correct; the *acceptance criterion* is not. **That is row 49's defect one layer up** — row 49 replaced a token count with an AST pass because a count cannot see shape-gutting, and the judge then measured 22 of 23 mutations walking through it; row 51 then wrote its own load-bearing proof as a token count. Filed as **NEW ROW 59**, not absorbed. **GATES, ALL RE-RUN BY THE CONTROLLER OUTSIDE THE CODEX SANDBOX:** `gofmt -l host/verifygate/` **0 bytes** · `go vet` rc=0 · `go build ./...` rc=0 · `go test ./host/verifygate/ -count=1` **52 RUN / 52 PASS / 0 SKIP** at both milestone boundaries · `./scripts/verify_ail.sh` rc=0 · **`./scripts/verify_go.sh` rc=0 with an EMPTY failing set** · **0** `.ail` files touched · porcelain 0. **GATE 3b GREEN ON THE MERGE COMMIT** `d195717`, SHA-addressed from a 40-char `rev-parse` with the length asserted `== 40` before the call: `present=2` **== expected=2**, expected enumerated from `ci.yml`'s own `jobs:` block scoped BELOW `jobs:` (`ailang-verify`, `go-verify`), `ls .github/workflows/` = **1** so the enumeration is complete by construction, not a hand-picked subset; every count asserted NUMERIC; `mergeable` read FIRST (`MERGEABLE`/`CLEAN`); parent control `2c698da` resolves at **2** checks. Post-merge the local checkout was 2-ahead/1-behind on squashed-duplicate content, reconciled with the protective `git checkout -B dev origin/dev` after confirming the trees are identical. **ROUTING:** controller `claude:claude-opus-5`; designer **`pi:ollama/deepseek-v4-flash:0731-cloud`** — the rotation's next entry after fable, probed rc=0, and it AUTHORED (422 s, 26 tool calls, 554-line doc) and REVISED (112 s, 23 tool calls) with typed verdict `ok` and a non-empty worktree diff both times, which is the first time this mission has a clean two-for-two designer record on that lane; planner `opus`; executor `codex:gpt-5.6-sol`; evaluator `sonnet`. `metered=$0.3427` of $5 — quorum R1 $0.1368 + R2 $0.1642 + the restored reviewer $0.0417. Every non-quorum lane is a quota bucket. **NEXT:** rows **52**, **53**, **54**, **55**, **56**, **57**, **58**, **59**, then **39**; rows **48** and **50** remain parked on `D-WORLD-28` and `D-WORLD-29`, which are this iteration's two unchanged asks.

## STATUS 2026-08-31 (iteration 140) — **`P50` PARKED `needs-human-review` ON `D-WORLD-29`: TWO FULL-STRENGTH QUORUM ROUNDS BOTH BLOCKED, AND THE SURVIVING OBJECTION IS THAT THE DESIGN — AND THE QUEUE ROW BEHIND IT — REST ON A SHELL-SEMANTICS PREMISE THE CONTROLLER THEN MEASURED FALSE.** Design doc written, revised once and BANKED at `design_docs/planned/w-shell-assignment-parser-drops-an-indented-assignment.md`; no code was routed, so no planner, executor or evaluator ran. **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives — `#107` (created `2026-08-31T09:26:51Z`, 2 comments, 0 from the allowlist) and, per the rotation-week catch, the predecessor `#89` read from the EPOCH, whose 3 directives all predate the `2026-08-27T18:20:00Z` watermark and were consumed by iter-139; both watermarks read and the OLDER taken (the issue-scoped `mission-107-last-seen` is ABSENT, so the read fails safe to epoch), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH. Inbox **22 unread, none actionable for this mission** — V1 and motoko controlplane traffic, `mark-attended-steering` notes scoped to V1's controller chain (driver territory, frozen core here), an `ailang-core` parse bug, and three rows this mission itself posted at iter-139. **THE GATE-ORDERING FRICTION IS NOW INSTANCE 2 OF 2, AND IT DID NOT BITE, BECAUSE I INVERTED THE ORDER:** Gate 0 step 6 mandates `mission_decisions.sh --check/--open` BEFORE Gate 1 syncs to origin, which is exactly how iteration 139 read a 2-commits-stale ledger and got a confident, specific, wrong **"14 rows, 0 OPEN"**; this iteration fetched FIRST and read the ledger against the reconciled base — **15 rows, `D-WORLD-28` OPEN** — correctly. Iteration 139 pre-registered the ≥2-friction bar; this is the second instance and it is a real ordering defect in the SHARED skill, so it is PROPOSED to V1 rather than absorbed (World cannot edit the shared skill). **GATE 1:** `dev` == `origin/dev` == `9c0ad0b`; tree clean; **0** open PRs; **0** stale worktrees; no uncommitted residue in either tree; skill drift **CLOSED for the tenth consecutive iteration** (`readlink` → same inode `60291442`, `cmp` IDENTICAL to `origin/dev`); CI **GREEN** on `9c0ad0b` — `checks=2`, both `success`, read SHA-addressed with `total_count` as the known-positive control. **GATE 2 — THE PICK WAS REPRODUCED BEFORE ANY ROUTING, AND THE TWO ARMS ARE THIS ITERATION'S MOST DURABLE ARTIFACT:** the silent arm (a second, INDENTED `KNOWN_BAD="go1.26.0"` inside an `if` block, beside the valid column-0 line) leaves both consumers **rc=0 RUN=2 PASS=2 FAIL=0** — the gate is blind — while the loud arm (the same assignment indented-ONLY) gives **rc=1 RUN=2 PASS=0 FAIL=2** with `assignment count=0, want 1` at `toolchain_pin_gate_test.go:267` and `:351`. Both mutants asserted LANDED by sha256, `bash -n` rc=0, intended effect asserted against the system's own view (`grep -cE '^[[:space:]]+KNOWN_BAD='` and `'^KNOWN_BAD='`) rather than against bytes, restored byte-identical, pristine control re-passing before and after, porcelain 0. So the row's claim reached the designer as a MEASUREMENT, never as an inherited evaluator claim. **QUORUM, TWO ROUNDS, BOTH BLOCKED — AND BOTH ROUNDS' OBJECTIONS WERE MEASURED RATHER THAN FORWARDED.** R1 ($0.0997): `gemini-3-1-pro` alleged a **fabricated** verification log because V-K's `grep -nE` carried escaped pipes; measured — as-printed **rc=1, 0 hits**, bare-pipe form **rc=0, 3 hits** at lines 24/25/26 — so the CONCLUSION is true and the COMMAND could not have produced it: a transcription defect from markdown table-cell `\|` escaping, CONFIRMED but not fabrication, and the honest verdict is neither the reviewer's nor the author's. `gpt5-6-sol`'s reuse-audit premise was **REFUTED** (`go.mod`: ONE direct dependency `modernc.org/sqlite`, zero shell-parsing libraries; the sole adjacent tokenizer is unexported, a different grammar, and in another package's `_test.go` — 4 hits against a same-scope known-positive control of **59** `HasPrefix`) — yet the audit it asked for had genuinely never been run, so the fix was to PUBLISH the audit, not to change the design. R2 ($0.1314) still BLOCKED, and the objections **localised onto the design direction** rather than spreading: `gpt5-6-sol` says leading whitespace does not make an assignment nested or conditional in bash. **I MEASURED IT AND THE REVIEWER IS RIGHT** — `COL0=`, `   INDENTED=` and a tab-indented `TABBED=` at top level all print their values, rc=0, with a paired control showing that a genuinely *conditional* assignment DOES change behaviour (`NARROW` unset → `wide list`, set → `narrow`). That refutes the doc's central rationale — which is the same finding I had praised one gate earlier — and it means the loud red the doc was protecting fires because the scanner cannot SEE a real assignment. **THE ABSENT REVIEWER WAS RESTORED RATHER THAN LEFT AS A NAMED HOLE:** `oc-glm-5-2` was recorded absent (`invalid`) in BOTH rounds; re-run alone at a raised cap it returns **PASS, $0.0203**, so round 2 is 2 rejects to 1 pass at FULL strength with `absent_reviewers` closed — and its non-blocking objection travels into the row, because `export KNOWN_BAD=` inside the same `if` leaves the IDENTICAL silent-narrowing hole open under EITHER candidate rule. **WHY THIS PARKS RATHER THAN TAKING THE NARROW-REFINEMENT CARVE-OUT:** the bounded one-revision-one-requorum allowance is spent, and the carve-out is available only when NO remaining objection disputes the design DIRECTION. This one does — and the deeper reason it is a human call is that **queue row 50 itself encodes the false premise** (it lists an indented-only assignment reading 0 and fataling among the correct loud behaviours), so answering `D-WORLD-29` amends ratified queue text, which is not the loop's to edit. **A SECOND, UNASKED-FOR FINDING, AND IT LANDS ON THIS LOOP'S OWN INSTRUMENT:** `scripts/verify_go.sh` is **rc=1 on PRISTINE `dev`**, so every sprint's `verify_go.sh rc=0` acceptance criterion is broken as written. I had introduced a load confound (two full suites plus a designer session in parallel) and controlled it — a second, UNLOADED arm returns the **identical** four-test failing set, `diff` silent — then isolated the mechanism instead of calling it environmental: the pinned binary answers `--version` in **96 ms**, a freshly-copied instance at a new path takes **1336 ms** on first exec and **372 ms** cached (macOS provenance/code-signing assessment; `spctl -a -t exec` rejects), against the tests' **10 s** deadline, on top of a per-invocation Observatory retention cleanup over a **513 MB** DB that every `ailang` invocation now pays. Rig-local, not a repo defect — **dev CI is GREEN on the same commit** — and filed as **NEW ROW 58** with the AC-repair prescription, since iter-139 recorded this same gate `rc=0` about five hours earlier. `verify_ail.sh` is rc=0 at base. **ROUTING:** designer = `claude:claude-fable-5` (rotation advanced from `pi:ollama/deepseek-v4-flash:0731-cloud`), probed rc=0, authoring + **one** protocol-mandated revision = the Fable diet's one-DOC ceiling, met and not exceeded; planner lane derived as `opus fail-closed:env-pin` and never spawned, because no plan is owed for a parked doc. `metered=$0.2514` of $5 (R1 $0.0997 + R2 $0.1314 + the restored reviewer $0.0203). **NEXT:** rows **51**, **52**, **53**, **54**, **55**, **56**, **57**, **58**, then **39**; rows **48** and **50** wait on `D-WORLD-28` and `D-WORLD-29`, which are this iteration's two asks.
...
48. **[PARKED `needs-human-review` 2026-08-28 (iter-137) on D-WORLD-28] w-racecontrol-floor-bump-disarms-the-race-control** · clause-2 · **THE SIBLING NESTED MODULE
...
52. **[PARKED `needs-human-review` 2026-09-01 (iter-142) on D-WORLD-30 — DESIGN DOC WRITTEN AND
