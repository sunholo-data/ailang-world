...
2. [IN-SPRINT — milestone 1 of 6 LANDED 2026-07-24 (iter-8), CI green]
   **w-world-library-m1** · clause-1 · Sprint AUTHORIZED (Mark, attended, iter-8-prior). Sprint plan
   ...
   PR #2 → squash `9d61d663e`, evaluator PASS 93/100 (generator≠judge). **Remaining: M2** Go
   bootstrap + `host/hashref` + `host/canon` + `go.mod` + golden SHA-256 tests → M3 SQLite store →
   M4 archive+epoch registry → M5 replay+replay-doubling → M6 CI Go gate. → [LANDED] + doc→implemented/
   when M6 lands.
