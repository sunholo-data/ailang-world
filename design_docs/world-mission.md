...
3. [IN-SPRINT — r3 APPLIED, plan approved, **MILESTONES M2.A + M2.B COMPLETE: A1 LANDED 2026-07-27 (iter-18), PR #10 → squash `b0deedb`; A2 LANDED 2026-07-27 (iter-19), PR #11 → squash `39b2115`; A3 LANDED 2026-07-27 (iter-20), PR #12 → squash `9579fe1`; M2.B LANDED 2026-07-27 (iter-21), PR #13 → squash `b412699`; all four dev CI green, both jobs. Next: M2.C, which LANDS the item**] **w-worldd-m2** · **single-writer fork resolved: ENFORCE (arm A), ratified by Mark attended and now SHIPPED — `store.Open` is the fail-closed writer path (non-waiting `flock(LOCK_EX|LOCK_NB)` on `<canonical-db>.writer.lock`, structured `WriterAlreadyActive`), additive `store.OpenReadOnly`, in-memory DSNs exempt, `schema.sql` byte-unchanged. Enforcement is proven CROSS-PROCESS (subprocess helper + READY handshake + <2s non-waiting assertion + negative control); a controller mutation (`LOCK_EX`→`LOCK_SH`) turns the suite red, so an in-process mutex cannot pass. evaluator PASS 97/100, zero blocking. **A2 (iter-19) LANDED the daemon shell**: `cmd/ailang-worldd` + `host/daemon` (loopback guard mirroring the Z3-proven `isLoopbackHost` — non-loopback bind REFUSES startup, no override flag; the D7 bound-constant block with all four `http.Server` timeouts + `maxCommitBytes` pinned to the Z3-proven `withinCommitBytes`; fail-closed lifecycle, stdout address announce, bounded drain then hard close; `/v1/health` + `/v1/head`), plus **`TestDaemonDependencyAllowlist` — zero-cloud now ENFORCED, not asserted** (walks `go list -deps` over both daemon-core trees, fails loudly if `go` is missing, fails on an empty list, names offenders). The evaluator BLOCKED round 1 because that allowlist test had been dropped; closed in a bounded round-2 fix and re-verified → **92/100**. A1's writer lock verified holding at its first real consumer: a second OS process on the same DB fails closed in 0s. **A3 (iter-20) COMPLETED M2.A**: Decision 6's perf machinery — `host/daemon/bench_test.go` (`BenchmarkStoreCommit` on a fresh temp-FILE store so fsync reality is in the number, `BenchmarkHeadRead` + `BenchmarkHealth` over real loopback; per-iteration samples → p50/p95 via `b.ReportMetric`), `scripts/bench_worldd.sh --smoke` asserting a HARDCODED BENCHMARK-NAME MANIFEST (a dropped benchmark still exits 0 from `go test` — the V27/B1 vacuous-pass class, now closed for benchmarks), the committed `bench/BASELINE.md` (store-commit p95 0.6093 ms, head-read 0.08596 ms, health 0.06288 ms — all inside the day-1 budget with 32–58× headroom; REST-commit + both log-range rows explicitly PENDING M2.B), and the CI bench-smoke step (confirmed RUNNING on the linux runner, not merely wired). Carry-forward CF-7 closed and PROVEN load-bearing by mutation. Executor was `codex:gpt-5.6-sol` — pin honoured, first successful real run of that lane — and it declined to invent the HTTP numbers its sandbox could not measure; the controller measured them outside the sandbox. evaluator PASS 89/100, zero blocking. **M2.B (iter-21) LANDED THE FULL REST v1 SURFACE**: the remaining five frozen routes + `POST /v1/commit` — payload opt-in behind `?payload=true` (bounded default response), `GET /v1/log?from&limit` as a bounded loop over the existing `GetLogEntry` with **zero new store methods** and `limit` through the Z3-proven `clampLimit`, the multi-segment `{name...}` registry wildcard, one shared JSON error envelope mirroring the sketch's `httpStatus` exactly, and the body capped at the Z3-proven `maxCommitBytes` → 413. **The evaluator BLOCKED round 1 (82/100) on a real defect: `POST /v1/commit` rejected a genesis commit that the embedded kernel ACCEPTS** (400 `observedHead: hashref: empty hashref text`), so "a genesis+commit episode driven entirely over REST" was unreachable — found independently by the codex executor (which flagged it and declined to invent a zero-ref encoding), by a first-party controller probe, and by the judge (which also named the cause: a helper called `seedRESTGenesis` that used the embedded API). Closed by `parseGenesisRef` accepting `""` as the zero HashRef for **`observedHead` only** — not a second encoding, since `HashRef.String()` already renders zero as `""`. **Deliberately NOT extended to `prevEntryHash`**: `store.Commit` writes a zero there that `store.GetLogEntry` cannot read back, so accepting it would let a client append an unreadable log entry — an M1 kernel asymmetry now carried as CF-B-2. Five controller mutations RED→GREEN; evaluator round 2 PASS 89/100, zero blocking. `bench/BASELINE.md` now covers the FULL surface with NO PENDING rows (one 200x invocation): REST commit 0.5763 ms p95 (61×), log range 1.248 ms @100 and 4.915 ms @500 — 3.94× time for 5× rows, so the deliberate N+1 is linear, 24× inside budget, and **no range-read store method is justified by the data**. Remaining: M2.C (CLI client + real-subprocess end-to-end + close-out), which LANDS the item.** ·
