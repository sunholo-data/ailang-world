...
## STATUS 2026-08-28 (iteration 137) — **P48 PARKED `needs-human-review` ON D-WORLD-28 AFTER TWO FULL-STRENGTH QUORUM BLOCKS.** Gate 0–1 clean: armed, correct `gh` account, billing CLEAN, tree synced, dev CI green, skill byte-identical to origin, zero Mark directives, zero open PRs/stale worktrees, decision ledger initially 0 OPEN. Queue head row 48 reproduced at HEAD and was not already landed. Fable designer probe hit weekly capacity (resets 2026-08-31 07:00 local), so the ratified rotation fell to `pi:ollama/deepseek-v4-flash:0731-cloud`; authoring and revision both returned `ok`. Quorum R1 ($0.1236) rejected the unrelated `KNOWN_GOOD` anchor and required measuring comment parsing; revision bound execution to root `ACTIVE_GO` and verified the comment parser. Quorum R2 ($0.1727) still rejected: the doc's escaped alternation ran zero baseline tests, CI linkage was unverified, and `GOTOOLCHAIN=local` below the root floor refuted the core implication. All three external reviewers were present in both rounds; `absent_reviewers` empty. The bounded revision allowance is exhausted and the remaining objection disputes direction, so no planner/executor/evaluator ran. **Decision D-WORLD-28 asks A (recommended fail-closed `ACTIVE_GO >= root floor` + exact binding) vs B (ambient auto-selection + independent nested floor).** `metered=$0.2963`; Next: rows 49–55, then 39, while 48 waits on Mark.
...
## STATUS 2026-08-28 (iteration 136) — **`P47` IS LANDED, AND THE HEADLINE IS THAT MY OWN MUTATION DRILL RETURNED FOUR PERFECTLY SYMMETRICAL ARMS AND ITS GREEN CONTROL IS THE ONLY REASON I DID NOT BANK THEM: EVERY ARM READ `rc=1 PASS=0 FAIL=0` BECAUSE THE READER WAS BROKEN, NOT BECAUSE THE MUTANTS WERE CAUGHT.** **`P47` IS LANDED.** PR [#103](https://github.com/sunholo-data/ailang-world/pull/103) → squash [`2a01c43`](https://github.com/sunholo-data/ailang-world/commit/2a01c43). **Gate 3b GREEN on the MERGE commit**, SHA-addressed from a 40-char `rev-parse` (length asserted `== 40` before the call): `present=2` **== expected=2**, and the expected count was **enumerated from `ci.yml`'s own `jobs:` block** — *scoped to below `jobs:`* after my first grep over the whole file returned **5**, because this very sprint added `workflow_dispatch:` at the same indentation as the job keys, so the item I landed broke the enumerator I was about to certify it with; `ls .github/workflows/` = **1**, so the enumeration is complete rather than a hand-picked subset; `notgreen=0`, `notdone=0`, `runs=1`, parent control `checks=2` **rev-parsed**; `mergeable` read **FIRST** (`MERGEABLE`/`UNSTABLE`), so the boring cause was excluded before any dropped-event theory; every count asserted NUMERIC before comparison and the poll ran from a **script file**. **THE DEFECT.** `.github/workflows/` holds one file and its `on:` block declared only `push:`/`pull_request:` — `grep -c workflow_dispatch` **0, rc=1** (a legitimate zero read from the exit code, never defaulted), same-file controls `pull_request`=**1** and `^  push:`=**1** firing, a fresh absent literal **0**, scope asserted `test -f`. A dropped webhook is never replayed, so on `dev` there was no lever at all — iterations 128 and 129 each paid for it. **THE CLAIM IS SCOPED, WHICH WAS THE WHOLE DIFFICULTY:** the lever buys **a verdict on a commit**, never **a mergeable PR**, and re-verifies **the tip of a named ref**, never an arbitrary SHA (`gh workflow run --ref` is documented *"Branch or tag name"*; `default_branch`=`dev` — both measured). **QUORUM: TWO ROUNDS, BOTH BLOCKED AT FULL STRENGTH** (`absent_reviewers` **EMPTY** both times, **3/3** external reviewers present, caps pre-raised to `$0.20`/`$0.25` so the self-selecting budget trap never fired). R1's three objections hit three different surfaces; R2's **localised onto one** — the parser's `on:` anchoring — which is the documented decomposition tell, though no reviewer flipped to pass, so it is one helper needing three repairs rather than a SPLIT. Closed under the **ratified narrow-refinement carve-out** (ratified iter-44, no first-use gate): all three carried concrete reviewer-authored `proposed_fix` text and **none disputed the DIRECTION**. **AND A REVIEWER'S CITED CONTROL WAS REFUTED BY MEASUREMENT WHILE ITS FIX WAS APPLIED ANYWAY — the distinction is the row.** `oc-glm-5-2` justified exact-equality by saying it matches P14's `awk '{$1=$1}; $0=="on:"'` *"exact match at column 0"*. Measured, two arms: that awk prints MATCH for a column-0 `on:` **and** for an indented `  on:`, because `{$1=$1}` rebuilds the record and strips leading whitespace; `grep -c '^on:$'` returns **0** vs **1** (control fires). **Neither the parser NOR the premise row that certified it was column-0 aware** — the objection was STRONGER than filed — so the fix was applied AND P14's own command repaired. **THE INSTRUMENT FAILURE, AND IT IS THE FINDING THAT TRAVELS.** My first four-arm drill returned `rc=1 PASS=0 FAIL=0` on ALL FOUR arms **and on the pristine green control**. Symmetric arms are equally consistent with *"the variable does not matter"* and *"both arms are already broken"* — and here the thing the arms shared was the READER: `go test` invoked with an ABSOLUTE package path from the MAIN CHECKOUT's module context returns `directory … outside main module`, `[setup failed]`, rc=1, **zero tests run**. Two readers on one identical pristine tree: broken `rc=1/PASS=0`, correct `rc=0/PASS=1`. Had the green control not been in the set I would have recorded four caught mutants from a drill that never executed a single test. Re-run correctly with a `=== RUN`-count floor: **4/4 RED**, green controls `rc=0/RUN=1/PASS=1` both sides, every restore byte-identical by sha256. **The generalisation is the one this mission already owns, aimed at my own hands: the location you run a check FROM is part of the instrument.** **EVALUATOR `sonnet` 82/100 PASS, ZERO BLOCKING** (generator≠judge: executor is OpenAI's codex), in its own worktree — and it **named the attacks that FAILED**, which is what makes it a measurement: hidden dotfile, nested subdirectory, case/extension variants, all nine canonical mutations, and the anti-vacuity floor's own precondition test. **None produced a false green.** Two findings reproduced first-party and **fixed in-PR** at [`eb215c3`](https://github.com/sunholo-data/ailang-world/commit/eb215c3): `workflow_dispatch: # manual re-run lever` — valid YAML, the most natural maintainer edit — tripped the scalar branch and redded CI, with the discriminating control that the SAME comment on its own line was `rc=0/PASS=1`; and the doc claimed *"the code comment … state[s] it plainly"* about the named-ref window while the shipped comment had **0** hits for named-ref/tip/SHA (control firing at **2**). The hardening was proven **NON-WEAKENING** by the arm that would catch over-approximation: `workflow_dispatch: garbage # note` **still REDs**. Three residual findings → **NEW ROW 55**. **GATES, ALL RE-RUN BY THE CONTROLLER OUTSIDE THE CODEX SANDBOX** (mandatory; the executor correctly labelled its own whole-package runs `UNINFORMATIVE UNDER SANDBOX`): `gofmt -l` **0 bytes** · `go vet ./...` rc=0 · `go build ./...` rc=0 · `go test ./host/verifygate/` rc=0/**0 FAIL** · `scripts/verify_go.sh` **rc=0** (9 passed) · `scripts/verify_ail.sh` **rc=0** at the **UNCHANGED** floor (11/11 identities, 40 named tests, 9/9 package steps; **0** `.ail` files touched). The **2** `WARNING: DATA RACE` lines attributed — **sixth** iteration running, and this time **verified from the stack frame** (`racecontrol/main.go:16` under its own known-positive banner) rather than inherited. Commit reconstruction from `.snap/MS1`,`.snap/MS2` proved faithful: manifest taken BEFORE the first commit, gates green at every boundary, final tree **byte-identical** by `shasum -c`. **ROUTING — THE DESIGNER ROTATION'S NEW SECOND LANE AUTHORED ON ITS FIRST TRY.** `pi:ollama/deepseek-v4-flash:0731-cloud`, added by the 2026-08-28 amendment, returned verdict `ok` **twice** (authoring 304 s / 29 tool executions → 369 lines; revision 203 s / 20 → all three fixes landed and verified), at **$0** flat-rate. Pointer advanced. Planner `opus`, lane derived VERBATIM `opus fail-closed:env-pin`. **THE PLANNER RETURNED FIFTEEN FINDINGS AND THE SHARPEST ONE IS AGAINST ME:** the doc shipped **6 ACs against 9 mutations**, with **0** ACs naming M7/M8/M9 — i.e. every construct my own carve-out added had no acceptance criterion — plus **3** stale `M1–M6` ranges and a preamble calling M5 a test-file arm when it is a YAML arm. It also proved by RUNNING it that M8's placement decides its blast radius (appended after `jobs:` it is multi-owner; inserted before, single-owner). Repaired by **re-deriving the SET**: 3 range references, all moved to `M1–M9`, none missed. **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; tree clean; billing **CLEAN**, re-checked in the tool shell before every nested provider call; **0** `MarkEdmondson1234` directives on `#89` since `2026-08-27T18:20:00Z` (both watermarks agree, 24 comments) via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH. Inbox **5 unread, none addressed to this mission**. Ledger valid, **14 rows, 0 OPEN**. Died-mid-flight sweep clean on all three traces (0 open PRs, `git worktree list` scoped to this clone, porcelain 0). dev CI **GREEN at entry** at `3a98c79` (`checks=2` both `success`, `runs=1`, parent control `checks=2`). Rotation not due (`#89` created `2026-08-24T07:03:49Z`, after the Monday-07:00 **local** boundary; 24 comments < 80). **Skill drift CLOSED for the eighth consecutive iteration** (`readlink`, same inode, `cmp` IDENTICAL to `origin/dev`). **RETRO — the friction is MINE and it is the THIRD consecutive iteration of the same shape:** my carve-out edit added three mutations and left four downstream sites describing the old set, and the **planner** found it, exactly as the planner found iteration 133's and the executor found iteration 132's. I swept for placeholder tokens because I had told the designer to; I did not sweep for mutation-RANGE references after adding mutations. **NEXT:** rows **48**, **49**, **50**, **51**, **52**, **53**, **54**, **55**, then **39**. `metered=$0.1900` of `$5`. **ZERO OPEN ASKS.**

## STATUS 2026-08-28 (iteration 135) — **`P46` IS LANDED, AND THE HEADLINE IS THE SECOND FINDING, NOT THE FIRST: THE GATE I SHIPPED TO CLOSE THE DEFECT CLASS WAS BUILT FROM ONE MUTATION AND PROVEN AGAINST THAT MUTATION, SO ITS GREEN SAID "THIS DEFECT IS GONE" IN THE SAME VOICE IT WOULD HAVE USED FOR "THIS CLASS IS CLOSED" — AND THE ARM THAT EXPOSED IT ALSO EXPOSED THAT `go build ./...` CANNOT SEE A TEST-FILE MUTATION AT ALL.** **`P46` IS LANDED.** PR [#102](https://github.com/sunholo-data/ailang-world/pull/102) → squash [`d8c2114`](https://github.com/sunholo-data/ailang-world/commit/d8c2114). **Gate 3b GREEN on the MERGE commit**, SHA-addressed from a 40-char `rev-parse` (length asserted `== 40` before the call): `present=2` **== expected=2**, expected **enumerated from `ci.yml`'s own `jobs:` block** (`ailang-verify`, `go-verify`) rather than counted from what turned up, with `ls .github/workflows/` confirming `ci.yml` is the ONLY workflow so the enumeration is complete rather than a hand-picked subset; `notgreen=0`, `notdone=0`, run existence `runs=1`, parent control `checks=2` **rev-parsed**; `mergeable` read **FIRST** (`MERGEABLE`/`CLEAN`); every count asserted NUMERIC before comparison, and the poll ran from a **script file** rather than an inline heredoc. **THE DEFECT, AND WHY BOTH CI AND A QUIET RIG MISS IT BY CONSTRUCTION.** `TestCLIRealSubprocessEpisode` binds a bare `bytes.Buffer` as `cmd.Stderr`, parks `cmd.Wait()` in a goroutine, and reads `daemonErr.String()` on two of the announcement select's four branches — `<-readErr` and the 5 s timeout — **without having received from `waited`**, which is the only thing that guarantees `os/exec`'s stderr-copier goroutine has stopped. On an unloaded rig the daemon announces in time, the `announced` branch wins, and **neither racy read is executed at all** — so the row's reported non-reproduction (`-race`, rc=0, zero warnings) was a true statement about the run and no statement about the code. **MEASURED, TWO ARMS, ONE MACHINE:** a standalone reproduction of the bare-buffer pattern is `WARNING: DATA RACE` plus `race detected during execution of test` under `-race` (**3 warnings in 5 runs** — the timing dependence, visible in the instrument); the same reproduction against a mutex-guarded sink is `ok`, **5/5 clean**. **THE FIX IS THE SINK, NOT THE BRANCH**, and the row's own suggested remedy would have deadlocked: waiting on `waited` inside the timeout branch waits for a daemon that has not exited, which is the very thing being reported. So `daemonErr` became a mutex-guarded `syncBuffer` mirroring the type `host/store/writer_lock_test.go:91` already defines for the same reason. **THE SET, NOT THE SITE** (iteration 134's retro, applied): repo-wide there are exactly **three** `Start()`-based subprocess sinks — this one and `host/store/{writer_lock,crash}_test.go`, **both already guarded** — while every other binding reads only after `cmd.Run()`/`CombinedOutput()` has waited. The class had been closed once and reopened by one file, which is what the new repo-wide AST gate `host/verifygate/subprocess_sink_gate_test.go` prevents. **AND THEN THE GATE FAILED THE SAME TEST IT WAS BUILT TO APPLY.** The `sonnet` evaluator (78/100 PASS, generator≠judge — the executor was codex) attacked the gate rather than the fix and named three shapes it could not see; all three reproduced first-party against the detector with a firing control on the fourth: `buf := bytes.Buffer{}` **0** (want 1), `buf := new(bytes.Buffer)` **0** (want 1), `Start()` inside a closure **0** (want 1), `var buf bytes.Buffer` + `&buf` **1** ← control fires. `declared` was populated only from `*ast.ValueSpec`, i.e. the one form the gate was mutation-tested against and **not** the one most Go authors reach for; the matcher accepted only `&ident`; and `hasStart` was computed per function body with literals EXCLUDED, so declaring and binding outside while calling `Start()` inside `go func(){}()` evaded it in one move with the defect fully intact. This is the mission's own recurring shape aimed at its own hands — **a stress control only certifies the axis you varied** — and it earns the headline because it was caught by the judge and not by the author. Hardened in-PR at [`49ad39e`](https://github.com/sunholo-data/ailang-world/commit/49ad39e): declarations from `var`, from `:=`, and from initialiser expressions with value/pointer kinds; the matcher accepts `&value`, a directly-bound `pointer`, and an inline `&bytes.Buffer{}`/`new(...)`; and each top-level function is walked **including** its nested literals, because over-approximating there can only make a LOUD false positive while under-approximating makes the silent miss the gate exists to prevent. Fixtures **2 → 9**. **THREE-ARM MUTATION CONTROL ON THE REAL TREE — ALL THREE CAUGHT**, exactly one violation each naming `cmd/ailang-worldd/cli_test.go`, restore sha256 **byte-identical**; and the gate reports `violations=0 filesParsed=110 startSites=5` on the fixed tree, so the widened walk produced no false positive either. **THE CONTROL THAT CORRECTED THE CONTROL, AND IT IS THE FINDING THAT TRAVELS.** Arm 3 first read **SURVIVED**, and the reason was not the detector: **`go build ./...` DOES NOT COMPILE TEST FILES**, so the *"assert the mutant BUILDS before you read any test result"* discipline this loop puts in front of every mutation verdict was **vacuous for every arm**, and would be for every test-file mutation any mission has ever run. Measured directly, two arms: a test file carrying an undefined symbol gives `go build ./...` **rc=0** and `go vet ./cmd/ailang-worldd/` **rc=1**. Re-run with `go vet` as the typecheck, all three arms typecheck AND are caught. Proposed to V1 as a skill-rule instance; World cannot edit the shared skill. **GATES, ALL RE-RUN BY THE CONTROLLER OUTSIDE THE CODEX SANDBOX** (mandatory — the diff touches `cmd/*`, and the executor correctly labelled its own CLI-race run `UNINFORMATIVE UNDER SANDBOX` rather than reporting it either way): `gofmt -l` clean · `go vet ./...` rc=0 · `go test -race -count=3 ./cmd/ailang-worldd/...` ok · the new gate PASS with 9/9 fixtures · `scripts/verify_go.sh` **PASSED** · `scripts/verify_ail.sh` **PASSED** (11 required identities, 40 named tests, 9/9 package steps). **AND ONE FINDING OFF THE PICK ENTIRELY, MEASURED WHILE READING THE ROUTING ENV → NEW ROW 54.** `launchd` runs **this repo's copy** of the fleet driver (`dev.ailang.mission-world.plist` names it by absolute path), and that copy is **430 lines shorter than the fleet's** (550 vs 980), **586 lines differ**, and it is missing **8 fleet commits** since 2026-08-15 — including `fc6e42682`, which dropped the `:floor` price-pin from the deepseek executor fallback precisely because it routes to the two least-healthy endpoints by construction. First-party proof it is live rather than theoretical: this session's own exported `MISSION_EXECUTOR_FALLBACK` is `pi:openrouter/deepseek/deepseek-v4-flash-0731:floor`, the dropped id. **`verify_go.sh`'s drift gate cannot see any of this**, because it is `git status --porcelain -- tools/launchd/` — the copy compared **to itself** — and its success line says so in words that read as the opposite: *"working tree matches HEAD"*. FLEET-owned under `D-WORLD-DRIVER-1`, so it is reported and not touched.
