...
| D-WORLD-33 | OPEN | **Redesign row65 around explicit mutation/compile-evidence records, or defer it?** REDESIGN (recommended): replace prose inference with explicit records and a fresh design/quorum; spends further harness work before implementation. DEFER: keep row65 parked and advance to rows66/68; the compile-fence guidance gap remains. Default if unanswered: DEFER immediately during unattended runs; no proposed lint ships. | Iter161: two blocked quorums; r1 three rejects, r2 Astra/Gemini reject and GLM invalid/absent. Controller reproduced Markdown-backtick blind spot with a firing plain-text control. design_docs/planned/w-go-test-compile-fence.md; design_docs/verification/world-iter161/README.md. |
| D-WORLD-35 | OPEN | **May a docs-only mission-record PR merge while the DE-FORK CI reds stay parked, or must every record wait?** A (recommended): MERGE docs-only mission-record PRs once the charter-declared Gate-3b job (`CI` / `ailang-code verify gate`) is green on the exact head and the other two checks are proven INHERITED — a documentation commit cannot make an already-red `dev` redder, and the reds are a separate, separately-parked question. B: HOLD every record until the CI reds are repaired — the current de-facto behaviour. Default while unattended: B (hold), because reversing a prior iteration's disposition is not an unattended call. **This is NOT the same question as the row-68/CI-repair park**: it asks only whether *records* are hostage to it. | Iter166. `sunholo-data/ailang-world` `dev` at `9166de0` is RED on `launchd drivers (bash 3.2)` and `go host build + test gate`, both because the attended DE-FORK `e92594c` deleted files those jobs invoke; `ailang-code verify gate` passes. PR [#127](https://github.com/sunholo-data/ailang-world/pull/127) (iterations 162-165's records) has been open and MERGEABLE since 2026-09-06 for this reason, and iteration 166's record PR joins it — so five iterations of charter/log/ledger state now live only on unmerged branches. Measured consequence, first-party: the committed ledger on `dev` contains no `D-WORLD-34` row (`rg -n 'D-WORLD-34' design_docs/world-mission.md` -> 0 hits, positive control `D-WORLD-DRIVER-1` -> hits), so `scripts/mission_decisions.sh --open` cannot see a park that iterations 162 and 165 both relied on, and the independent evaluator flagged exactly that provenance gap. |
<!-- decision-ledger:end -->
