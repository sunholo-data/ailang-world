...
2. [IN-SPRINT — milestones 1–5 of 6 LANDED 2026-07-24 (iter-8/9/10/14/15), CI green]
   ...
   **v0.30.0** binary (by TAG, sha256-verified, version-asserted) + exports `AILANG_BIN`, so the replay
   tests actually RUN in CI instead of silently `t.Skip`-ing — verified in the go-verify log. **Remaining:
   M6** CI Go gate finalize (`scripts/verify_go.sh`) + final green sweep + carry-forwards NB2
   (interpreter-member end-to-end re-verify) + NB5 (`verify_go.sh`). → [LANDED] + doc→implemented/ when
   M6 lands. **NOTE: M6 is the next (final) build step; then the item lands.**
