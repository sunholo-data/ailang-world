...
10. [**[IN-SPRINT] — `BG.A` LANDED 2026-08-06 (iter-58), PR #47 → squash `278f102`; evaluator
    ...
    **`AC2`, `AC3`, `AC4`, `AC5` are DISCHARGED**, mutations `M1`, `M2(a)`, `M2(b)`, `M4` run under
    ...
    `checkGoGroup` RETURNED. **CI CAVEAT, RAISED AND THEN SETTLED WITHIN THE ITERATION — dev is GREEN at HEAD (`e3808c0`, BOTH jobs, SHA-addressed, all 11 verify-gate steps `success` in the STEP LOG), and `e3808c0` is a DESCENDANT of the BG.A merge, so the same job passed on the same code:** at the merge SHA `go host build +
