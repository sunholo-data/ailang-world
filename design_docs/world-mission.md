...
|---|---|---|---|
| D-WORLD-5 | OPEN | A: import upstream `serveapi` at v0.33.1; B: remain dependency-free and build the public serving seam locally as designed. | Iteration 88 measured all prerequisites discharged; the chosen architecture's no-public-upstream-seam premise is now false. |
| D-WORLD-17 | OPEN | A: bind every seal to its minting validator while keeping tranche 1 library-only; B: defer authority-bearing `ResolvedGradeProven` to a tranche with a production composition root. | Iteration 87 quorum left this as the sole direction dispute after the MAC-seam revision. |
| D-WORLD-18 | OPEN | A: ship scoped daemon-read cancellation plus the five zero-cost fold-ins and ratchet; B: re-size to repo-wide bounded waits after settling approval and commit policy. | Iteration 86 quorum left the scope boundary as the sole direction dispute. |
...
| D-WORLD-DRIVER-1 | OPEN | A: World commits the 2026-08-15 fleet routing bundle into this repo (`tools/launchd/mission-control.sh`, `derive-planner-lane.sh`, `test_mission_routing.sh`, `testdata/`, and the `verify_go.sh` step that runs them), amending CLAUDE.md's frozen-core rule to permit it; B: the bundle stays fleet-owned and lands from `sunholo-data/ailang`, with World's checkout receiving it as a sync it never commits. | Iteration 89 measured `dev.ailang.mission-world.plist` executing this repo's **working-tree** `tools/launchd/mission-control.sh`, which is absent from `origin/dev`; `derive-planner-lane.sh` is untracked, live, and returns `opus fail-closed:env-pin`. CLAUDE.md forbids World from modifying `tools/launchd/*`, so the controller may not resolve this itself. |
<!-- decision-ledger:end -->
...
5. [**ALL THREE P6.B PREREQUISITES ARE NOW DISCHARGED — MEASURED 2026-08-15 (iter-88). THIS ROW IS NO LONGER BLOCKED, AND THE CHARTER'S "QUEUE IS FULLY BLOCKED" HEADLINE (iter-87) IS THEREBY CORRECTED.** What this row is blocked on now is a **design REVISION**, not a prerequisite — see the A/B below. Measurements, each with a firing control:
...
17. [**[PARKED `needs-human-review`] 2026-08-15 (iter-87) — REVISED TWICE, STILL NOT LANDED.** The
    ...
    **THE ASK, one word.** **A** = bind every seal to its minting validator (`Validator.Resolve(sealed)`,
