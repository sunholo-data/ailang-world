...
4c. [**[NEXT]** — unblocked 2026-07-29 (iter-35): item 4 `w-effect-broker-m3` is COMPLETE, so the
   "picks AFTER item 4 completes" condition is MET. **NEW-DOC → the designer rotation fires (next
   entry after `codex:gpt-5.6-sol`) and the design quorums at pick.** Its `host/store` kernel
   reopen is pre-ratified IN PRINCIPLE by `c26b27d`, but the design itself is NOT pre-approved.
   It inherits **CF-N-2** (`maxRecoveryPages` unjustified) and **CF-N-3** (`retryAllowed(false,
   true)` untested). It owns the **dispatch→record window**, which M3 left OPEN and stated.]
   **w-effect-journal** · clause-3 · effect-shaped intents in `host/store` (payload-codec change,
   no DDL): closes the dispatch→record window at the exact granularity of the original 2A
   objection. The kernel reopen is **pre-ratified in principle** (Mark 2026-07-28, attended);
   design doc + quorum at pick as usual · ~1–1.5d
