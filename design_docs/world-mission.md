...
- **`OD-<n>` IS A MISSION-GLOBAL NAMESPACE, AND THIS TABLE IS ITS REGISTRY** (process fix, iter-43;
  ...
  **Rules.** (a) Before allocating an `OD-<n>`, enumerate `### OD-` headings across **every** doc in
  ...
  | `9/OD-10` | 9 `w-verify-binary-lockfile` | CI job 1 installs `releases/latest` for `verify_ail.sh` legs 1-2 (today v0.33.0) while `:64`/`:118` pin v0.30.0 — PIN job 1 to the v0.30.0 tag, or ACCEPT the drift and keep only the warning? | **OPEN, blocking the remaining half of item 9** (iter-66). Coupled: the hard `verify_ail.sh` version assertion must NOT be added without this ruling — alone it reds CI on the next upstream release, headless. PIN = CI stops tracking upstream releases and World notices a compiler change only when it chooses to; ACCEPT = the primary `.ail` gate keeps re-validating against every future compiler, now at least VISIBLY (iter-66 landed the announce + DRIFT warning). Allocated as `10`, the next free ID per this table. |
...
9. [**[HALF-LANDED 2026-08-10 (iter-66) — the CHEAP, HEADLESS-SAFE half shipped; the HUMAN-GATED
