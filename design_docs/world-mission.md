...
| D-WORLD-26 | OPEN | **ONE WORD — the SESSION CREDENTIAL CARRIER for the MCP/A2A projection.** Clause 3 landed the session/capability model **in-process**: `broker.Session` is a Go value, and a repo-wide read finds **no** HTTP credential convention anywhere in `host/` — no `Authorization`, no `Bearer`, no World-named header. `protocol.SessionResolver.ResolveSession(ctx, *http.Request)` hands World the raw request, so World must name the carrier before it can write the resolver. The static `serve-api` API key remains forbidden as a session model (iter-24 measured it process-wide, so it cannot represent a session). **A** = the standard `Authorization: Bearer <session-credential>` — protocol-native for MCP/A2A clients, nothing bespoke to document; the designer's and controller's RECOMMENDED default. **B** = a World-named header `X-World-Session` — explicit that this is not an API key and cannot be confused with one by a client that already speaks OAuth. Raised as a blocking quorum objection by `gemini-3-1-pro` at round 3 (2026-08-25): an executable design must close its own forks. **This does NOT stall the queue** — it gates only `P6.B`, which is downstream of `P6.T`, `P6.D` and `P6.V`, all three of which are executable now. Answer `A` or `B`. | Measured first-party 2026-08-25 (iter-124), controls firing, and the count is stated exactly rather than rounded to the convenient answer: over `host/` + `cmd/`, `Bearer` **0**, `X-World-Session` **0**, `X-Session` **0**, and `Authorization` **1** — that single hit is `host/broker/approve.go:51`, the English word inside a comment (*"Authorization, debit, result persistence…"*), **not** an HTTP header. So no HTTP credential convention exists in the daemon, but the honest reading is 1-in-prose, not 0. Known-positive control in the same sweep: the landed in-process session surfaces (`type Session struct` **1**, `func NewSession(` **1**), so the zeros are measurements; fresh negative control `X-Zzz-Sentinel-124` **0**. *(Recorded as a friction: the controller's first draft of this row asserted a flat `Authorization` = 0 from the designer's premise row rather than re-running it — this repo's own iter-123 lesson, a control matching PROSE rather than code, reaching instance 2 in the charter's own evidence column.)* `protocol.SessionResolver.ResolveSession(ctx, *http.Request)` confirmed at `git show v0.33.2:serveapi/protocol/interfaces.go:10-11`. Raised as a BLOCKING quorum objection by `gemini-3-1-pro`, round 3, artifact `.ailang/state/mission-quorum/w-mcp-projection-2026-08-25T16-33-38Z.json` ($0.0489, present=true). Design doc `design_docs/planned/w-mcp-projection.md`, Open Decisions row 2. |
<!-- decision-ledger:end -->
