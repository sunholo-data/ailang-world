...
## STATUS 2026-09-03 (iteration 153) — **ROW 58 LANDED, AND ITS OWN HEADLINE WAS STILL WRONG AFTER ONE AMENDMENT: THE PROBE TIMEOUT IS NOT ABOUT SPEED, IT IS ABOUT *CONCURRENCY* — macOS SERIALIZES FIRST-EXEC ASSESSMENT GLOBALLY, SO A PER-PROBE BOUND IS CROSSED BY THE NINTH CALLER NO MATTER HOW FAST ANY SINGLE PROBE IS.** PR [#115](https://github.com/sunholo-data/ailang-world/pull/115) → squash [`12b8c87`](https://github.com/sunholo-data/ailang-world/commit/12b8c87), **Gate 3b GREEN on the MERGE commit** (SHA-addressed, `present=3 == expected=3`, `not_green=0`, `runs_total=1 event=push`; the PR head polled to the same standard first, every count asserted numeric before comparison; parent control `checks=3`). **GATE 0:** kill switch armed (namespaced path); `gh` = `sunholo-voight-kampff`; tree clean; billing **CLEAN**; overlap pidfile holds my own PID; pin present at `~/.pinned-ailang/ailang` (**v0.30.0**/`e37b370`). **0** `MarkEdmondson1234` directives on `#107` (17 comments) and **0** on the predecessor `#89` (44), BOTH watermarks read and the OLDER taken (they agree at `2026-09-03T10:57:25Z`), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH (row 69: still absent here; all gates stamped through the V1 heartbeat path for the same reason). Decision ledger **18 rows, `--check` valid, ONE OPEN** (`D-WORLD-31`, re-asked unchanged — **this iteration adds no new ask**); no ledger row changed since the watermark, so **no attended ruling and no self-resolution**. No rotation owed (`#107` created `2026-08-31T09:26:51Z`, 17 of 80 comments), no weekly sweep owed. **GATE 1:** `dev` == `origin/dev` at `d36359d`, tree clean, **0** open PRs, no died-mid-flight residue beyond the known-clean `.wt-iter150`; running skill **byte-identical to `origin/dev`** (`cmp` against the RESOLVED symlink target). CI **GREEN 3/3** on HEAD with `runs_total=1 event=push` and the parent at `checks=3` as a firing control. **GATE 2 — THE ROW'S AMENDED HEADLINE IS REFUTED TOO, AND THE AXIS IS THE WHOLE FINDING.** Row 58 as filed said *"rc=1 on pristine dev"*; iter-141 corrected that to *flaky* and named the macOS first-exec assessment plus a 513 MB Observatory cleanup as the likeliest drivers. **Both halves of that mechanism are now measured, and one of them is REFUTED.** Pinned v0.30.0 on Darwin/arm64: one warm exec at an established path **47–52 ms** (n=5); one FIRST exec of a freshly-copied path **1211–1294 ms** (n=5); **8 concurrent** first execs return at **1289, 2332, 3396, 4436, 5531, 6747, 7788, 8871 ms**; **12 concurrent** at **1255 … 13691 ms**, still linear at ~1.13 s each. **The cold series is LINEAR, NOT PARALLEL** — macOS assesses a never-before-executed binary under a global serializing lock — and `probeTimeout` is a **PER-PROBE** bound, so it is crossed at **N ≥ 9** regardless of single-probe speed. That is the entire flake: green package-by-package, flaky under `go test ./...` (which schedules up to 16 packages, ≥5 of which archive the pinned interpreter into fresh `t.TempDir()` paths), never red on Linux CI, and **immune to the axis iter-141 varied**. **THE LOAD-BEARING NEGATIVE CONTROL:** 8 concurrent **warm** execs of the same already-assessed path max out at **348 ms** — not serialized — so the linearity is a property of *first* exec and not of concurrency per se. **REFUTED, by the same measurements rather than by argument:** CPU contention moves a cold exec only to **1322/1373/1385 ms** under 16 spinners (iter-141's axis, and the decorative one), and the per-invocation Observatory cleanup over the now-**553 MB** database is paid by the ~50 ms warm arm. This is rule 3m aimed at a *bound* rather than at a stress control: **the stimulus scales with a dimension the bound cannot observe.** **GATE 3 — controller-authored direct fix; no designer, planner, executor or evaluator spawned; `metered=$0.00` of $5.** **DISPOSITION, and it is a deliberate NON-change:** `probeTimeout` is **NOT raised**. The cost scales with concurrency, so any constant merely moves the cliff, and rule 3m's remedy — derive the bound from the measured stimulus — is unavailable here because the probe cannot see how many siblings are queued ahead of it. The deliverable is therefore the row's own: `archive.EnvironmentFailure` (KindExecFailure **AND** `context.DeadlineExceeded` — both conjuncts load-bearing, since `KindExecFailure` alone covers a genuinely broken interpreter that must keep its own attribution) plus `archive.AttributeFailure`, wired at **8** `Archive()` call sites across `host/broker`, `host/capsule`, `host/replay` and `host/archive`. **Nothing is skipped and nothing suppressed** — a test that hits this still FAILS; it just names which of the two things went wrong and carries the one command that discriminates. **MUTATION DRILL, 6 mutants, each LANDED by sha256, `go vet` rc=0 read BEFORE any test result, restored byte-identical, pristine control green either side — all 6 RED with the correct assertion named:** M1 Kind check neutered → the wrong-Kind row; M2 deadline check neutered → both exec-defect rows and both label arms; M3 ReplayError requirement neutered → wrong-Kind + bare-deadline; M4 always-label → the defect-branch arms; M5 never-label → the instrument arms; **M6 `probeVersion` drops its `%w` → the SHIPPED-path arm ALONE**, which is what establishes that arm is not redundant with the error literals typed in the test file (rule 3k). **AND A HARNESS CORRECTION AGAINST MY OWN FIRST DRILL RUN:** M1 and M2's first forms did not compile (`go vet` rc=1, unused `re`), and the harness reported `test_rc=0` for them — because `local out=$(…)` **swallows the exit code**, rule 3's "exit codes through pipes lie" arriving through a shell builtin. Both were re-run as compiling mutants with the rc captured directly; the recorded verdicts are the second run's. **M7, not a mutant but the proof the floor reaches production call sites:** with `probeTimeout` forced to `1ns`, all three tests row 58 names — `host/replay` `TestFixtureEpisodeReplaysBitForBit`, `host/capsule` `TestF1PinnedInterpreterHashMismatchRefusedBeforeExec`, `host/broker` `TestEpisodeLiveReplayThreeArmsAndEvidence` — fail carrying the ENVIRONMENT label and the guidance; restored byte-identical, all three green on the pristine tree either side. **A SECOND DEFECT, FOUND IN THE FILE I WAS ALREADY EDITING AND FIXED HERE:** `host/archive`'s real-interpreter arm resolved the pin from a hardcoded `/tmp/ailang-v0300/ailang`. Iteration 151 moved the pin to `~/.pinned-ailang` because macOS wipes `/private/tmp` on boot — so that literal was dead **on every machine**, CI never had it and the rig no longer did, and `TestArchivePinnedInterpreter` had become a silent **SKIP with no red anywhere**. It now resolves `AILANG_BIN` exactly as `host/capsule` and `host/replay` do, which is what `ci.yml`'s own comment (*"Without AILANG_BIN, pinnedBinary(t) t.Skip()s and CI is false-green"*) had already asked for one job over. Measured **SKIP → PASS (0.95 s)** locally, and it now runs green in CI as well. **VERIFY GATE GREEN, BOTH LEGS:** `./scripts/verify_ail.sh` **rc=0** (11 required identities, 40 named tests, 9/9 world-package steps non-vacuous, `compiler pinned by exact bytes: AILANG v0.30.0`) and `go build ./... && go test ./... -count=1` **rc=0** (**19** packages `ok`, **0** FAIL), with the identical pair recorded at base before any edit. **A THIRD FINDING, FILED AS NEW ROW 76 RATHER THAN ABSORBED — AND IT IS THE ROW'S OWN ACCEPTANCE COMMAND:** `./scripts/verify_go.sh` is **rc=1 in 0.99 s** at pristine base with `FATAL: DRIVER DRIFT vs FLEET (D-WORLD-DRIVER-1)`, naming three World-committed driver blobs behind fleet HEAD `5e860afeb` (`derive-planner-lane.sh` and `test_mission_routing.sh` from 2026-08-18, the latter's fleet side landing **today**; `mission-control.sh` from 2026-09-02). **The red is correct — `CLAUDE.md` says it means "the fleet must commit", never "absorb it" — and the DEFECT IS THE ORDERING:** the drift arm fatals at `:224` while `go build` is at `:443` and `go test` at `:462`, so a fleet-owned condition suspends every Go assertion behind it, with **no opt-out** (`grep` for a skip variable = **0**; only `$CI` bypasses, which is why CI is green while the rig cannot run the gate at all). It stayed invisible because iterations 150–152 each recorded *"both legs green"* via `verify_ail.sh` + a hand-rolled `go build && go test` — the right assertions by a route that skips the drift arm — so the gate the ACs name has not been run here since the drift opened. **RECONCILE PERFORMED, NOT ESCALATED:** local `dev` was 1 behind after my own merge, `0` ahead, and the dirty set was byte-identical to the incoming blobs, so `git checkout origin/dev -- <7 paths>` then `git checkout -B dev origin/dev`; re-verified **7/7 files byte-identical to a pre-reconcile backup**, so no byte on disk changed. **NEXT:** rows **59**, **60**, **61**, **62**–**66**, **68**–**76**, then **39**. Row **50** stays parked on `D-WORLD-31`; row **57** remains tracking-only on upstream `#984`/`#1036`, its predicate to be RUN at pick time. **Decision ledger: 18 rows, ONE OPEN — `D-WORLD-31`, re-asked unchanged.**
