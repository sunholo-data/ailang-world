...
## STATUS 2026-09-06 (iteration 159) — **ROW 63 LANDS, AND THE HALF THE ROW LEFT UNFIRED IS THE WORSE HALF: `stepCol < 0` IS FIRED BY A PURE RE-STYLE WHOSE PARSED DOCUMENT IS *DEEP-EQUAL* TO THE PRISTINE `ci.yml` — A LEGITIMATE INPUT ACTIONS RUNS IDENTICALLY — WHILE `anchor < 0` NEEDS A WORKFLOW CARRYING NO `steps` KEY AT ALL.** **GATE 0:** kill switch armed (namespaced `mission-world.disabled`); `gh` = `sunholo-voight-kampff`; billing **CLEAN**; pin at `~/.pinned-ailang/ailang` (**v0.30.0**). **0** `MarkEdmondson1234` directives on `#107` since watermark `2026-09-05T21:42:00Z` (of 24 comments) AND **0** on the predecessor `#89` (44 comments), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH (row 69: it and the heartbeat are still absent here — the heartbeat's absence is why no gate stamp fired this iteration). The script's own allowlist guard was exercised as a positive control: `MISSION_DIRECTIVE_AUTHORS=""` is REFUSED. Ledger **18 rows, `--check` valid, ZERO OPEN** — this iteration asks Mark for nothing; no ledger row changed, so no attended ruling and no self-resolution. No rotation owed (`#107` created `2026-08-31T09:26:51Z` = 11:26 local, AFTER Monday 07:00 local; 24 of 80 comments), no weekly sweep owed. Inbox: 12 unread, **0** addressed to World (V1's `approvals` ask, `pkg:*` task notices, `aitana-platform` threads); no `[nightly-eval]` issues. **GATE 1:** running skill **byte-identical to `origin/dev`** (`cmp` against the RESOLVED symlink target, `readlink -f`). Local `dev` was **4 behind** `origin/dev` (`81ca5d7` vs `834d2d0`) with **0** ahead — the routine post-iteration state of this shared checkout, since every landing goes by worktree and PR — so all mission state was READ FROM ORIGIN and both the sprint and this record were written in worktrees branched on `origin/dev`; no reconcile attempted, standing authorisation for one being a human decision this charter does not carry. CI **GREEN 3/3** on `834d2d0`; **0** open PRs, **0** stale worktrees, main checkout clean but for the fleet's `tools/launchd/mission-control.sh.tmp.astra`. **GATE 2 — THE PICK IS ROW 63**, the queue head, `~0.1d`, gated on nothing; not landed (no commit, no merged PR, no doc). Baseline both legs rc=0 with `AILANG_BIN` set (**19** `ok`, **0** FAIL); `verify_go.sh` **rc=1 at base** on the FLEET-OWNED drift arm, re-measured rather than transcribed and now naming fleet HEAD `19d6b03c` (it was `f516881a` one iteration ago — the fleet has moved again), so the two-leg substitute stands (row 76). **GATE 3 — CONTROLLER-AUTHORED DIRECT FIX; no designer, planner, executor or evaluator spawned (~0.1d row, iter-153/156/157/158 precedent). `metered=$0.00` of $5. STATED PLAINLY: NO INDEPENDENT JUDGE RAN — generator == judge for this item**, and the compensating discipline is that every claim below is a landed-and-restored mutation rather than an assertion. **THE ROW'S CLAIM, REPRODUCED FIRST-PARTY BEFORE ANY CODE (rule 3f), AND IT CAME BACK *BIGGER* ON THE HALF THE ROW GUESSED WAS SAFE.** The row-52 locator's five loud refusals include two DERIVATION refusals with no committed killer arm. Measured at base: neutering each to a silent fallback (`anchor = 0`; `stepCol = expectedStepCol`) leaves `go test ./host/verifygate/ -count=1` **rc=0** — both branches deletable with the whole suite green — while the discriminating positive control, neutering the `continue-on-error` value check, reds **5** arms. So the harness could see a red; these two simply had none. By this mission's own standing rule that is a guard, not a gate. **BOTH ARE REACHABLE, FIRED ON THE REAL `ci.yml`, AND THEY ARE NOT EQUALLY BENIGN.** `anchor < 0`: renaming both `steps:` keys above the identifying line → **rc=1** naming the anchor; the mutant parses as YAML but leaves `jobs.go-verify` with **no `steps` key**, so Actions would reject it — the row's own caveat, confirmed. `stepCol < 0`, which the row recorded as *unfired* and *plausibly unreachable*: rewriting each `      - key: v` as a bare `      -` with the mapping on the following lines → **rc=1** naming the column, and the mutant is a pure RE-STYLE — parsed with a real YAML loader it is **deep-equal to the pristine document** (`a == b`), so Actions runs it identically while the derivation cannot read it. That inverts the row's guess: the branch it left unfired is the one reachable from a LEGITIMATE input. **THE FIX:** the derivation moves out of the test body into `stepBlockAnchors(lines, identifyingLine) (anchor, stepCol int, instrumentErr string)`, both refusal messages byte-identical, so the two branches can carry committed arms — the same shape `continueOnErrorRefusalsIn`/`stepScanFixture` already established one screen below. Three fixture arms on a TWO-job workflow, so the upward scan must pass an earlier job's `steps:` at the same indent and the downward scan must start at the anchor it chose. The bare-dash fixture deliberately keeps `- name:` entries ABOVE the anchor, so its refusal proves the scan is anchored rather than globally blind. **SENSITIVITY DRILL, 6 NEUTERINGS, EVERY CONJUNCT KILLED AND THREE SOLE:** N1 anchor refusal → **sole** (`no steps: key above the identifying line is refused`); N2 step-column refusal → **sole** (`a bare block-sequence dash is refused rather than guessed at`); N3 outermost-anchor selection → the pristine arm + the bare-dash arm; N4 anchoring of the downward scan → **sole**; N5 dash-plus-space requirement → **sole**. `go vet` rc=0 read BEFORE every test verdict, each mutant landed by exact-count substitution and restored byte-identical against a captured sha256, pristine control green either side. **MY FIRST N3 WAS NOT A VERDICT AND SAID SO:** the neutering left `anchorCol` unused, `go vet` came back **rc=1**, and the empty failing-arm list was read as a build failure rather than a green — re-run in a compiling form (`<` → `<=`), which reds the two arms above. **DECLARED RESIDUAL, MEASURED RATHER THAN ARGUED, AND IT IS NOT A FAIL-OPEN:** dropping the refusal at the production call site (`_, stepCol, _ := …`) is green on a pristine `ci.yml`, the row-61 shape one file over. Landing BOTH mutations together — the discard *and* the bare-dash `ci.yml` — still **rc=1**, because a refusal returns `stepCol = -1` and the `stepCol != expectedStepCol` check consumes it. The surviving message is `derived step column -1; update expectedStepCol after an intentional ci.yml re-indent`, which blames an indent change for a shape the scan cannot read — so the residual is a MIS-ATTRIBUTION, and returning `-1` on every refusal is what makes the backstop hold. Both facts are now stated in the helper's doc comment, matching the MUT-H precedent of declaring a disposition with the measurement that supports it. **VERIFY GATE GREEN, BOTH LEGS, outside any drill:** `verify_ail.sh` rc=0 (11 identities, 40 named tests, 9/9 world-package steps), `go build ./...` rc=0, `go vet ./...` rc=0, `gofmt -l` clean, `go test ./... -count=1` rc=0 (**19** `ok`, **0** FAIL). **LANDED:** PR [#121](https://github.com/sunholo-data/ailang-world/pull/121) -> squash [`2115172`](https://github.com/sunholo-data/ailang-world/commit/2115172); Gate 3b GREEN on the MERGE commit, `present=3 == expected=3` with expected ENUMERATED from `ci.yml`'s own job list (`ailang-verify`, `go-verify`, `launchd-drivers`; `ci.yml` is the only workflow, so the enumeration is complete), `not_green=0`, `runs_total=1 event=push`, parent control `834d2d0` at `checks=3`, `mergeable` read FIRST (`MERGEABLE/UNSTABLE` -> `MERGEABLE/CLEAN`). **MY OWN GATE-3b POLL FAILED FIRST AND PRINTED `INSTRUMENT FAILURE` RATHER THAN A VERDICT** — an inline `jq` expression was mangled by the shell and returned empty; the numeric floor caught it, the filter moved to a file, and the first read on the merge commit (`present=0 expected=3`) was correctly kept polling rather than greened. **CONTAINMENT:** the main checkout's only untracked file remains `tools/launchd/mission-control.sh.tmp.astra` — a fleet artifact, frozen core, left alone and reported for the fifth iteration running. **NO NEW ROWS FILED; ONE PROCESS FIX.** The Gate-3b poll breakage above is the THIRD instance of the charter's own `zsh`-idiom guardrail (iter-37 filed two; iter-157's `set -- $raw` was a fourth of the same family), so the guardrail gains idiom **(c)**: a multi-line `jq` filter passed inline as a `--jq` argument is mangled in transit — write it to a file and use `jq -r -f`. Routed to the process-fix lane rather than a queue row because the guardrail it extends already exists and World owns its own charter. **NEXT:** row **64**, then **65**, **66**, **68**–**78**, **81**, **82**, **83**, then **39**. **Decision ledger: 18 rows, ZERO OPEN.**

## STATUS 2026-09-06 (iteration 158) — **ROW 62 LANDS, AND FOR THE SECOND ITERATION RUNNING THE ROW'S OWN PROPOSED REMEDY IS MEASURED FAIL-OPEN: A STEP'S *FIRST* KEY RIDES THE BLOCK-SEQUENCE DASH, TWO COLUMNS LEFT OF EVERY SIBLING KEY, SO "MATCH THE KEY AT THE STEP'S OWN KEY INDENTATION" LETS `- continue-on-error: true` THROUGH — ROW 83's FIRST APPLICATION, AND IT PAID.** **GATE 0:** kill switch armed (namespaced `mission-world.disabled`); `gh` = `sunholo-voight-kampff`; billing **CLEAN**; pin at `~/.pinned-ailang/ailang` (**v0.30.0**). **0** `MarkEdmondson1234` directives on `#107` since watermark `2026-09-05T19:04:00Z` (of 23 comments) AND **0** on the predecessor `#89` (44 comments), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH (row 69: it and the heartbeat are still absent here — the heartbeat's absence is why no gate stamp fired this iteration). The script's own allowlist guard was exercised as a positive control: `MISSION_DIRECTIVE_AUTHORS=""` is REFUSED. Ledger **18 rows, `--check` valid, ZERO OPEN** — this iteration asks Mark for nothing; no ledger row changed, so no attended ruling and no self-resolution. No rotation owed (`#107` created `2026-08-31T09:26:51Z` = 11:26 local, AFTER Monday 07:00 local; 23 of 80 comments), no weekly sweep owed. Inbox: 14 unread, **0** addressed to World (V1's `approvals` ask, `pkg:*` task notices, `aitana-platform` threads, and World's own `world-probe-i139` arms); no `[nightly-eval]` issues. **GATE 1:** running skill **byte-identical to `origin/dev`** (`cmp` against the RESOLVED symlink target, `readlink -f` — inode `67997727`). Local `dev` was **2 behind** `origin/dev` at Gate 1 (`81ca5d7` vs `ae9a615`) with **0** ahead — the routine post-iteration state of this shared checkout, since every landing goes by worktree and PR — so all mission state was READ FROM ORIGIN and both the sprint and the record were written in worktrees branched on `origin/dev`; no reconcile was attempted, because standing authorisation for one is a human decision this charter does not carry. CI **GREEN 3/3** on `ae9a615` with the parent at `checks=3` as a firing control; **0** open PRs, **0** stale worktrees, main checkout clean but for the fleet's `tools/launchd/mission-control.sh.tmp.astra`. **GATE 2 — THE PICK IS ROW 62**, the queue head, `~0.1d`, gated on nothing; not landed (no commit, no merged PR, no doc). Baseline on a pristine tree rc=0 with `AILANG_BIN` set; `verify_go.sh` **rc=1 at base** on the FLEET-OWNED drift arm, re-measured rather than transcribed and now naming fleet HEAD `f516881a` (it was `59571d77` one iteration ago — the fleet has moved again), so the two-leg substitute stands (row 76). **GATE 3 — CONTROLLER-AUTHORED DIRECT FIX; no designer, planner, executor or evaluator spawned (~0.1d row, iter-153/156/157 precedent). `metered=$0.00` of $5. STATED PLAINLY: NO INDEPENDENT JUDGE RAN — generator == judge for this item**, and the compensating discipline is that every claim below is a landed-and-restored mutation rather than an assertion. **THE ROW'S CLAIM, REPRODUCED FIRST-PARTY BEFORE ANY CODE (rule 3f), AND IT WAS EXACT — LINE NUMBERS INCLUDED.** On the real `ci.yml`, mutants landed by sha256 and restored byte-identical against a captured backup, pristine control green either side: (a) `continue-on-error: false` on the guarded step → **rc=1 at `ci.yml:175`**; (b) a benign `echo "note: never add continue-on-error to this step"` inside that step's own `run:` scalar → **rc=1 at `ci.yml:177`**, a red over script TEXT with no flag present at all; (c) the discriminating control — the identical `echo` in an UNRELATED step — **rc=0**, which is what makes this a scan defect and not a repo-wide text ban. **AND THEN ROW 83's DISCIPLINE FIRED ON ITS FIRST OUTING.** Row 62 proposed *"match the KEY at the step's own key indentation … and treat a `false` value as compliant"*. Taken literally that is **fail-OPEN**: a step's FIRST key rides the block-sequence dash at `stepCol`, two columns left of every sibling key at `stepCol+2`, so `- continue-on-error: true` — valid YAML, and the shape an author writing the flag first would reach for — evades an indentation rule written only for siblings. Drill arm **N1** neuters exactly that case and its **sole** killer is `true_riding_the_block-sequence_dash_is_refused`. Two consecutive iterations, two rows, two proposed remedies that were fail-open as written. **THE FIX:** `continueOnErrorRefusalsIn` reads the step's own block-mapping key at BOTH positions and then reads its VALUE. Only a literal `false` (any case, quoted or not, with or without a trailing comment) is an accepted opt-out; `true`, an empty value and a `${{ }}` expression are refusals — the expression because its run-time value is not decidable here. Row 62 is about false POSITIVES, and widening it into a fail-open would trade this instrument for its opposite. A step written as a YAML **flow mapping** is an instrument FAILURE, not a green. The neighbouring `run.sh` check one screen below already drew exactly this line — rejecting only EXECUTABLE uses of `go env GOOS` while letting a comment name the channel it warns about — so this is *guard the helper, miss the call site* inside a single test function. **PRODUCTION-PATH DRILL ON THE REAL `ci.yml`, AFTER THE FIX, six arms:** A (`: false`) green · B (benign echo) green · D (`: true`) **RED** · E (`true` riding the dash) **RED** · F (`${{ }}`) **RED** · C (unrelated step) green. `go vet` rc=0 read before every verdict; each mutant landed by sha256 and restored byte-identical. **SENSITIVITY DRILL, 7 NEUTERINGS, EVERY CONJUNCT KILLED AND FOUR SOLE:** N1 dash position → **sole**; N2 value check → the four refusal arms; N3 false-is-compliant → the four compliant arms; N4 flow-mapping guard → **sole**; N5 key-boundary colon → **sole**; N6 inline-comment strip → **sole**; N7 revert to the bare substring → 8 arms. `go vet` rc=0 read before every test result, each restored byte-identical to a captured sha256, pristine control green either side of all seven. **VERIFY GATE GREEN, BOTH LEGS, outside any sandbox:** `verify_ail.sh` rc=0 (11 identities, 40 named tests, 9/9 world-package steps, `AILANG v0.30.0` pinned by exact bytes), `go build ./...` rc=0, `go vet ./...` rc=0, `go test ./... -count=1` rc=0 (**19** `ok`, **0** FAIL). **LANDED:** PR [#119](https://github.com/sunholo-data/ailang-world/pull/119) -> squash [`dcf534f`](https://github.com/sunholo-data/ailang-world/commit/dcf534f); Gate 3b GREEN on the MERGE commit, `present=3 == expected=3` with expected ENUMERATED from `ci.yml`'s own job list (`ailang-verify`, `go-verify`, `launchd-drivers`; `ci.yml` is the only workflow, so the enumeration is complete), `not_green=0`, `runs_total=1 event=push`, parent control `ae9a615` at `checks=3`, `mergeable` read FIRST (`MERGEABLE/UNSTABLE` -> `MERGEABLE/CLEAN`, never banked on the first non-CONFLICTING reading), and `#107` asserted still OPEN after the merge. Every poll count asserted numeric before comparison. **CONTAINMENT:** the main checkout's only untracked file remains `tools/launchd/mission-control.sh.tmp.astra` — a fleet artifact, frozen core, left alone and reported for the fourth iteration running. **NO NEW ROWS FILED.** **NEXT:** row **63**, then **64**–**66**, **68**–**78**, **81**, **82**, **83**, then **39**. **Decision ledger: 18 rows, ZERO OPEN.**

## STATUS 2026-09-05 (iteration 157) — **ROW 61 LANDS, AND THE ROW'S OWN PREFERRED FIX IS MEASURED FAIL-OPEN *EVEN UNMUTATED*: DROPPING THE VARIABLE AND READING `case "$?"` IS DEFEATED BY THE `set -e` SITTING BETWEEN THE COMPARISON AND THE READ, WHICH IS ITSELF A SUCCESSFUL COMMAND.** **GATE 0:** kill switch armed (namespaced `mission-world.disabled`); `gh` = `sunholo-voight-kampff`; billing **CLEAN**; pin at `~/.pinned-ailang/ailang` (**v0.30.0**). **0** `MarkEdmondson1234` directives on `#107` since watermark `2026-09-05T19:04:00Z` (of 22 comments) AND **0** on the predecessor `#89` (44 comments), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH (row 69: it and the heartbeat are still absent here — the heartbeat's absence is why no gate stamp fired this iteration). The script's own allowlist guard was exercised as a positive control: `MISSION_DIRECTIVE_AUTHORS=""` is REFUSED. Ledger **18 rows, `--check` valid, ZERO OPEN** — this iteration asks Mark for nothing; no ledger row changed, so no attended ruling and no self-resolution. No rotation owed (`#107` created `2026-08-31T09:26:51Z` = 11:26 local, AFTER Monday 07:00 local; 22 of 80 comments), no weekly sweep owed. Inbox: 14 unread, **0** addressed to World (V1's `approvals` ask, `pkg:*` task notices, `aitana-platform` threads, and World's own `world-probe-i139` arms); no `[nightly-eval]` issues. **GATE 1:** running skill **byte-identical to `origin/dev`** (`cmp` against the RESOLVED symlink target, `readlink -f` — inode `67997727`, the same file the invocation named). **A FLEET SESSION IS LIVE IN THIS SHARED CHECKOUT AND ADVANCED `origin/dev` MID-GATE**: `808e78f` → `81ca5d7c` (`fix(launchd): guarantee /usr/sbin on PATH …`) at `21:55:01`, two minutes into Gate 0, so every subsequent read was re-taken against the new head and all sprint work went to a worktree (rule 4). CI **GREEN 3/3** on `81ca5d7c` (`ailang-code verify gate`, `go host build + test gate`, `launchd drivers (bash 3.2)`) with `runs_total=1 event=push` asserted and the parent `808e78f` at `checks=3` as a firing control; **0** open PRs, **0** stale worktrees. **MY OWN GATE-1 POLL FAILED AND SAID SO:** `set -- $raw` does not word-split in zsh, so the three counts arrived as one string and the numeric floor printed `INSTRUMENT FAILURE — not a verdict` twenty times rather than a green — the floor working exactly as the shared skill prescribes, on the rig fact this mission has already recorded. Killed the poller (rule d-bis) and read the check set directly. **GATE 2 — THE PICK IS ROW 61**, the queue head, `~0.2d`, gated on nothing; not landed (no commit, no merged PR, no doc). Baseline both legs rc=0 (**19** packages `ok`, **0** FAIL) with `AILANG_BIN` set; `verify_go.sh` **rc=1 at base** on the FLEET-OWNED drift arm — now naming three drifted driver files against fleet HEAD `59571d77` — so the two-leg substitute stands (row 76). **GATE 3 — CONTROLLER-AUTHORED DIRECT FIX; no designer, planner, executor or evaluator spawned (~0.2d row, iter-153/156 precedent). `metered=$0.00` of $5. STATED PLAINLY: NO INDEPENDENT JUDGE RAN — generator == judge for this item**, and the compensating discipline is that every claim below is a landed-and-restored mutation rather than an assertion. **THE ROW'S CLAIM, REPRODUCED FIRST-PARTY BEFORE ANY CODE (rule 3f), AND IT UNDERSTATED ITSELF.** The P1 block extracted standalone: pristine arms rc=0 / rc=1 / rc=1; with one inserted `floor_rc=0` **all three go rc=0**, so the mutant opens not just the below-floor branch the row named but the **malformed-token instrument floor**, printing `✓ toolchain floor gate: devel >= root module floor go1.26.6` — an assertion that is not merely false but meaningless. Landed in the real file: `grep -c '^floor_rc=0$'` **0 → 1**, `bash -n` rc=0, `go vet` rc=0, `TestRaceControlFloorStaysBelowRootToolchain` **`ok`**; restored byte-identical against a captured sha256 (row 82: `git checkout --` restores to HEAD, never to an uncommitted baseline). **THE ROW OFFERED TWO OPTIONS AND ONE OF THEM IS BROKEN — MEASURED, NOT ARGUED.** Option (a) "branch directly on the call" as literally written (`go_version_ge …` then `case "$?"`) is **fail-OPEN with no mutation at all**: below-floor input, rc=**0**, because `set -e` between the call and the read is a successful command that resets `$?`. The `if`/`else` variant is the one that works, and it works for a reason worth stating: **every arm of the attributing `case` exits**, so a break can only change WHICH refusal fires. Measured on the shipped shape — pristine rc=0/1/1; one inserted line in the `else` branch **rc=1** with attribution shifted from `is BELOW the root module floor` to `cannot order toolchain tokens`; one inserted line in the `then` branch **rc=1** unchanged. **TWO NEW CONJUNCTS, EACH WITH A PROVEN SOLE KILLER (the iter-156 lesson: a red arm asserting only `err != nil` cannot see re-attribution, so `assertRed` pins the message).** N1, neutering the `if`-condition binding: **only** `RED/comparator verdict not consumed directly by the branch` fails, `go vet` rc=0 read first, restored byte-identical. N2, neutering the `<var>=$?` check: **only** `RED/comparator verdict re-laundered through a reassignable variable` fails. Pristine control green either side of both. **PRODUCTION-PATH REGRESSION PROOF:** the row-61 mutant class landed in the real `scripts/verify_go.sh` — `bash -n` rc=0, `go vet` rc=0, **GREEN at base** — is now **RED**, naming `P1 comparator call \`if go_version_ge "$X" "$Y"; then\` count=0 … its verdict is no longer consumed directly by the branch (queue row 61)`; restored byte-identical, tree clean. **ROW 48's DECLARED RESIDUAL WIDENED** from "rewrite" to any dataflow break including an insertion, and it now says what remains rather than trailing off: a comparator-BODY rewrite, covered by the `exit 0`/`exit 1`/`exit 2` contract needle and V15's 13-case battery, not by the consumption-shape binding. **VERIFY GATE GREEN, BOTH LEGS:** `verify_ail.sh` rc=0, `go build ./...` rc=0, `go vet ./...` rc=0, `go test ./... -count=1` rc=0 (**19** `ok`, **0** FAIL), `bash -n scripts/verify_go.sh` rc=0. **CONTAINMENT:** the main checkout's only untracked file remains `tools/launchd/mission-control.sh.tmp.astra` — a fleet artifact of the 2026-09-05 attended rotation amendment, frozen core, left alone and reported for the third iteration running. **LANDED:** PR [#118](https://github.com/sunholo-data/ailang-world/pull/118) -> squash [`8b600fd`](https://github.com/sunholo-data/ailang-world/commit/8b600fd); Gate 3b GREEN on the MERGE commit, `present=3 == expected=3` with expected ENUMERATED from `ci.yml`'s own job list (`ailang-verify`, `go-verify`, `launchd-drivers`; `ci.yml` is the only workflow, so the enumeration is complete), `not_green=0`, `runs_total=1 event=push`, parent control `81ca5d7c` at `checks=3`, `mergeable` read FIRST (`MERGEABLE/UNSTABLE` -> `MERGEABLE/CLEAN`, never banked on the first non-CONFLICTING reading). **ONE NEW ROW FILED, A FIRST INSTANCE, SO IT MAY NOT SPEND THE GATE-5 SKILL EDIT: 83** — the loop live-repros a queue row's DEFECT claim (rule 3f) and takes its PROPOSED REMEDY on trust; row 61 offered two fixes and the one it called "the cheapest honest fix" is fail-open with no mutation at all. **NEXT:** row **62**, then **63**–**66**, **68**–**78**, **81**, **82**, **83**, then **39**. **Decision ledger: 18 rows, ZERO OPEN.**
