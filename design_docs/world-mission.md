...
4c. [**[IN-SPRINT — MJ.A LANDED 2026-07-29 (iter-37)]**, PR #26 → squash `82d9128`, dev CI green
   ...
   **AC5's third clause is STRUCK as a doc defect, not implemented**: measured first-party, landed
   ...
   carries the mutation table). **Remaining: MJ.B** (broker rewiring + crash-window proof +
   effect-granularity recovery; **OQ-5 binding — `LogicalTime` must come from a caller-supplied
   logical clock; the executor STOPS rather than inventing one or passing zero**) **and MJ.C**
   (CF-N-2/CF-N-3 discharge + bench + close-out, folding CF-MJA-1/CF-MJA-2).
...
4e. [**[QUEUED 2026-07-29 (iter-37)]** — found by chasing a line the MJ.A executor mentioned in
   ...
   **not cgo** (`CGO_ENABLED=1` is this rig's default and passes without `-race`). Exactly one
   ...
   progressing and is `signal: killed` at ~142 s. Root enabler: **`grep -rn race
