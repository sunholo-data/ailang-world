...
7. [PARKED until 4 AND 6b land] **w-approval-inbox** · clause-5 · the approval inbox + provenance
   ...
   HUMAN-SURFACE.md** · ~2d
8. [**[NEXT] — UNPARKED 2026-08-05 (iter-50). Its only gate has cleared and a human just invested in it.** The park condition was "until 4 lands"; item 4 `w-effect-broker-m3` completed at iter-35 (all five milestones, doc verified present at `design_docs/implemented/w-effect-broker-m3.md` — checked, not transcribed). Mark then re-scoped this row attended on 2026-08-04 (`de80792`) with the naming + flow decisions below, which is the strongest queue signal available. **Routes to design-doc-creator** (no doc exists — `grep -ril 'w-self-mod-vertical' design_docs/` finds only charter/log rows), and its VERIFY-FIRST clause is binding at pick: live-repro the `ailang publish` auth + vendor-registration mechanics against the pinned binary BEFORE any milestone is written, exactly as the naming note requires. Note the standing tension the designer must resolve rather than absorb: the publish is an outward effect on a repo whose clause 2 is zero cloud deps in the core, so the brokered/receipted framing is load-bearing, not decorative.] ~~[PARKED until 4 lands]~~ **w-self-mod-vertical** · clause-7 · one World extension shipped through **[NAMING + FLOW, Mark 2026-08-04: World extensions publish to the PUBLIC AILANG
