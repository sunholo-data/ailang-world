...
| D-WORLD-21 | OPEN | **At the validator's object-read seam, which reviewer wins?** Round 8 left `gemini-3-1-pro` PASS and `gpt5-6-sol` REJECT, and their fixes are incompatible at the same seam. `gpt5-6-sol` is right that a `context.WithTimeout` bounds the OPEN call but not the subsequent `Read` loop — `io.ReadCloser.Read` takes no context and is under no obligation to unblock on `ctx.Done()` — and right that the doc's own mutation M22 is VACUOUS BY CONSTRUCTION, because its prescribed fake is wired to observe the context, so the mutant dies for a property the real `host/store` reader has never been shown to have. **A** = arm 1, its own words: replace the seam with `ReadObject(ctx, ref, maxBytes) (ObjectMeta, []byte, error)`, the store enforcing `maxBytes` BEFORE materialization and performing the complete read under the supplied context — cancellation becomes enforceable, and the STREAMING reader is retired, i.e. the thing objection 6b demanded, `D-WORLD-19` arm A ratified, and `gemini-3-1-pro` has just PASSED. **B** = arm 2, its own words: keep the reader and define a context-aware reader contract whose reads are guaranteed to terminate on `ctx.Done()`, documenting the concrete store mechanism that enforces it — keeps gemini's pass intact and puts the burden on naming a real mechanism in `modernc.org/sqlite`. Both arms carry, unconditionally, `gpt5-6-sol`'s real-store integration test (a blocked read under `context.Background()` relying only on `ObjectReadTimeout`, mutating the ACTUAL cancellation mechanism rather than `WithTimeout`) — that is the arm that would have caught M22's fixture vacuity, and it is owed under either answer. This is Mark's call and not the controller's because the two reviewers want different things at one seam: streaming to avoid materialization versus a complete-read-under-context to make cancellation enforceable. Fifth consecutive foreclosure of the narrow-refinement carve-out on this document, and the first where the dispute is between the two REVIEWERS rather than between a reviewer and the design. | Round 8 artifact `.ailang/state/mission-quorum/w-validated-proven-evidence-boundary-2026-08-19T07-17-50Z.json`: `absent_reviewers: []`, both slots present, `metered=$0.244360`; `gemini-3-1-pro` pass ($0.0718), `gpt5-6-sol` reject ($0.1725). Doc §10.9 records the three-round escalation of the same class (parameter → deadline-on-open → deadline-not-reaching-the-read). Its non-blocking half — stderr needs its own capped reader — was applied VERBATIM in the same commit rather than deferred, because §10.6 records a non-blocking note left unforwarded returning as BLOCKING one round later. |
<!-- decision-ledger:end -->
