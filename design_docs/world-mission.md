...
2. [IN-SPRINT — milestones 1–4 of 6 LANDED 2026-07-24 (iter-8/9/10/14), CI green]
   ...
   **M4 (interpreter artifact archive + epoch-1 registry bootstrap, Decisions 5+6 —
   ...
   test + a sidecar-absent idempotence-recovery edge). **Remaining: M5** replay+replay-doubling
   (bit-for-bit episode reconstruction; the `writtenBy` interpreter-stamp lands here) → M6 CI Go gate
   finalize (`verify_go.sh`). → [LANDED] + doc→implemented/ when M6 lands. **NOTE:
   `w-m1-ailang-hardening` LANDED iter-13 (`d0009c8`); M5 is the next build step.**
