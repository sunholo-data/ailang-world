...
## STATUS 2026-09-03 (iteration 152) — **ROW 57's CAUSAL CLAIM IS REFUTED BY MEASUREMENT, AND THE ISSUE IT REFUTES IS ONE THIS MISSION ITSELF FILED: `--type` IS NOT IGNORED, IT IS MISFILED — AND FIXING IT WOULD LEAVE THE FALSE GREEN BYTE-IDENTICAL.** No PR to this repo's code: the fix is upstream in `sunholo-data/ailang`, so per the frozen-core rule the deliverable is MEASUREMENT + UPSTREAM ISSUES + a cross-mission note, never a local workaround. **GATE 0:** kill switch armed (namespaced path); `gh` = `sunholo-voight-kampff`; tree clean; billing **CLEAN**; pinned `ailang` present at `~/.pinned-ailang/ailang` (**v0.30.0**/`e37b370`) with PATH's `v0.34.0-...-dirty` correctly NOT used for gates; **0** `MarkEdmondson1234` directives on `#107` (16 comments) and **0** on the predecessor `#89` (44), BOTH watermarks read and the OLDER taken (they agree at `2026-09-02T19:27:22Z`), via the V1 checkout's `mission_directives.sh` by ABSOLUTE PATH (row 69: still absent here). Decision ledger **18 rows, `--check` valid, ONE OPEN** (`D-WORLD-31`, re-asked unchanged — **this iteration adds no new ask**); no ledger row changed since the watermark, so **no attended ruling and no self-resolution**. No rotation owed (`#107` created `2026-08-31T09:26:51Z`, AFTER Monday 07:00 local; 16 of 80 comments), no weekly sweep owed. **GATE 1:** `dev` == `origin/dev` at `6874a98`, tree clean, **0** open PRs; running skill **byte-identical to `origin/dev`** (`cmp` against the RESOLVED symlink target, inode-confirmed the same file). CI **GREEN 3/3** on HEAD (`go host build + test gate`, `launchd drivers (bash 3.2)`, `ailang-code verify gate`), run existence asserted (`runs_total=1`, `event=push`) with the parent commit as a firing control at `checks=3`. Died-mid-flight traces: one stale worktree `.wt-iter150`, **clean** (`porcelain=0`), whose work landed as `a7b58dd` — leftover, not orphaned work. **GATE 2 — GHOST DISCIPLINE ON AN ISSUE I FILED MYSELF, AND IT DID NOT SURVIVE.** Row 57 claims *“the typed sub-query finds zero `approval_request` rows”*. **There is no typed sub-query and one is not expressible:** `printApprovalsInboxPending` queries `InboxListOptions{Inbox:"approvals",UnreadOnly:true}` with **no type filter** — proven EMPIRICALLY, because it **listed a row whose `message_type` is `notification`** (my own spine post `inbox_1788448377647_d836d0e8`) — and `messaging.InboxListOptions` declares **10** filter fields, none of them a message-type filter. The green line instead comes from `coordinator_list.go:137` reading `FROM approval_requests WHERE status='pending'` (`store_sqlite_approvals.go:88`), a different table and on a `storage=gcp` node a different backend; that file references `inbox_messages` **0** times (control: `approval_requests` **16**). **THE LOAD-BEARING NEGATIVE CONTROL (rule 3d — the result came back in the direction I predicted):** `ailang coordinator pending` at **0** unread inbox rows and at **1** printed a **BYTE-IDENTICAL** `✓ No pending approval requests`; the verdict is provably invariant to the inbox, the only delta being the spine block above it. And **nothing repo-wide compares `message_type` to `approval_request`** — the sole hit is a writer (`daemon_tasks_exec_run.go:612`). **DEFECT 1, RESTATED CORRECTLY:** `messages_send.go:42` binds `--type` to `Category`, `:132` **hardcodes** `MessageType: InboxTypeNotification`. Across all **18** `approvals` rows the split is total by author class — **12** mission-loop rows `notification`/`approval_request`, **6** `coordinator` rows `approval_request`/empty — and my own send reproduced it live. **Its real blast radius, newly measured:** `messages_activity.go:59` aggregates `ByType[MessageType]`, and `ailang messages activity` reports `29 notification, 1 completion` with **ZERO `approval_request`** while listing `1 approvals` inbox; `agent_registry.go:56` routes inbound templates by `TemplateByMessageType`; `backstop_sweep.go:157,202` and `message_adapter.go:51` key `Kind` off it. **THE ROW'S ONE EXPLICITLY-UNMEASURED CLAIM IS NOW SETTLED, IN THE SAFE DIRECTION:** `internal/daemon/messageNotification` references `MessageType` **0** times (controls: `ToInbox` **7** same file, **106** repo-wide, fresh absent literal **0**), switches on `ToInbox`, `humanTriageInbox` accepts `approvals`, and it is LIVE from `daemon.go:204` by CALLER search — so mission-authored approvals **DO** reach Discord as “🔔 Approval needed” despite the wrong type. **The ask is not lost**, which lowers the whole class from *asks are invisible* to *asks are mis-typed*. **GATE 3 — no sub-agent spawned; controller-authored measurement, `metered=$0.00` of $5.** Upstream: the correction posted to [ailang#984](https://github.com/sunholo-data/ailang/issues/984) (comment count asserted **0→1**; it had **0** comments, so nothing had been built on the bad premise) and the real cause filed as [ailang#1036](https://github.com/sunholo-data/ailang/issues/1036) — *an unqualified verdict over a two-source union*; cross-mission note to `mission-control` (`inbox_1788448675286_e0797635`, body asserted, no flag leak). Also found in passing and carried in `#1036`: `coordinator_pending_spine.go:9`'s doc comment claims it *“lists unread approval_request messages”*, a filter the code does not have. **VERIFY GATE GREEN, BOTH LEGS, at base:** `./scripts/verify_ail.sh` **rc=0** (11 required identities, 40 named tests, 9/9 world-package steps non-vacuous, `compiler pinned by exact bytes: AILANG v0.30.0`) and `go build ./... && go test ./...` **rc=0** (**19** packages `ok`, **0** FAIL). **A SECOND, UNRELATED FINDING, REMEDIATED AND HALF-FILED:** the shared skill's ATTENDED-LEDGER contract cites `make check-no-personal-email` as enforcement. **No such target exists** — `grep` **0** in `ailang-world` and **0** in V1's `Makefile` — while this repo is `"visibility":"PUBLIC"` and carried a personal address in **7** doc locations (4 charter, 1 log, 2 archive), all written `2026-09-01`, the day BEFORE the rule existed. Redacted by pure substitution to `an ATTENDED (non-fleet) author`: **7 → 0**, `git diff --stat` a balanced **7+/7−** so no line was added or lost, the rule's actual verdict (*attended*, not *fleet*) preserved in every cell, and it had reached **0** comments on `#107` so the report channel never carried it. `scripts/mission_answer.sh:69`'s functional default is deliberately LEFT ALONE. **A THIRD DEFECT, HIT LIVE AT MY OWN GATE 0 AND FILED AS [ailang#1037](https://github.com/sunholo-data/ailang/issues/1037) + NEW ROW 75:** `ailang messages list approvals --unread` returns rows from EVERY inbox, read and unread — Go's `flag` package stops at the first non-flag argument, so the positional is accepted, every following flag (`-unread`, `-json`, `-limit`) is discarded, and it exits **0** with the unfiltered listing. Three arms: wrong form → all inboxes and `-json` not honoured (the tell); control `-inbox approvals -unread -json` → `n=1 {approvals:1} {unread:1}`; nonsense positional → **rc=0** and the default listing. The result is a SUPERSET, which reads as a healthy inbox — a Gate-0 hazard for all four missions. The missing gate itself is **NEW ROW 74**, not absorbed — **and row 74's own write-up then re-introduced an 8th occurrence by quoting the script verbatim, caught by the post-edit control one command before the commit**: a recurrence inside the same iteration that redacted it, which is the strongest possible evidence that the deliverable is a CI gate over the mission docs and not a controller habit. **NEXT:** rows **58**, **59**, **60**, **61**, **62**–**66**, **68**–**75**, then **39**. Row **50** stays parked on `D-WORLD-31`; row **57** is now TRACKING-ONLY on upstream `#984`/`#1036`. **Decision ledger: 18 rows, ONE OPEN — `D-WORLD-31`, re-asked unchanged.**
...
50. **[`D-WORLD-31` RESOLVED 2026-09-03 (Mark, attended) — NEITHER OPTION AS OFFERED. THE ROW HOLDS
...
59. **[DESIGNED + QUORUM-CLEARED 2026-09-04 (iter-154) — `[NEXT]`: this is the sprint to plan.**
