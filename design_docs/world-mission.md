...
- **`OD-<n>` IS A MISSION-GLOBAL NAMESPACE, AND THIS TABLE IS ITS REGISTRY** (process fix, iter-43;
  ...
  **Rules.** (a) Before allocating an `OD-<n>`, enumerate `### OD-` headings across **every** doc in
  ...
  |---|---|---|---|
  | `4e/OD-1` | 4e `w-race-gate-blindspot` | lower the `go.mod` floor 1.26.4 → 1.25.6 | **RATIFIED** 2026-08-03 — pin Go 1.25.6 |
  | `4e/OD-2` | 4e | file the 52-line reproducer upstream at `golang/go` | **AUTHORIZED** 2026-08-03 |
  | `4e/OD-3` | 4e | *also* change the two `scan.go` sites to a compiler-safe shape | **DECLINED as primary** (optional belt) |
  | `4e/OD-4` | 4e | make `maxRecoveryPages` injectable (CF-MJC-1) | open |
  | `4d/OD-3` | 4d `w-ddl-gate-teeth` | add + enforce a `PRAGMA user_version` contract now | **RATIFIED** 2026-08-03 — alt 1, fail LOUD → milestone **DG.B**, still undesigned |
  | `4d/OD-4` | 4d | on DDL change, fail or migrate | open (recommend fail-loud only) |
  ...
  | `4d/OD-7` | 4d `w-ddl-gate-teeth` | how are valuable deployed **version-0** stores transitioned once DG.B starts refusing them — reconstruct, certify-and-stamp, or delay rollout? | **OPEN** (added iter-44). Recommend **inventory first**, then alt 1 (reconstruct) only with first-party proof every store is reconstructible; else alt 2 as a **separate ratified** milestone. Blocks **rollout only**, NOT implementation |
...
4d. [**DG.B DESIGNED 2026-08-03 (iter-44) — IMPLEMENTATION IS ROUTABLE; ROLLOUT IS PARKED ON `4d/OD-7`.**
