...
- **THE PINNED v0.30.0 BINARY CANNOT RESOLVE HALF THE SHARED SKILL'S QUORUM ROSTER — AND THE
  ABSENCE READS AS AN ORDINARY DEGRADE, NOT AS A CONFIGURATION ERROR** (process fix, iter-170).
  The roles table names `gpt6-astra` as both a designer rotation entry and a quorum reviewer, and
  the astra-collision remedy prescribes `--reviewers gpt5-6-sol,gemini-3-1-pro,oc-glm-5-2`. Measured
  first-party at iteration 170: `ailang design-quorum --reviewers gpt6-astra,gemini-3-1-pro,oc-glm-5-2`
  against `~/.pinned-ailang/ailang` v0.30.0 records **two** absentees with reason `unknown-model`
  (`model "gpt6-astra" not in models.yml`), leaving a quorum of ONE. Both ids DO exist in the fleet's
  current `internal/modelreg/models.yml` — the pin is simply older than the roster. **Why this is
  worth a rule rather than a shrug:** the artifact reports it through the same `absent_reviewers`
  channel as a budget refusal or an outage, so the natural reading is "degrade to N−1 and carry on",
  and standing rule 8 has no class for it — it is neither a judgment park nor a capacity park that
  a clock resolves. It is a **capability limit of the pinned toolchain**, and re-probing will never
  clear it. **Rule: on this mission the usable reviewer set is `gpt5-6-sol,gemini-3-1-pro`.** Name any
  additional reviewer only after probing that the PINNED binary resolves it, and when an absence
  reads `unknown-model`, record it as a pin-version gap — never as a degrade. The fleet-facing half
  (the roster the shared skill names is not resolvable on a pinned consumer) is a PROPOSAL, since
  World may not edit that file.
- **THE PINNED v0.30.0 BINARY HAS NO `ailang mission` COMMAND AT ALL, SO GATE 4's PRESCRIBED INDEX
  REGENERATOR CANNOT RUN HERE — AND THE COST WAS ALREADY ON DISK BEFORE ANYONE LOOKED** (process
  fix, iter-172; **second instance of the pin-version-gap class** the `gpt6-astra`/`oc-glm-5-2`
  bullet above records). Gate 4 says the iteration index is *"regenerated wholesale by `ailang
  mission rotate-log`, never appended to"*, and `design_docs/world-mission-index.md` repeats it —
  correctly, because an append-only index drifts the moment an entry is edited. Measured
  first-party at iteration 172: `~/.pinned-ailang/ailang mission rotate-log world --keep 31` prints
  **`Error: unknown command 'mission'`** and falls through to the top-level help, with the positive
  control (`design-quorum --help`) resolving fine — so it is the whole command group that is absent
  from the pin, not one flag. **The consequence had already happened and nothing reported it:**
  iteration **171** had a full log entry and **no index row** (`grep -c '^| 171 |'` → **0**, control
  `170` → **1**), which is exactly charter row **86**'s class — now with a root cause instead of an
  incident. **Why this is worth a rule rather than a shrug**, and it is the same shape as the
  reviewer-roster bullet above: the failure is SILENT in the direction that matters. Gate 2's very
  first instruction is to grep the index before picking, so a missing row does not produce an error
  — it produces a *confident* "nothing like this has been tried". **Rule: on this mission, adding
  the iteration's index row is a MANUAL Gate-4 step, performed in the same commit as the log entry,
  and Gate 4's rotation-regeneration prescription is inapplicable until the pin gains the command.**
  Record a missing row as a **pin-version gap**, never as a controller lapse, and never as a reason
  to distrust the index's other rows. The fleet-facing half (a shared gate prescribing a command the
  pinned consumer does not have) is a PROPOSAL, since World may not edit that file. The tell: a gate
  tells you to regenerate an artifact, and you have not asked whether the binary you are required to
  use has the verb.
- **RUN A `bash` SCRIPT UNDER `bash` — THE LOGIN SHELL HERE IS `zsh`, AND USING IT AS THE INSTRUMENT
  MANUFACTURES FAILURES IN THE MOST ALARMING DIRECTION AVAILABLE** (process fix, iter-170).
  Verifying the fleet's newly landed de-fork guard, the controller sourced
  `tools/launchd/lib/pin-root.sh` in the login shell and ran its helpers directly:
  `_pin_same_repo` returned **rc=2 ("indeterminate origin identity") for every repository on the
  rig, including a freshly `git init`'d positive control**. Read literally, that says a guard the
  fleet shipped hours earlier fails closed everywhere — a report this loop was one step from
  filing. Re-run as `bash -c '. pin-root.sh; …'`, all twelve acceptance criteria matched their
  stated expectations exactly (rc=1 for World, rc=0 for motoko, the override winning both ways, the
  origin-less repo LOUD at `PIN_STATUS=STALE`). **Rule: any `#!/bin/bash` script this mission
  sources, greps for functions in, or executes as an acceptance criterion is invoked as
  `bash <script>` or `bash -c`, never through the interactive shell.** This is verification rule 3c
  aimed at a SHELL rather than a service — *the shell you run a check from is part of the
  instrument* — and it pairs with the already-recorded `grep` fact (this login shell's `grep` is
  **ugrep**; use `/usr/bin/grep` in scripts and acceptance criteria). The tell: a check comes back
  red in a way that indicts someone else's freshly landed work, and you have not yet re-run it under
  the interpreter the file declares.
...
- **THE ATTENDED-LEDGER PRIVACY CONTRACT HAS A REAL GATE HERE — `scripts/check_no_personal_email.sh` (process fix, row 74, landed iter-175).** The shared skill's Gate 0.6(b) names `make check-no-personal-email` as the enforcement behind "compare, never record"; World has no Makefile, so the gate is the script itself, run by CI's `go-verify` job as two steps (`Refuse a personal email in the loop-written surface` → `bash scripts/check_no_personal_email.sh`; `Personal-email gate self-test` → `bash scripts/test_check_no_personal_email.sh`). Scope is the loop-written surface — the six `design_docs/*mission*.md` docs and `scripts/` — so **the charter, the log, the archives, the dashboard and the index are all scanned on every push**: write a provenance VERDICT ("attended"/"fleet"), never an address, and when a record must show an address-shaped token that is not a person (an SSH remote, a Go module id), break it with ` AT ` as row 91 does. Allowed by anchored exclusion: `…@users.noreply.github.com`, `noreply@…`, `…@example.com|org|net`, `.invalid|.test|.localhost`, `….gserviceaccount.com`, `…@sentry.io` — every clause anchored to the whole token, because the fleet's copy (`sunholo-data/ailang` `f0d44915d`) matches substrings and admits `attacker-noreply AT <personal domain>` (this iteration's judge, reproduced). Exit 2 = zero in-scope files (never a ✓ over nothing). `scripts/mission_answer.sh`'s default identity is the GitHub noreply form (the fleet's `8369877d9`), so the script no longer carries the address the gate exists to catch. Run it locally before any Gate-4 commit: `bash scripts/check_no_personal_email.sh; echo rc=$?`.
...
- **QUORUM CAP: PASS `--max-cost-usd 0.30` ON EVERY `design-quorum` RUN IN THIS MISSION (process fix, iter-169 — THIRD instance).** The shared skill's absent-reviewer rule says a `proceed`/`blocked` verdict with a non-empty `absent_reviewers` is a verdict with a named hole, and prescribes re-running the absentee alone at a raised cap. Correct, and it fires *after* the money has already been wasted on an incomplete round. The default cap is **$0.10** and `gpt6-astra` costs **$0.09–$0.11** on a doc of ordinary length, so it is refused **pre-flight, at zero spend**, by a margin of cents — iteration 169 round 1: *"estimated cost $0.1130 (doc ~6181 input tok) exceeds cap $0.1000"*, i.e. refused over **$0.0130**. **The trigger is self-selecting in the worst way**: the estimate scales with DOC LENGTH, so the reviewer drops out exactly when the doc has grown — which is immediately after a substantial revision, which is when its opinion is most load-bearing. Three instances in this mission, and in every one the restored reviewer was not merely present but *decisive*: iter-167 (the restored absent reviewer found the iteration's best defect for $0.13); iter-169 round 1 (restored for **$0.0858**, returned the round's strongest objection — the proposed shared helper was not behaviour-preserving — and was confirmed by measurement); iter-169 round 2 (astra again correct, and again the only reviewer to see that the fix was conditional rather than unconditional, controller-reproduced first-party). Note what the default cap actually buys: refusing a $0.11 reviewer to protect a **$5** iteration ceiling — 0.2% of the budget — at the cost of running the gate at N−1. **Rule: pass `--max-cost-usd 0.30` on the FIRST round, not as a repair on the second.** The re-run rule stays as the belt; this is the braces. World-local because the cap is a per-invocation flag, not a skill constant — no shared-skill edit is needed, and none is proposed.
...
- **A PI EVALUATOR SPAWN IS TRANSPORT-DEAD UNTIL THE SESSION-PROTOCOL GATE IS PRESENT IN THE
  EVALUATOR SESSION'S PROJECT SCOPE — VERIFY BEFORE LAUNCH AND REPAIR WORKTREE-SCOPED (process
  fix, iter-171, first instance: the REQUIRED evaluator's first attempt stopped at its
  handshake's step 4 because World's repo scope ships no `.pi/extensions/`, so
  `session_protocol_ack` does not exist in a World pi session).** The gate is an ailang-shipped
  pi extension (`session-protocol-gate.ts`, fleet repo `.pi/extensions/`, 370 lines); V1's pin
  worktree carries it project-scoped, World does not, so every pi lane fails identically — the
  failure is TRANSPORT, not weights, and the openrouter twin would have died the same way. The
  evaluator did exactly right: it stopped per the handshake contract, reported the exact missing
  tool, and produced NO verdict (a role transport failure never consumes the judgment budget).
  The controller repair is bounded and rig-local: copy the gate into
  `<eval-worktree>/.pi/extensions/` before the launch (untracked; zero repo/fleet/global
  change), record the repair in the routing block, and re-run the SAME lane. Do NOT run
  `ailang pi install` from an unattended fire — that changes every pi session on the rig, not
  just this mission's. The durable repo-side fix is queue row 89's decision, not this bullet.
...
- **THE RIG'S SHELL IS `zsh`, AND THREE OF THE SKILL'S OWN INSTRUMENT IDIOMS BREAK HERE — TWO OF
  ...
  Every Bash call in this loop runs under `zsh`, not bash, and (a) and (b) return something that
  looks exactly like a passed check:
  ...
  (c) **A MULTI-LINE `jq` FILTER PASSED INLINE AS A `--jq` ARGUMENT IS MANGLED IN TRANSIT** —
  added iter-159, the third instance in this family and the second to land on the Gate-3b poll in
  three iterations (iter-157 was (b)'s sibling: `set -- $raw` does not word-split in `zsh`, so
  three counts arrived as one string). The filter's own `"`, `!=` and newlines do not survive the
  argument, and `gh` reports `failed to parse jq expression … unexpected EOF`. Unlike (a) and (b)
  this one fails LOUDLY, and it is listed here anyway because the loop rewrites this poll from
  scratch every iteration and has now paid for it twice. **Write the filter to a file and use
  `jq -r -f <file>`** — the file is not re-scanned by the shell, so quotes and operators stay
  data. The general rule the three share: **anything this loop passes to another parser as one
  shell argument is a claim about quoting, not about the parser.** And keep the numeric floor
  regardless — `case "$counts" in ''|*[!0-9]*) echo "INSTRUMENT FAILURE — not a verdict";; esac`
  is what turned both of the last two breakages into a refusal instead of a vacuous green.
...
- **A ROLE'S `ok` IS A CLAIM ABOUT A WORKTREE, NOT ABOUT ITS DELIVERABLE — ASSERT THE NAMED FILE, AND ASSERT EVERY ID A REVISION CITES (process fix, iter-187; two instances in one iteration, different roles).** (1) The kimi planner returned `mission_pi_run.sh` verdict **`ok`** with `worktree_changed_files: 10` after an Ollama Cloud 429 had killed it mid-run — all ten files were its prototype, and the plan it existed to write did not exist. The runner's load-bearing assertion is a non-empty diff, which a prototype satisfies. (2) The r1 designer revision's quorum-log prose promised `AC-DENIAL-JSONRPC` and `MUT-DENIAL-401`; neither reached the AC or mutation tables (grep count 1 each, both in the prose), and r2 blocked 3/3 on exactly that gap. **Rule:** after any role run whose deliverable is a named file, `test -s <that file>` (and `jq -e .` for JSON) before reading the verdict as success; after any revision, grep every AC/MUT/F-row ID the revision's own summary cites and require a hit in the binding table, not only in the narrative. World-local (a controller step, not a skill edit); if a sibling mission hits (1), it is a PROPOSAL to V1 for `mission_pi_run.sh` to accept an `--expect <path>`.
...
### Evidence hygiene (World-local process, iteration160)

- Configuration diagnostics select an explicit allowlist of fields inside the parser;
  never print whole authentication files or surrounding grep context. Incident owner
  actions belong in the decision ledger and approvals inbox, without credential values.
- Check evaluator rubric arithmetic, cited-path existence and actor attribution against
  primary command records. Bank independent commands with the verdict. Label any
  controller transcription and preserve the evaluator's actual numerical judgement.
- Use observed lifecycle timestamps and identify their source; never guess start/end
  times. Preserve measured driver routing exports when invoking resolvers in worktrees.
...
| D-WORLD-28 | RESOLVED | **ONE WORD — how should `verify_go.sh` guarantee its nested race-control module can execute?** **A (recommended): fail closed unless the selected `ACTIVE_GO` is at-or-above the root module floor, then bind the race-control invocation to that exact `ACTIVE_GO`; statically require `racecontrol/go.mod` floor <= root floor.** **B: keep ambient nested-module auto-selection and pin `racecontrol/go.mod` to an explicit minimum independent of the root floor.** Quorum round 2 found that A still needs the explicit `ACTIVE_GO >= root floor` runtime floor: under `GOTOOLCHAIN=local` with a base below the root floor, `go env GOVERSION` can return that lower base, so the static implication alone is false.  **ANSWERED — A — fail closed unless the selected ACTIVE_GO is at-or-above the root module floor, then bind the race-control invocation to that exact ACTIVE_GO, with the static racecontrol/go.mod floor <= root floor requirement. Include the runtime floor check quorum round 2 found necessary: under GOTOOLCHAIN=local with a base below the root floor, go env GOVERSION can return that lower base, so the static implication alone is false.** (Mark Edmondson, attended 2026-09-01, recorded directly in this ledger.)| Iteration 137; two full-strength quorum rounds, all three external reviewers present, both BLOCKED. Artifacts `w-racecontrol-floor-bump-disarms-the-race-control-2026-08-28T15-21-12Z.json` and `…T15-27-58Z.json`.  **Attended ruling 2026-09-01** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue; provenance is an ATTENDED (non-fleet) commit author, which the fleet bot does not hold and the loop may not author with. Attended ruling, matching the loop recommendation on all three; each option was already measured first-party by the loop and no reviewer disputed the direction. Unparks rows 48, 50 and 52 for the normal queue order.|
| D-WORLD-29 | RESOLVED | **ONE WORD — after the whitespace-tolerant rewrite of `shellAssignmentValues`, should a single *indented* assignment be ACCEPTED or REJECTED?** **A (recommended): ACCEPT it — one whitespace-tolerant scan; trim leading spaces/tabs, ignore comments, extract `NAME="…"` regardless of indentation, require `len(values) == 1`.** The row's silent arm then yields count=2 and REDS, so the row's whole purpose is served; an indented-only assignment yields count=1 and supplies its value, which is what bash actually does. Consequence: today's loud red on the indented-only shape disappears — and it is measured to fire because the scanner cannot SEE a real assignment, not because anything is wrong. This is queue row 50's own text and `gpt5-6-sol`'s verbatim `proposed_fix`. **B: REJECT it — keep the design doc's two-sided invariant (column-0 count == 1 AND indented count == 0).** Strictly stronger: reds on ANY indentation, including a benign re-indent — but its stated rationale ("an indented assignment is a conditional/nested shadow") is measured FALSE, so B would be chosen for defence-in-depth, not for the reason the doc gives. **Answering also AMENDS queue row 50**, which lists "an indented *only* assignment likewise reads 0 and fatals" among the correct loud behaviours; that sentence carries the same false premise and is ratified text the loop may not edit itself. **Default if unanswered:** the item stays parked and the queue advances past it to rows 51–58.  **ANSWERED — A — ACCEPT the indented assignment. One whitespace-tolerant scan: trim leading spaces/tabs, ignore comments, extract NAME="..." regardless of indentation, require len(values) == 1. B's stated rationale ("an indented assignment is a conditional/nested shadow") is measured FALSE, and defence-in-depth bought with a false premise is not worth a loud red on a benign re-indent. AMEND queue row 50 in the same iteration to drop the sentence carrying that premise — it is ratified text the loop could not edit itself, and this ruling authorises exactly that edit.** (Mark Edmondson, attended 2026-09-01, recorded directly in this ledger.)| Raised iter-140 (2026-08-31) after two full-strength quorum rounds both BLOCKED. Premise measured first-party by the controller: a bash script setting `COL0=`, a space-indented `INDENTED=` and a tab-indented `TABBED=` at top level prints all three, rc=0; paired control — a genuinely conditional assignment DOES change behaviour (`NARROW` unset → `wide list`, set → `narrow`). Quorum artifacts `.ailang/state/mission-quorum/w-shell-assignment-parser-drops-an-indented-assignment-2026-08-31T14-03-36Z.json` and `…T14-16-33Z.json` (that directory is gitignored, so the decision-bearing subset is quoted here); round 2 restored `oc-glm-5-2` → PASS at $0.0203, so `absent_reviewers` is closed and the quorum is full-strength.  **Attended ruling 2026-09-01** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue; provenance is an ATTENDED (non-fleet) commit author, which the fleet bot does not hold and the loop may not author with. Attended ruling, matching the loop recommendation on all three; each option was already measured first-party by the loop and no reviewer disputed the direction. Unparks rows 48, 50 and 52 for the normal queue order.|
| D-WORLD-30 | RESOLVED | **ONE WORD — should the row-52 CI step-scoping fix stay a LINE SCAN, or become a YAML PARSE?** **A (recommended): LINE SCAN, hardened.** Keep the text scanner and add `gemini-3-1-pro`'s verbatim fix — require the derived step column to equal the constant `6` and fatal loudly otherwise — or equivalently derive the anchor from the SHALLOWEST enclosing `steps:` rather than the nearest one. Measured: the shallowest-anchor variant catches the round-2 attack (`stepCol=6`, `start=173`, flag found → RED) on the identical file where the doc's nearest-anchor derivation is blind (`stepCol=12`, both invariants PASS, flag excluded → GREEN). Consequence: no new dependency; the residual is that the scan stays text-level, so `gpt5-6-sol`'s general point — block-scalar content is author-controlled — remains true in principle, and a future re-indent of `ci.yml` makes the gate fatal until the constant is updated. **B: YAML PARSE.** Adopt `gpt5-6-sol`'s verbatim fix — parse `ci.yml` into a node tree, fail loudly on parse errors, aliases, flow-style steps and duplicate keys, locate the step semantically and reject `continue-on-error` on that mapping. Strictly stronger: it closes the CLASS rather than the measured instances. Consequence: adds the **second** direct dependency to a `go.mod` that has exactly **one** (`modernc.org/sqlite`), and widens a ~0.1d row into a dependency decision. **Recommendation: A.** This gate is a repo-internal tripwire against our own accidental regressions, not an adversarial boundary — the threat model that makes B necessary is an actor who can already edit `ci.yml`, and such an actor can simply delete the test. **Default if unanswered:** row 52 stays parked and the queue advances to rows 53–59 then 39. **ANSWERED — A — LINE SCAN, hardened, deriving the anchor from the SHALLOWEST enclosing `steps:` rather than the nearest one (measured to catch the round-2 attack that the doc's nearest-anchor derivation is blind to). This gate is a repo-internal tripwire against our own accidental regressions, not an adversarial boundary: the threat model that would justify B is an actor who can already edit `ci.yml`, and such an actor can simply delete the test. Not worth adding the second direct dependency to a `go.mod` that has exactly one. The residual is accepted and named: the scan stays text-level, and a future re-indent of `ci.yml` makes the gate fatal until the constant is updated.** (Mark Edmondson, attended 2026-09-01, recorded directly in this ledger.) | Raised iter-142 (2026-09-01) after two FULL-STRENGTH quorum rounds both BLOCKED (`.synthesis.absent_reviewers` `[]` in both, cross-checked against `[.reviewers[]\|select(.present==false)]`, control `has("synthesis")` true). Round 1 $0.1035 (reject/reject/pass + controller reject); round 2 $0.1489, all three external reviewers rejecting. The carve-out does not apply because the reviewers' `proposed_fix` fields disagree and `gpt5-6-sol`'s disputes the DIRECTION, naming park-or-widen itself. Both locators were refuted by first-party measurement, not by argument, and the row's own "non-exploitable" claim was refuted in both directions before any routing (ARM A false positive at `ci.yml:166` on an unrelated step; ARM B `rc=0 --- PASS` with the forbidden flag live on the guarded step). Doc banked at `design_docs/planned/w-wiring-test-step-scoping-imprecise-under-key-reorder.md`; quorum artifacts under `.ailang/state/mission-quorum/` dated `2026-09-01T00-50-31Z` and `T01-05-28Z` (that directory is gitignored, so the decision-bearing subset is quoted here). **Attended ruling 2026-09-01** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue; provenance is an ATTENDED (non-fleet) commit author, which the fleet bot does not hold and the loop may not author with. Attended ruling, matching the loop recommendation on all three; each option was already measured first-party by the loop and no reviewer disputed the direction. Unparks rows 48, 50 and 52 for the normal queue order. |
| D-WORLD-31 | RESOLVED | **ONE WORD — `D-WORLD-29` ratified rule A on a rationale that a measurement taken AFTER the ruling shows does not cover every shape. Ship A as ratified, or hold row 50 for the fixture migration?** `D-WORLD-29` chose A ("ACCEPT the indented assignment … which is what bash actually does"). That rationale is TRUE for a **top-level** indented assignment — measured, and it is what was in front of you. It is **FALSE for an assignment inside a multi-line branch that never executes**, and that shape is not a corner: `run.sh` carries **17** control-flow openers and **65** indented lines (control: a fresh absent keyword reads 0), so there are 17 live positions where a refactorer can put one. **The measurement, first-party at `cb73cab`, mutant LANDED by sha256, `go vet` rc=0 read before every test result, restored byte-identical, pristine control green either side:** with the only `KNOWN_BAD` inside `if false; then` / `fi` and no column-0 copy, the CURRENT helper is **rc=1 `KNOWN_BAD assignment count=0, want 1`** (correctly loud) and the ratified rule-A helper is **rc=0 RUN=2 PASS=2** — while `bash -c 'if false; then KNOWN_BAD="x"; fi; echo "${KNOWN_BAD:-<UNSET>}"'` prints **`<UNSET>`**. So rule A binds the toolchain floor against a deny-list that does not exist at runtime. Paired control, same batch, proving the helper is not merely permissive and that **row 50's own defect really is closed by A**: the row's silent arm (a second, indented `KNOWN_BAD` beside the column-0 one) goes **rc=1 `count=2, want 1`** at both consumers. **This is a defect rule A INTRODUCES, not one it fails to fix** — which is why it is a decision and not a queue row. **A (recommended): SHIP A AS RATIFIED.** Residual 6 is declared in the doc and in the helper comment, pinned by unit arm 11, and recorded as `V-ARMC1`/`V-ARMC2` with its runtime control. Row 50's defect closes; the gate trades one silent hole (a second indented assignment narrowing the list) for another (a never-executed branch supplying it), and the new hole is louder to a reader because it is written down. Cost: ~0.1d, already designed and planned. **B: HOLD row 50 for the fixture migration** — `gpt5-6-sol`'s verbatim `proposed_fix`: move the pins into a declarative, data-only fixture holding exactly one whitespace-tolerant `NAME="…"` record per name, have `run.sh` source it and the Go gates scan it, rejecting non-comment/non-assignment syntax, with mutation arms proving duplicate, `export` and control-flow text all RED. Closes BOTH holes and removes the text-scanner class entirely; costs a new design doc and roughly a day, and it is scope this row never had. Its verbatim words: *"If that migration is out of scope, keep the row blocked rather than accepting the demonstrated V-ARMC2 false green."* **NOT AVAILABLE, and measured so rather than assumed:** a cheap "refuse if the script contains control flow" floor is dead on arrival — it would fatal on the pristine `run.sh` at all 17 openers. A hybrid that also requires column-0 count ≥ 1 is exactly rule B on the one point `D-WORLD-29` decided, so it is not offered. **Default if unanswered:** row 50 stays parked and the queue advances to rows 52, 54–61, then 39; nothing else is blocked by it.  **ANSWERED — NEITHER OPTION AS OFFERED. Take the stated default — row 50 stays parked (zero cost, nothing else blocked; the queue advances to 52, 54-61, then 39) — and FOLD option B's fixture migration into row 59's design rather than funding it as a doc of its own. The reason is a fact the row did not state: row 59, already the next pick, is the SAME defect class as rule A's residual, demonstrated with the SAME construction. Row 59 is "a `grep -c` cannot tell an assertion that runs from one that is compiled and never reached", proven by wrapping the assertion block in `if false { ... }`; rule A's residual is a `KNOWN_BAD` assignment inside `if false; then ... fi` that the helper reads green while bash leaves the name UNSET. So shipping A spends 0.1d to ADD a fresh instance of the very class the next row exists to remove, and the class then gets paid for twice. Option B's remedy is the general cure for both — a declarative, data-only fixture holding one `NAME="..."` record per name, sourced at runtime and scanned by the gates, replaces "read the code and guess what runs" with "read one declarative record" — so buy it ONCE, inside row 59, and let row 50 close as a consequence. This also lands `gpt5-6-sol`'s own stated position rather than overriding it. CONDITION, and it is load-bearing: this hold is contingent on row 59 actually being taken next. If row 59 slips or is deprioritised, row 50's original defect must not sit open indefinitely — come back and ship A rather than leaving both parked, and say so in that iteration's report. Credit where due: the loop MEASURED the cheap "refuse if the script contains control flow" floor as dead on arrival (it fatals on the pristine `run.sh` at all 17 openers) instead of assuming it, which is what made this a genuine two-way choice rather than a false one.** (Mark Edmondson, attended 2026-09-03, recorded directly in this ledger.)| Quorum round 3, full strength — 3 of 3 reviewers PRESENT, `.synthesis.absent_reviewers` `[]` cross-checked by `[.reviewers[]\|select(.present==false)]`, verdict **blocked**, `metered=$0.15101`: `gpt5-6-sol` **reject** (this objection), `oc-glm-5-2` **reject** (AC7's base FAIL set quoted at `9c0ad0b` while the doc targets `cb73cab` — a PREMISE objection, measured and fixed in-loop rather than forwarded), `gemini-3-1-pro` **pass**. Doc `design_docs/planned/w-shell-assignment-parser-drops-an-indented-assignment.md` at 678 lines, revised to direction A this iteration by `pi:ollama/deepseek-v4-flash:0731-cloud` (typed verdict `ok`). `D-WORLD-29` is NOT reopened by this row — it is a new, uniquely named decision carrying a fact that post-dates the ruling, per the shared skill's ATTENDED LEDGER EDITS rule (g).  **Attended ruling 2026-09-03** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Mark gave this ruling in an attended session; the agent transcribed it and the commit is authored by the fleet bot, because this machine's git identity is the fleet bot for every session including Mark's. Provenance is therefore the attended session, NOT the commit author — and nothing in `scripts/mission_decisions.sh` or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-03, positive-controlled), so no control is weakened by saying so plainly rather than asserting an authorship that did not happen. Row 59 text in design_docs/world-mission.md (`w-static-grep-cannot-prove-an-assertion-is-live`, clause-2) quoted first-party this session: same class, same `if false` construction, already the next pick — the fact that decides this row and the one the D-WORLD-31 row did not carry. Rule A's residual measured at `cb73cab` (V-ARMC1/V-ARMC2, unit arm 11); runtime control `bash -c 'if false; then KNOWN_BAD="x"; fi; echo "${KNOWN_BAD:-<UNSET>}"'` prints `<UNSET>`. Quorum round 3 full strength, verdict blocked, metered $0.15101. Attended ruling 2026-09-03 after reviewing both options and the row-59 overlap.|
| D-WORLD-32 | RESOLVED | **Rotate/revoke and replace the exposed local credential now, or defer?** A (recommended): rotate and replace, retiring the exposed value. B: defer, leaving it valid. Default while unattended: no account changes by this mission; rotation stays parked until owner confirmation at the next attended review.  **ANSWERED — A: authorize scoped rotation/revocation and replacement of the exposed credential. Identify the owning service/account from existing private configuration, install and verify the replacement for affected consumers, then revoke the exposed value; revoke immediately if active misuse is detected. Do not print or publish secret values. If ownership/access cannot be established, record that concrete operational blocker and advance unrelated ready work; this policy decision is resolved, not a claim rotation is complete.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iter160 tool-transcript exposure; no value copied to GitHub. Controlplane inbox_1788674084211_78d26150; approvals inbox_1788674300667_58de2db6.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-WORLD-33 | RESOLVED | **Redesign row65 around explicit mutation/compile-evidence records, or defer it?** REDESIGN (recommended): replace prose inference with explicit records and a fresh design/quorum; spends further harness work before implementation. DEFER: keep row65 parked and advance to rows66/68; the compile-fence guidance gap remains. Default if unanswered: DEFER immediately during unattended runs; no proposed lint ships.  **ANSWERED — DEFER row65 for this week and advance ready product work. Do not ship the proposed prose-inference lint. Revisit explicit mutation/compile-evidence records only with a fresh design if measured run evidence warrants it.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iter161: two blocked quorums; r1 three rejects, r2 Astra/Gemini reject and GLM invalid/absent. Controller reproduced Markdown-backtick blind spot with a firing plain-text control. design_docs/planned/w-go-test-compile-fence.md; design_docs/verification/world-iter161/README.md.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-WORLD-34 | RESOLVED | **Does attended DE-FORK supersede D-WORLD-DRIVER-1/row76 enough for World to retire the obsolete live `--driver-fleet-check` diagnostic, or must it preserve that frozen diagnostic byte-identical and only remove it from default product verification?** A (recommended): retire the obsolete live diagnostic and its fixtures as part of the DE-FORK transition; World verifies only its own inputs and no longer carries a permanently red local-copy interface. B: preserve the frozen diagnostic byte-identical and limit the repair to removing default invocations; file/track fleet remediation separately. Default while unattended: B, park planning/execution until explicit authority resolves the ratified-row conflict.  **ANSWERED — A: the attended DE-FORK supersedes the local frozen-diagnostic requirement in D-WORLD-DRIVER-1/row76. World may retire the obsolete local --driver-fleet-check diagnostic and its fixtures/invocations; shared fleet verification belongs to the shared AILANG driver. Preserve World application verification. Correct the scoped design/AC gap, then plan/execute/evaluate the CI ownership repair; no blanket deletion of product checks.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iter162: DE-FORK commit `e92594c` made dev CI red by deleting the local driver/routing files while CI and `verify_go.sh` still invoked them. Two design quorums blocked on `w-de-fork-ci-ownership`; r2 had Gemini rejecting a mechanical AC7 scope gap and GLM rejecting the authority split around row76. Independent fallback evaluator PASS90/100 says parking is correct and no code should land.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-WORLD-35 | RESOLVED | **May a docs-only mission-record PR merge while the DE-FORK CI reds stay parked, or must every record wait?** A (recommended): MERGE docs-only mission-record PRs once the charter-declared Gate-3b job (`CI` / `ailang-code verify gate`) is green on the exact head and the other two checks are proven INHERITED — a documentation commit cannot make an already-red `dev` redder, and the reds are a separate, separately-parked question. B: HOLD every record until the CI reds are repaired — the current de-facto behaviour. Default while unattended: B (hold), because reversing a prior iteration's disposition is not an unattended call. **This is NOT the same question as the row-68/CI-repair park**: it asks only whether *records* are hostage to it.  **ANSWERED — A: permit documentation-only mission-record PRs to merge when the charter-declared ailang-code verify gate passes on the exact head and remaining reds are demonstrated inherited against the base. This does not waive required branch protections, authorize code changes under a records label, or declare inherited CI repaired. Reconcile all pending ledger records, including D-WORLD-34, without overwriting attended rulings.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iter166. `sunholo-data/ailang-world` `dev` at `9166de0` is RED on `launchd drivers (bash 3.2)` and `go host build + test gate`, both because the attended DE-FORK `e92594c` deleted files those jobs invoke; `ailang-code verify gate` passes. PR [#127](https://github.com/sunholo-data/ailang-world/pull/127) (iterations 162-165's records) has been open and MERGEABLE since 2026-09-06 for this reason, and iteration 166's record PR joins it — so five iterations of charter/log/ledger state now live only on unmerged branches. Measured consequence, first-party: the committed ledger on `dev` contains no `D-WORLD-34` row (`rg -n 'D-WORLD-34' design_docs/world-mission.md` -> 0 hits, positive control `D-WORLD-DRIVER-1` -> hits), so `scripts/mission_decisions.sh --open` cannot see a park that iterations 162 and 165 both relied on, and the independent evaluator flagged exactly that provenance gap.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-WORLD-36 | RESOLVED | **ONE WORD — row 39's sprint is COMPLETE and judged (PASS 95/100, zero blocking) but cannot land from a pi session: the fleet prepush-gate extension is unpassable in this repo (Go under `cmd/`, no Makefile, escape hatch unimplemented). How does it land?** **A (recommended): land it attended** — from any non-pi session (or Mark at the terminal), run `git push origin sprint/w-session-authority` in the main checkout and open the PR to `dev`; the branch is `6f2eaf8` (worktree `.wt-world-iter178`), green on the repo's own gates, and CI re-runs them. **B: fix the harness first** — an attended session implements the extension's documented `AILANG_SKIP_PREPUSH=1` escape hatch (or narrows the repo predicate to repos that actually have a Makefile), after which the next fire lands row 39 itself and every future pi-controller fire with it. Default while unanswered: the park already in place — row 39 stays PARKED, the loop picks row 92 next, re-attempts nothing.  **ANSWERED — B — fix the harness (Mark, attended 2026-09-23). The prepush-gate defect is fixed at source in ailang 754fc7a43: each make gate runs only when the repo's Makefile defines the target, so ailang-world (Go under cmd/, no Makefile) now gets gofmt only. It is installed live on the rig (~/.pi/agent/extensions/prepush-gate.ts, delivered by ailang pi install). Next fire: push sprint/w-session-authority @ 6f2eaf8 plus the unpushed record(180) commit and open the PR to dev. Row 95 is resolved by this fix. The refused fires are correct posture; quota returns later in the week.** (Mark Edmondson, attended 2026-09-23, recorded directly in this ledger.)| Iter-180 (2026-09-23): `AILANG_SKIP_PREPUSH=1` exported, land attempt refused byte-identically (doc/code drift measured against `~/.pi/agent/extensions/prepush-gate.ts`, whose code never reads the variable); `make lint` → `No rule to make target 'lint'` with no Makefile in the repo (control: `scripts/verify_ail.sh` resolves and is green, 11 identities/40 tests/9-9 steps); full defect record in row 95; judge PASS 95/100 at `mission-world-iter180-evidence/EVAL_REPORT_iter180.md`.  **Attended ruling 2026-09-23** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. ailang 754fc7a43 + 37c1b76c8; bun harness against the real ailang-world tree: no make runs, not blocked; against ailang: lint + file-sizes still run; old gate fails 8/14 of the same checks|
| D-WORLD-37 | RESOLVED | **ONE WORD — row 23's plumbing has landed (`b9ecabe`: all 11 store reads inherit the caller's context, and the ratchet is empty). D-WORLD-18 returned two policy questions to you before the closing move routes: what bounds the approval path, and `Commit` atomicity vs deadline. Which direction for the policy tranche?** **A (recommended): finite active operations.** Keep the declared strict store-boundary guard (reject nil or deadline-free contexts) as the closing move. Require finite budgets for daemon startup/bootstrap, approval active I/O, publish validation and credential lookup; human think time between polls consumes no budget (measured: the approval protocol is request/poll, so no store read waits on a human). `Commit` accepts a ctx and may be cancelled before the durable commit, all-or-nothing; an uncertain outcome is reconciled, never blindly retried. A later design brings you the concrete bound table before anything ships. **B: preserve admitted work.** Keep deadline-free startup and attended paths as documented exceptions, and let an admitted `Commit` finish regardless of caller cancellation. This revises D-WORLD-18's universal-guard closing move into an exception policy. Measured impact of a strict guard today: 4 production root paths would fail (approval mint, publish validation, daemon bootstrap, credential lookup), see the design's §4. Default while unanswered: nothing ships in the policy tranche; row 23 stays OPEN as its owner, and the loop picks other rows.  **ANSWERED — A: finite active operations. Keep the strict store-boundary guard (reject nil or deadline-free contexts) as the closing move; require finite budgets for daemon startup/bootstrap, approval active I/O, publish validation and credential lookup, with human think time between polls consuming no budget; `Commit` accepts a ctx and may be cancelled before the durable commit, all-or-nothing, and an uncertain outcome is reconciled, never blindly retried. The policy tranche routes as loop work under row 23; its design brings the concrete bound table to Mark before anything ships.** (Mark Edmondson, attended 2026-09-26, recorded directly in this ledger.) | Filed iteration 194 (fleet, unattended); design `design_docs/implemented/w-store-deadline-free-residue-owner.md` §4–§5, guard probe `~/.ailang/state/world-iter194/design/guard_probe.py` (8/8 → 4 fail with the guard, 4 bounded controls pass). |
| D-WORLD-38 | OPEN | **ONE WORD: which phrase must an operator type to publish a transition-registry revision?** Row 107 landed `world-publish transitions` (`b7cfbdb`) behind the existing attended-operator fence. That fence has three parts: a CI tripwire, a controlling-TTY probe and a typed phrase. The verb currently reuses the package verb's phrase, `publish world/core@0.1.0 irreversibly`, which names a package version and not a registry revision. **A (recommended by the designer, shipped as the default):** keep the shared phrase. The gate's strength is the TTY probe and the CI tripwire, not the wording, and a second phrase is more fence surface for no authority gain. **B:** give the verb its own phrase, `publish transition registry irreversibly`, as a ~5-line follow-up. The operator then types what they are actually doing. Default while unanswered: A stays in place and nothing else is blocked. The loop's own lean is B, because an attended confirmation that names the wrong object is weaker evidence that the operator understood what they approved. | Filed iteration 195 (fleet, unattended); design `design_docs/implemented/w-transition-registry-production-publisher.md` §Decision for Mark; code `cmd/world-publish/transitions.go`. |
| D-WORLD-39 | OPEN | **ONE WORD: may row 23's policy tranche (the `D-WORLD-37` = A closing move: context-aware receipt lookup, connection acquisition and pre-durable `Commit`/`AppendIntent`) move from groom position 7 to position 2, directly ahead of row 106's M5?** Row 106 landed M1–M4c (`a9a3382`): the invocation coordinator, with no production caller. Quorum r2 (gpt6-astra, verbatim fix) made bounded durable operations a PREREQUISITE of M5, the `/a2a/` wiring, because `GetReceipt`/`AppendIntent`/`Commit` take no context and wait on a single pooled connection with no bound. So clause 6 cannot move again through row 106 until that tranche lands, while the 2026-09-26 regroom ranks it 7th ("loop work only when positions 1–6 are all blocked or landed"). **A (recommended):** move it to position 2, making the critical path 23-policy → 106 M5/M6 → 108 → 93. **B:** keep the groom order; 106 M5 waits, and the loop takes 108 (blocked on `ailang#885`), then 93 (blocked on 106 + 108), 114 and 94. Default while unanswered, per the Gate-2 critical-path rule (d): the loop picks the critical-path row, i.e. A. | Filed iteration 196 (fleet, unattended); design `design_docs/planned/w-transition-invocation-coordinator.md` § Round 2 carve-out; quorum artifact `.ailang/state/mission-quorum/w-transition-invocation-coordinator-2026-09-26T18-18-39Z.json`. |
<!-- decision-ledger:end -->
...
## STATUS 2026-09-26 (iteration 196) — **ROW 106's FIRST SLICE LANDED: WORLD NOW HAS AN INVOCATION COORDINATOR — PROPOSE → VERIFY → EXECUTE IN THE CAPSULE → COMMIT, WITH A STEP-1a IN-FLIGHT GUARD AND STEP-1b JOURNAL RECONCILIATION SO A RESENT TASK ID NEVER RE-EXECUTES — BUT WITH NO PRODUCTION CALLER. `/a2a/` STILL REFUSES. QUORUM r2 MADE ROW 23's POLICY TRANCHE (BOUNDED DURABLE OPERATIONS) A PREREQUISITE OF THE `/a2a/` WIRING, SO `D-WORLD-39` ASKS TO MOVE IT TO GROOM POSITION 2. JUDGED 95, ZERO BLOCKING [PRODUCT]** — **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives on `#140` since `2026-09-26T15:44:43Z` (of 29 comments); newest bookkeeping comment is iteration 195's own report, so no crash notice. Inbox: one `harness-resolved` reply from `mission-fleet` (`driver:slot-kill-leaves-orphan-descendants`, fleet PR #1325, `e3dadcd`). No queue row carries that `[HARNESS]` tag, so nothing was unparked; acked. Ledger 25 rows valid, ONE OPEN (`D-WORLD-38`). **GATE 1:** `dev` == `origin/dev` at `535330d`, check-runs 2/2 `success`. The resolved skill (V1 main checkout) is byte-identical to the fleet's `origin/dev` on all 13 files. **GATE 2 — CLAUSE MAP:** clause 1 MET · 2 MET · 3 MET · 7 MET · **4 UNMET**: row 93, blocked on capability (needs 106 + 108) · **5 UNMET**: row 114, waits for real operation · **6 UNMET**: 107 LANDED; **106 routable → PICKED (groom position 2)**; 108 BLOCKED, re-measured: the `serveapi/protocol` blob set hashes identically at `v0.33.2` and the latest tag `v0.44.1`, and `ailang#885` is OPEN. 0 open PRs; the only stale worktree is row 92's. **GATE 3 — full inner loop.** Designer rotation glm → **`pi:ollama/kimi-k3:cloud`** (probe rc=0) ran 629 s and 110 reads, then the Ollama Cloud **weekly limit** returned `429` ×4: verdict `empty_worktree` rc=10, **capacity, not model**. It fell through to the next rotation entry, **`claude:claude-opus-5-5`** (`claude-sub`, probe rc=0): doc + prototype M1–M4b in 1133 s, 30-row Verification Log, 26/26 mutations. The controller re-ran the prototype with pinned v0.41.0: vet 0, 24 ok / 0 FAIL. The doc measured two new facts at HEAD: the publish check stages under `AILANG_RELAX_MODULES=1` while the capsule runs strict (a module-named source **passes publication and fails invocation** with MOD010), and the capsule had no argument channel (the pinned interpreter's `--args-file` is one). **Quorum r1 BLOCKED 2/2** (glm and kimi seats ABSENT, unreachable on the same Ollama limit; reserve astra seated; Claude benched as author vendor). gemini claimed `Descriptor` has no skill ID: the controller measured it **FALSE** (`transitionreg.go:22` `ID`; prototype `plan.go:79`). astra said the durable tail is unbounded: measured **TRUE** (`store.Commit` `s.db.Begin()` with no ctx, `SetMaxOpenConns(1)`, `busy_timeout` 2000 ms bounds lock waits only). Revision (opus, resumed, 515 s): skill ID made explicit, the deadline claim weakened to steps 1–9, resend-by-task-id reconciliation added, R-106-11 → row 23. **r2 BLOCKED 2/0 on one surface**: gemini found a TOCTOU window between 1b and 10 (a concurrent retry double-executes); astra said reconciliation does not bound anything. Both carried concrete fixes and neither disputed direction → **narrow-refinement carve-out** `bf5e213`, fixes verbatim: step-1a in-flight tracker (R17), and **M1–M4c land without a production caller; M5/M6 blocked on row 23's policy tranche**. Planner **`codex:gpt-6-sol`** (resolver `opus fail-closed:planner-lane-field-missing` under a provider pin → routed to the pin), plan `0228250`: 6 milestones, every cumulative boundary vet + compile rc 0, R17 = immediate refusal with deferred release. Executor **`codex:gpt-6-sol`**: M1–M4c from the prototype plus R17; 45/45 mutations after it strengthened two tests its own first drill exposed. The controller rebuilt one commit per milestone from the snapshots, running vet, the compile fence and the package tests at each boundary. The final tree is **sha256-identical** (10/10 files). Full gates outside the sandbox: vet 0, **24 ok / 0 FAIL**, verify_ail PASS. **EVALUATOR `sonnet` (Agent tool, foreground, own worktree `.eval-world-iter196`): PASS 95/100, ZERO BLOCKING**; 11/11 of its own mutations killed, including a panic-path release. It confirmed no production importer of `host/coordinator`, and that `OpenBinder` narrows authority (TR.C count stays 3). Its non-blocking finding that reconciliation does not re-hash the objects it reads back was reproduced (`coordinator.go:163-188`) and filed as R-106-12. **GATE 3b:** PR [#152](https://github.com/sunholo-data/ailang-world/pull/152) head `254f4f4` 2/2 green, `MERGEABLE/CLEAN`; squash-merged **`a9a3382`**; the SHA-pinned poll on the MERGE shows 2 present / 2 completed / 2 success. **Metered $0.48 of $5** (quorum r1 $0.21, r2 $0.27). **CLAUSE 6 MOVED (capability), STILL UNMET:** the invocation path exists and is tested, but `/a2a/` is not wired to it until row 23's tranche and M5 land. **NEW ROW 115** (the pre-existing macOS capsule EPERM flake, 3/320 by the designer, not reproduced by the controller). **NEXT (critical path, default of `D-WORLD-39`):** 23 policy tranche → 106 M5/M6 → 108 when #885 ships → 93, 114, 94. **Decision ledger: 26 rows, TWO OPEN (`D-WORLD-38`, `D-WORLD-39`).**

## STATUS 2026-09-26 (iteration 195) — **ROW 107 LANDED: THE TRANSITION REGISTRY HAS A PRODUCTION WRITER. `world-publish transitions`, BEHIND THE ATTENDED-OPERATOR FENCE, PUBLISHES THROUGH `transitionreg.PublishSet`, WHICH REFUSES AN ABSENT SOURCE, A SOURCE THE PINNED INTERPRETER'S `check` REFUSES, AN EPOCH THE EPOCH REGISTRY DOES NOT NOMINATE FOR THAT DESCRIPTOR'S OWN INTERPRETER, AND A SAME-ID CAS-RETRY CONFLICT. A DAEMON TEST SHOWS THE A2A CARD LISTING A SKILL PUBLISHED THAT WAY. FIRST, DEV'S CI RED FROM THE 09-26 GROOM WAS FIXED. JUDGED 93 → 96, ZERO BLOCKING [PRODUCT]** — **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives on `#140` since `2026-09-26T07:13:40Z` (of 28 comments). The newest bookkeeping comment is the driver's lane notice for this fire (codex over daily ration → planner and executor `opus`). No `mission-world` inbox item newer than 09-14. Ledger 24 rows valid, ZERO OPEN. **GATE 1:** `dev` == `origin/dev` at `ad13c7b`, and **its check-runs were RED**: `go host build + test gate` failed at `queue_census.sh` F3. The census counted the attended groom's column-0 `1.`/`2.`/`3.` standing-rule list inside `## Queue` as duplicate rows 1–3 (lines 1723/1725/1729). Parent `9efe2f4` was 2/2 green. The fix is format-only, relabelled `(a)/(b)/(c)` with the wording unchanged: **`0aa53e6`**, census rc=0 (controls 1/79), CI **2/2 green**. Running-skill drift: the resolved symlink target (V1 main checkout, 21 behind) DIFFERS from the fleet's `origin/dev` on `SKILL.md`, `gate-2-pick.md` and `gate-5-retro.md`. The delta is `541e220a1`, the attended Gate-2 CRITICAL-PATH CHECK and Gate-5 drift alarm. I read and followed the ORIGIN version, because the regroom this iteration serves was written against it. **GATE 2 — CLAUSE MAP:** clause 1 MET · clause 2 MET · clause 3 MET · clause 7 MET · **clause 4 UNMET**: row 93, blocked on capability (needs 106 + 108; 107 now landed) · **clause 5 UNMET**: row 114, waits for real operation to produce an unseeded question · **clause 6 UNMET**: **107 routable → PICKED (groom position 1)**; 106 routable (position 2); 108 BLOCKED upstream, re-measured: the `serveapi/protocol` blob SHAs are identical at `v0.33.2` and the latest tag `v0.44.0`, and `ailang#885` is OPEN. Premise re-measured at HEAD: `Publish`/`BuildNext` have 0 non-test callers (control: 26 test references). 0 open PRs. The only stale worktree is row 92's, landed long ago. **GATE 3 — full inner loop.** Designer **`pi:ollama/glm-5.3:cloud`** (rotation astra → glm; probe rc=0; `mission_pi_run.sh` verdict `ok`, 812 s) wrote doc `267e871` with a 7/7 prototype. **Quorum r1 BLOCKED 3/3**, all present (author vendor Z-AI benched; astra, gemini, kimi seated): astra and gemini said the operator-typed `SemanticsEpoch` must derive from the epoch registry; kimi said the source check was presence-only and the same-ID retry merge was unspecified. The controller measured both premises before routing: the epoch registry maps epochs to release strings, and `canon.Source` only normalises. One glm revision `0759ee7` (1796 s) derived the epoch inside `PublishSet`, added a bounded `ailang check` gate and a typed same-ID conflict: 12/12 killed, and the controller's own `verifyEpochs → nil` mutation reds 4 tests. **r2 BLOCKED 2/1, gemini PASS.** astra: release nomination is advisory, so the doc overclaimed. kimi: "the capsule could resolve" contradicted F18, and no landed `.ail` source is publishable. → **narrow-refinement carve-out** `c91b5c9`: kimi's fixes verbatim, astra's claim-weakening verbatim, and its two-interpreter test adopted as AC-EPOCH-TWIN. astra's authorized HashRef↔epoch binding record was **refuted by ratified D1** (epoch = compatibility metadata). **astra re-run alone ($0.22):** it no longer contested the epoch, and it rejected a new overclaim ("guaranteed capsule-executable", from kimi's verbatim text). Its claim-weakening was applied verbatim in `b2fb92e`; the invocation contract is Residual 8, owned by row 106. Planner **`opus`** (`derive-planner-lane: opus fail-closed:env-pin`): plan `5dd977b`, 7 milestones reordered to compile order. It found a real platform defect: `CheckSource` refused every module-declaring source on macOS (MOD010 under `/private/var`), not on Linux. The controller reproduced it (strict rc=1 / relax rc=0 / garbage still rc=1) and accepted the one-line `AILANG_RELAX_MODULES=1`. AC-EPOCH-TWIN measured load-bearing (its mutation survived the prior suite). Executor **`opus`**: M1–M7 plus bookkeeping, 8 commits, **19/19** mutations killed, final **23 ok / 0 FAIL**, verify_ail rc 0. The controller re-ran the same. **EVALUATOR `sonnet` (own worktree `.eval-world-iter195`): PASS 93/100, ZERO BLOCKING**; 5 of its own 6 mutations killed. The survivor was a per-descriptor epoch lookup keyed on the first descriptor's interpreter. The controller closed it with test-only `a5b748d`, the sole killer (with `-skip`, the same mutation is green in all 3 packages). **r2 judge (resumed): PASS 96/100**, survivor now KILLED. Verdicts on the judge's four questions: the store opens before the fence, as `runPublish` already does, so it is consistent; `AILANG_RELAX_MODULES` is narrow; the claim strength matches the code; the daemon test uses the production path. **GATE 3b:** PR [#151](https://github.com/sunholo-data/ailang-world/pull/151) head `a5b748d` 2/2 green, `MERGEABLE/CLEAN`; squash-merged **`b7cfbdb`**; the SHA-pinned poll on the MERGE shows 2 present / 2 completed / 2 success. **Metered $0.91 of $5** (quorum r1 $0.28, r2 $0.41, astra re-run $0.22; glm on the ollama subscription $0). **CLAUSE 6 MOVED, STILL UNMET:** the registry now has a production writer, but **no landed `.ail` source can be published through it yet** (Residual 7: `world/transitions.ail` imports `world/*`, refused hermetically by LDR001). The first real card content needs a self-contained transition module or row 106 widening capsule staging. **NEXT (critical path):** 106 (position 2), then 108 when #885 ships, then 93, 114, 94. **Decision ledger: 25 rows, ONE OPEN (`D-WORLD-38`, one word, A recommended, shipped as default).**

## STATUS 2026-09-26 (iteration 194) — **ROW 23's PLUMBING TRANCHE LANDED: ALL 11 FORMERLY DEADLINE-FREE STORE READS (APPROVE 8, REGISTRY 2, REPLAY 1) NOW INHERIT THE CALLER'S CONTEXT, THE RATCHET IS AN EMPTY MAP, AND A NEW ROOT CENSUS PINS THE 15 REMAINING NAMED `context.Background()` ROOTS SO A HOIST CANNOT FAKE THE ZERO. NO DEADLINE VALUE, NO STORE-BOUNDARY GUARD, NO `Commit` CHANGE: THOSE ARE D-WORLD-18's POLICY QUESTIONS AND GO TO MARK AS `D-WORLD-37`. JUDGED 98, ZERO BLOCKING [PRODUCT]** — **GATE 0:** kill switch armed; `gh` = `sunholo-voight-kampff`; billing **CLEAN**; **0** `MarkEdmondson1234` directives on `#140` since `2026-09-26T02:45:45Z` (of 26 comments); self-notice read `A FIRE DIED (newest rc=143 at 2026-09-26T02:45:45Z on #140)` = orphan 192, already credited by iteration 193 (0 open PRs; only stale worktree is row 92's); ledger 23 rows valid, ZERO OPEN. **GATE 1:** `dev` == `origin/dev` at `b5b4a7d`; check-runs 2/2 `success`; the running skill is byte-identical to the fleet's `origin/dev` on all 13 files (resolved symlink target). **GATE 2:** groom position 9, **row 23**, premise re-measured at HEAD (8+2+1 literal sites present). D-WORLD-18 says the approval-bound and `Commit` questions return to Mark before the follow-on routes, so the pick was scoped by measurement: `HumanHandler.Execute` DISCARDED the context it receives and the approval protocol is request/poll, so no store read waits on a human. Threading the caller's existing context decides no policy; a deadline value, the guard and `Commit` do. **GATE 3 — full inner loop.** Designer **`codex:gpt-6-astra`** (rotation claude → astra; probe rc=0) prototyped all 11 sites plus `GetVerifyResult`: doc `601bb1b`, 29/29 mutations killed. **Quorum r1 BLOCKED 3/3**, all present (glm: scanner branches unwitnessed; kimi: the scan domain was never shown to cover all production Go, and the controller measured it TRUE, 2 nested-module reproducers outside host/cmd; sonnet: a `/tmp` citation) → one revision `27815ba`, 35/35. **r2 BLOCKED 2/1, glm PASS** (kimi: dot-import and allow-list-existence rules unwitnessed; sonnet: the controller's out-of-sandbox run had no row of its own), neither disputing direction and both with concrete fixes → **narrow-refinement carve-out** `c01fb31`: controller ran M36/M37 (both killed) and added Vtest2 and Vrepro, 37/37. Planner **`codex:gpt-6-sol`** (resolver `opus fail-closed:planner-lane-field-missing` under a provider pin → routed to the pin): plan `f00dcd0`, prototype unmodified. Executor **`codex:gpt-6-sol`** applied the hunks plus 7 API-note comment lines. Its M3 red was the new `TestProductionGoSurface` correctly flagging the executor's own `.snap/` Go copies. The controller rebuilt one commit per milestone outside the sandbox: **M1 `f7213ea`** (pins 11→3; broker, world-publish and store ok), **M2 `7fe44b9`** (pins → empty; daemon, registry, replay, store and broker ok), **M3 `e46e238`** (vet 0, **23 ok / 0 FAIL**, verify_ail PASS); the final tree is sha256-identical to the M3 snapshot. **EVALUATOR `sonnet` (own worktree `.eval-world-iter194`): PASS 98/100, ZERO BLOCKING**; 9/9 of its own mutations killed, milestone-level reverts included. Its one non-blocking finding (the design's +67/-46 figure predates M3's +7 comment lines) was reproduced (+74/-46) and corrected in `5f298e7`. **GATE 3b:** PR [#150](https://github.com/sunholo-data/ailang-world/pull/150) head `5f298e7` 2/2 green, `MERGEABLE/CLEAN`; squash-merged **`b9ecabe`**; the SHA-pinned poll on the MERGE shows 2 present / 2 completed / 2 success, plus 1 push run `success`. **Metered $0.45 of $5** (quorum only; codex is subscription). **UNBLOCKED:** row 26 (its row-23 gate, 11 → 0 with an empty pin, is met) and row 111 arm A (sequenced after this threading). **ROW 23 STAYS OPEN** as the owner of the policy tranche. **NEXT:** 25, 26, 32, 112, 111, 106, 107, 109, 113, 99, 100. **Decision ledger: 24 rows, ONE OPEN (`D-WORLD-37`, one word, A recommended).**
...
- **Baseline isolation applies to every role permitted to mutate, including the designer** (iter161). Run pristine acceptance baselines in a separate sibling worktree. A designer reproduced a temporary test mutation while the controller baseline used its tree; that reading was discarded and the full Go suite rerun on an isolated pristine tree (rc0). A later restore does not retroactively make an overlapping measurement pristine.
...
| # | Row | Why here |
|---|---|---|
| 1 | ~~**107**~~ `w-transition-registry-production-publisher` (clause-6) — **LANDED 2026-09-26 (iter-195), `b7cfbdb`**; no landed `.ail` source publishable yet (Residual 7) | Gated on nothing, ~1d. Without it the A2A card lists zero skills and clause 6's "the transition registry is served" has no content |
| 2 | **106** `w-transition-invocation-coordinator` (clause-6, clause-3) — **M1–M4c LANDED 2026-09-26 (iter-196), `a9a3382`, no production caller; M5/M6 BLOCKED on row 23's policy tranche (`D-WORLD-39` proposes moving it here, default A)** | Gated on nothing, ~2d. Without it `/a2a/` can only refuse and MCP `tools/call` has nothing to dispatch into. It is also the World arm of row 93 |
...
| 7 | **23** policy tranche (`D-WORLD-37` RULED A), then 25, 26, 32, 109, 111, 112, 113, 115 (clause-2) — **the policy tranche now gates row 106's M5; `D-WORLD-39` proposes moving it to position 2** | Real daemon robustness, but it does not move the bar. Loop work only when positions 1–6 are all blocked or landed |
...
| 2 | **39** `w-session-authority` (clause-3) | **LANDED 2026-09-24 (iter-181): PR #141 squash-merged `a036062`, CI green on the merge; judged 95/100 (iter-180) + 97/100 + r2 100/100 (iter-181), zero blocking; landed per the attended `D-WORLD-36` ruling after its harness fix. Original why: the repo has **no inbound credential → session resolution at all**. Hard 1.0 blocker, and row 40 depends on it.** |
| 3 | **92** (new, clause-5) | The value demonstration. This is what World is FOR |
| ~~4~~ | ~~**34**, 35, 38~~ (clause-5) | **34 LANDED 2026-09-24 (iter-185): PR #144 squash `158d8ee`, CI green on the merge; judged 95/100 + r2 98/100, zero blocking — closed grammar, href guard and verdict line pinned (test-only). POSITION 4 COMPLETE.** **35+38 LANDED 2026-09-24 (iter-184): PR #143 squash `9574d08`, CI green on the merge; judged 95/100 + r2 100/100, zero blocking — paged timeline, selected entry with existence-checked edges.** **Same-surface follow-ons 98+102 LANDED 2026-09-25 (iter-186): PR #145 squash `909f27f` — object page's provenance walk and grade reason never blank.** Original why: the workbench human surface the demonstration runs through |
| ~~5~~ | ~~**40** `w-a2a-session-projection` (clause-6)~~ | **CARD HALF LANDED 2026-09-25 (iter-187): PR #146 squash `8bb8502`, CI green on the merge; judged 93 + r2 98, zero blocking. Invocation split to rows 106 (coordinator) + 107 (registry publisher); the MCP child is row 108.** |
| ~~6~~ | ~~**27** `w-interface-hash-does-not-cover-the-interface` (clause-1)~~ | **LANDED 2026-09-25 (iter-188): PR #147 squash `73e06ae`, CI green on the merge; judged 97/100, zero blocking. `interfaceHashV2` (upstream v2) now travels beside the manifest-coverage `interfaceHash`; reconcile compares four digests.** Original why: kernel correctness: the field whose NAME promises interface coverage has manifest coverage |
| 7 | **93** (new, clause-4) | The non-inferiority floor run. **BLOCKED ON CAPABILITY, NOT ORDERING, as of 2026-09-25 (iter-189, measured).** The clause-4 World arm is "MCP transition tools only", and World has no MCP surface. `grep -rln "tools/call\|tools/list\|\"mcp\"\|/mcp" host cmd`, non-test, returns **0** files, while the `a2a` control returns 3. It also has no invocation path (row **106**) and no populated transition registry (row **107**). The MCP half is row **108**, which is blocked upstream on `ailang#885`: reopened 2026-09-25, and `serveapi/protocol` blobs are identical at v0.42.0, the latest release. Re-enters at this position when 106 + 107 + 108 land. The shell-arm eligibility precondition could run alone, but it measures nothing until the World arm exists (the doc's own §5 logic) |
| ~~8~~ | ~~**75** (clause-2)~~ | **CLOSED 2026-09-25 (iter-189): SOLVED UPSTREAM.** `ailang#1037` was closed COMPLETED on 2026-09-13. On the pinned **v0.41.0**, `messages list zzz_… -json -limit 2` now exits **rc=1** (`unexpected argument … takes only flags; the inbox goes in --inbox`). The `-inbox` flag-form control gives rc=0. Original why: A real product CLI bug with a measured attribution hazard: a filtered query returns a strict superset **indistinguishable from a correct answer**. Admissible under Gate 2 on those grounds |
| 9 | ~~**22**~~, ~~**23**~~ (plumbing), ~~**24**~~, **25, 26, 32** (23's plumbing **LANDED 2026-09-26, iter-194**: PR #150 squash `b9ecabe`, CI green on the merge; judged 98, zero blocking; its policy tail is routable — `D-WORLD-37` RULED A (attended 2026-09-26) — and rows 26 and 111-arm-A are unblocked. 22 **LANDED 2026-09-25, iter-189**: PR #148 squash `e34416f`, CI green on the merge; judged 97, r2 99, zero blocking; successor row **111**. 24 **LANDED 2026-09-26, iter-193**: PR #149 squash `24df816`, CI green on the merge; judged 98, zero blocking; successor rows **112, 113**) | Genuine daemon robustness — the rows clause-2 is actually about. These ARE World product and remain loop work |
...
- Mission-harness + bookkeeping: **69, 76, 77, 78, 83, 84, 85, 86, 88, 89, 90, 91, 101**
...
95. **w-prepush-gate-blocks-every-land-from-pi-sessions-in-this-repo** · clause-2 (mission-harness) · **THE FLEET'S PREPUSH-GATE PI EXTENSION IS UNPASSABLE BY CONSTRUCTION IN THIS REPO, AND IT BLOCKS EVERY PUSH AND PR — THE EXPOSURE BEGAN THE DAY THE CONTROLLER LANE FELL TO PI (2026-09-21) AND FOUR SLOTS DIED BEFORE ONE SURVIVED TO REACH THE PUSH.** Measured first-party 2026-09-23 (iter-180): the extension (`~/.pi/agent/extensions/prepush-gate.ts`) hooks every bash tool call matching a push or PR-create pattern and runs ailang's gates first — `gofmt -l cmd/ internal/`, `make lint`, `make check-file-sizes`. Its repo predicate is "has tracked Go under `cmd/` or `internal/`": TRUE here (`cmd/ailang-worldd`) — but this repo has NO Makefile, so `make lint` fails `No rule to make target 'lint'` and the gate can never pass. The header's documented escape hatch `AILANG_SKIP_PREPUSH=1` is **NOT IMPLEMENTED in the code** — measured: exported, the land attempt refused identically (doc/code drift in the harness). The extension's own comments record the identical harm class in ailang-packages: an agent with correct, committed work "spent its whole remaining budget failing to push". Facets measured this fire: (a) the unimplemented escape hatch; (b) the gate also refuses any bash call that merely QUOTES the pattern in prose — appending THIS mission's own log entry was refused because the entry's text names the blocked commands (it cannot tell a record from a command; the record went in through the write tool); (c) it fires only in pi sessions, so claude/codex controller lanes never saw it. **NOT loop work** (2026-09-21 admissibility rule: harness work is ATTENDED-ONLY, and the file lives in the fleet's home dir, not this repo) — filed here as the escalation record, blocking the LAND of row 39 (`D-WORLD-36` carries the ask). · `escalate-not-fix` obeyed: no tool-boundary evasion attempted · **filed 2026-09-23 (iter-180). RESOLVED 2026-09-23 attended (per the `D-WORLD-36` ruling, option B) and VERIFIED LIVE 2026-09-24 (iter-181): the extension was fixed at source in ailang `754fc7a43` — make gates now run only when the repo's Makefile defines the target (ailang-world: gofmt only), and `AILANG_SKIP_PREPUSH` is implemented; this repo's first pi-controller push + branch push + PR-open + merge since 2026-09-21 all passed the gate.**

92. **[LANDED 2026-09-24 (iter-183) — PR #142 squash `157d6f9`, remote CI green on the merge (Gate 3b SHA-pinned); judged PASS 94/100 zero blocking (sonnet, own worktree); artifact `design_docs/verification/w-1-0-value-demonstration.md` — 4/4 PASS at 10–20 s, baselines UNMEASURED, the walk measures retrieval of a recorded diagnosis (stated in the artifact); M1–M3 by orphan 182, M4 by opus]** **w-provenance-teeth-value-demonstration** · clause-5 · **THE 1.0 VALUE PROOF HAS NEVER
...
96. **w-object-lookup-by-semantic-id** · clause-5 · **A PROVENANCE WALK CANNOT FIND AN OBJECT BY NAME — IT MUST LINEAR-SCAN THE LOG.** Measured by row 92's walk (iter-183, finding F-1 of `design_docs/verification/w-1-0-value-demonstration.md`): `GET /v1/objects/{ref}` accepts a hashref only, and `POST /v1/commit` cannot register names in `epoch_registry_heads` (writers: `registry.Bootstrap`, broker approve, transitionreg — F-2), so every walk located its incident by `log range --from 0` plus one `object get` per entry until the semanticId matched. Cheap at 4 entries, which is all row 92 proved; at mission scale it threatens clause 5's ≤5-minute bar. **THE ITEM:** a read route or index that resolves a semanticId (e.g. `GET /v1/objects/by-semantic-id/{name...}`), with the walk re-timed on a log of realistic size. · ~1d · gated on nothing · filed iter-183 from a measured finding.

97. **w-commit-content-mismatch-is-a-client-error** · clause-3 · **A CLIENT-SUPPLIED CONTENT-HASH MISMATCH ON `POST /v1/commit` RETURNS HTTP 500 `{"class":"Internal","message":"internal store failure"}`, NOT A 4xx.** Measured by the iter-183 controller mutation MU-3 (one base64 character flipped in a fixture's incident payload): the store's content verification (`host/store/store.go` PutObject) correctly refuses the object, but the handler classifies a bad hash from the client as an internal failure — so a caller cannot tell its own malformed commit from a daemon fault, and an operator sees a 500 for a client bug. Pre-existing at base (row 92 changed no daemon code). **THE ITEM:** map the store's content-address violation to a 4xx with a typed error class, plus a handler test that sends a mismatched hash and asserts the status and class. · ~0.3d · gated on nothing · filed iter-183 (finding F-3 of the row-92 artifact).

98. **[LANDED 2026-09-25 (iter-186) — PR #145 squash `909f27f`, remote CI green on the merge (Gate 3b SHA-pinned); designed together with its sibling in `design_docs/implemented/w-workbench-object-provenance-and-grade.md`; judged 97/100 + r2 100/100, zero blocking]** **w-workbench-provenance-walk-section-is-always-blank** · clause-5 · **THE WORKBENCH'S "PROVENANCE WALK" SECTION RENDERS NOTHING IN PRODUCTION: `ObjectView.Edges` HAS 0 WRITERS IN `host/daemon`.** Measured by the row-35+38 designer (iter-184, doc §9 R-b, V4): `grep -rn --include='*.go' Edges host/daemon | wc -l` → 0, with the control `StateRoot: workbench.EdgeView` → 1. The template's `{{range .Edges}}` therefore iterates nothing, and the section shows BLANK rather than an explicit `UNAVAILABLE`. The original workbench design (§2.7) explains why the data does not exist yet (there is no typed provenance projection). The defect is the silent blank. After row 35+38 the selected entry's edges are the page's only working provenance hop. **THE ITEM:** either project an object's provenance edges from what the store records, or render an explicit UNAVAILABLE naming why. The design must choose. · ~0.5d · NEEDS A DESIGN DOC · filed iter-184.

99. **w-workbench-world-stateroot-link-is-unchecked** · clause-5 · **THE WORLD PANE EMITS ITS `StateRoot` EDGE AS `Available: true` WITHOUT CHECKING THE OBJECT EXISTS, AND THE LINK 404s.** Measured by the row-35+38 designer (iter-184, doc §9 R-c, V15, throwaway probe deleted): on `/workbench` and `?from=0&entry=0` the StateRoot href is `/workbench?object=sha256:ad312da4…`, `GetObject` → ok=false, and fetching it → **404**. The same pattern as the entry edges row 35+38 just fixed (F6); `host/daemon/workbench.go` world-pane construction. **THE ITEM:** existence-check the StateRoot target exactly as `entryEdges` does (stored → link, missing → UNAVAILABLE, store error → 5xx). Then widen `TestWorkbenchTimelinePaging/emitted-links-resolve` to include the world pane. · ~0.3d · gated on nothing · filed iter-184.

100. **w-workbench-more-fields-render-nowhere** · clause-5 · **FOUR MORE VIEW-MODEL FIELDS HAVE ZERO TEMPLATE ACTIONS:** `TimelineView.From`/`Limit`, `WorldView.Revision`/`LogHead` (doc §9 R-a, V1). `From`/`Limit` are the explicit exemptions in the new `TestWorkbenchViewFieldsAllRender`. `WorldView` and `ObjectView` are outside that test's struct set. Same class as row 35. **THE ITEM:** render or delete each, and extend the all-fields-render test to `WorldView`/`ObjectView`. · ~0.3d · gated on nothing · filed iter-184.

101. **w-gofmt-drift-with-no-ci-gate** · clause-2 (MAINTENANCE — attended-only per the 2026-09-21 rule) · **`gofmt -l host cmd` IS RED AT BASE ON 3 FILES AND NO CI STEP CHECKS IT.** Measured by the row-35+38 planner, re-measured by the controller at `0a69f1a`: `host/daemon/session_middleware_test.go`, `host/store/schema_version_test.go`, `host/store/store.go`. `grep -n gofmt .github/workflows/ci.yml` at `origin/dev` → nothing. The fleet prepush-gate runs `gofmt -l cmd/ internal/`, which does not cover `host/` (iter-181 hit the same gap). Filed as the escalation record, not loop work. · filed iter-184.

102. **[LANDED 2026-09-25 (iter-186) — PR #145 squash `909f27f`, remote CI green on the merge (Gate 3b SHA-pinned); designed together with its sibling in `design_docs/implemented/w-workbench-object-provenance-and-grade.md`; judged 97/100 + r2 100/100, zero blocking]** **w-workbench-grade-is-never-supplied** · clause-5 · **EVERY OBJECT PAGE RENDERS `GRADE UNAVAILABLE — ` WITH AN EMPTY REASON: THE DAEMON NEVER SETS `ObjectView.Grade`.** Measured by the row-34 designer (iter-185, doc §9 R-a, V12 — daemon probe `GET /workbench?object=<stored>` → 200 with grade text `GRADE UNAVAILABLE — </p>`) and re-measured by the controller at `158d8ee`: `grep -l Grade host/daemon/*.go` excluding tests → **0** files; same-file control `PayloadTruncated` in `host/daemon/workbench.go` → **1**. Same view-model-field class as rows 35/38/100, and it is the honest-UNAVAILABLE half of clause 5: the page never says *why* a grade is unavailable. Row 34 pinned the template for a SUPPLIED reason (V9 killer `TestRenderGradeWithoutVerdictClaim/unavailable`), so the render side is ready; the handler side is the gap. **THE ITEM:** decide what the daemon can honestly supply per object kind (a PROVEN/TESTED/ATTESTED/CLAIMED projection where one exists, else `NewGradeUnavailable(<named reason>)`), and pin it. · ~0.5 d · NEEDS A DESIGN DOC · queued iter-185 (designer-found, controller-reproduced, control firing).

103. **w-store-object-reference-index** · clause-5 · **THE OBJECT PAGE'S `referencedBy` RELATION IS A NAMED STOP BECAUSE NO STORE INDEX MAPS AN OBJECT TO THE LOG ENTRIES OR WORLDS THAT REFERENCE IT.** Measured by the rows-98+102 designer (iter-186, doc §9 P-a, V5/V6) and re-verified by the r1 judge at `e1c07c5`: the schema declares **0** indexes over 9 tables; `log_entries.transition_ref`/`transition_fn_ref`/`interpreter_ref` and `worlds.state_root` are unindexed; the daemon's `readStore` seam has 5 point reads and no reverse read. **THE ITEM:** add the indexes plus a bounded reverse read ("entries/worlds referencing X, limit N") and replace the `referencedBy` stop with existence-checked edges. Schema + seam change. · ~1d · NEEDS A DESIGN DOC · filed iter-186.

104. **w-store-commit-object-membership** · clause-5 · **THE OBJECT PAGE'S `committedBy` RELATION IS A NAMED STOP BECAUSE THE STORE RECORDS NO LINK FROM AN OBJECT TO THE COMMIT THAT INSERTED IT.** Measured by the rows-98+102 designer (iter-186, doc §9 P-b, V4): `objects` has 5 columns, none relating it to a commit or entry; `Store.Commit` inserts `Objects` in the same transaction as the entry but records no link; `provenance` is a free-text label, not a reference. **THE ITEM:** record commit membership (schema change) and project it as a checked edge. · ~1d · NEEDS A DESIGN DOC · filed iter-186.

105. **w-workbench-grade-projection** · clause-5 · **[PARKED behind item 13's validated-boundary follow-on]** · **EVERY OBJECT PAGE NOW SHOWS ONE CONSTANT, TRUE GRADE REASON ("no canonical host projection: this workbench handler does not resolve subject-bound evidence into an object grade"); A REAL GRADE NEEDS STORED, DECODED EVIDENCE BOUND TO A SUBJECT AND A DAEMON-HELD VALIDATOR.** Measured by the rows-98+102 designer (iter-186, doc §9 P-c, F5/V10/V12/V20): 0 non-test importers of `host/evidence`; `Validator` needs a key, compiler config and required identities the daemon does not hold. Filed so the constant reads as a waypoint, not the destination. · filed iter-186.

106. **[M1–M4c LANDED 2026-09-26 (iter-196) — PR #152 squash `a9a3382`, remote CI green on the merge (Gate 3b SHA-pinned); judged PASS 95/100 zero blocking (sonnet, own worktree, 11/11 own mutations killed). `host/coordinator` (step-1a in-flight guard R17, step-1b journal reconciliation, propose → verify → execute → commit), `capsule.RunContext` (`--args-file`, caller ctx, `AILANG_RELAX_MODULES=1` publish-check parity — closes row 107's MOD010 check/run divergence) and `broker.OpenBinder` (bind-only, TR.C held) exist with NO production caller; `/a2a/` still refuses -32603. **M5 (`/a2a/` wiring) and M6 (end-to-end + QUICKSTART) are BLOCKED on row 23's policy tranche** (quorum r2 astra scope-cut, verbatim); `D-WORLD-39` proposes moving that tranche to groom position 2. RESIDUAL R-106-12 (judge finding, reproduced by the controller): step-1b's `committed()` (`host/coordinator/coordinator.go:163-188`) reads the committed record/output objects back without re-hashing them, asymmetric with R6's source re-hash — fold into M5. Design + plan stay in `planned/` until M6.]** **w-transition-invocation-coordinator** · clause-6 (and clause 3) · **WORLD HAS NO SERVER-SIDE PATH THAT INVOKES A REGISTERED TRANSITION, SO THE A2A `/a2a/` ENDPOINT CAN ONLY REFUSE (CONSTANT -32603) AND MCP `tools/call` HAS NOTHING TO DISPATCH INTO.** Measured iter-187 at `a8e12fd` (design doc F7, controller first-party): `grep -rniE "func .*(propose|coordinat)" host cmd` (non-test) → **0**; `host/transitionreg` had **0** production importers (control: `host/broker/invoke_boundary_test.go`); `host/capsule` and `host/replay`, which run AILANG transition source, have no production callers outside themselves; the daemon's only mutation is `POST /v1/commit`, which takes an already-computed commit. **THE ITEM:** design the propose → verify → commit coordinator that composes `transitionreg.Bind` + the capsule runner + a broker session + `store.Commit` under one bounded context, then wire `/a2a/` `tasks/send` for authorized skills to it (replacing the constant -32603). The deferred material is preserved verbatim in [`implemented/w-a2a-session-projection.md`](implemented/w-a2a-session-projection.md) § "Deferred to invocation" (P4's dispatch half, `Invoker`, AC13, socket closure). · ~2d, likely multi-milestone · NEEDS A DESIGN DOC · gated on nothing · filed iter-187.

107. **[LANDED 2026-09-26 (iter-195) — PR #151 squash `b7cfbdb`, remote CI green on the merge (Gate 3b SHA-pinned); judged PASS 93 → r2 96/100, zero blocking (sonnet, own worktree). `world-publish transitions` → `transitionreg.PublishSet` (presence + standalone `check` under the pinned interpreter + advisory epoch nomination per descriptor interpreter + typed same-ID CAS conflict); daemon test proves the card lists a skill published through the production path. Doc `implemented/w-transition-registry-production-publisher.md`. RESIDUAL 7: no landed `.ail` source is publishable yet (imports `world/*`, LDR001) → first real card content needs a self-contained module or row 106's staging. RESIDUAL 8: invocation contract → row 106. `D-WORLD-38` (phrase) OPEN, default A shipped.]** **w-transition-registry-production-publisher** · clause-6 · **THE TRANSITION REGISTRY IS NEVER POPULATED IN PRODUCTION, SO A LIVE DAEMON'S A2A CARD ALWAYS LISTS ZERO SKILLS.** Measured iter-187 (design doc F8): `transitionreg.StoreReader.Publish` and `BuildNext` have **0** non-test callers (control: 23 test references to `store.TransitionRegistryV1`); `cmd/world-publish` publishes packages through `broker.EffectRegistryPublish`, a different registry. Row 40's card handles the absent head honestly (200, zero skills — tested), but clause 6's "the transition registry is served" needs content. **THE ITEM:** a production path (a CLI verb, or registration from `world/*.ail` exports that honours P3's "not a startup copy") that publishes transition revisions via the landed `Publish`/CAS, with a daemon-level test that the card then lists them. · ~1d · NEEDS A DESIGN DOC · gated on nothing · filed iter-187.

108. **w-mcp-dispatch-projection** · clause-6 · **[BLOCKED on UPSTREAM — `sunholo-data/ailang#885` was CLOSED 2026-09-13 AS "DELIVERED" BUT NOTHING SHIPPED; RE-OPEN REQUESTED iter-187]** · The MCP half of clause 6, child #1 of `w-mcp-projection` ([`planned/w-mcp-dispatch-projection.md`](planned/w-mcp-dispatch-projection.md)), until now tracked only inside row 5's prose. **Predicate, measured iter-187:** all five files in `serveapi/protocol` have byte-identical blob SHAs at `v0.33.2` and `v0.42.0` (`gh api repos/sunholo-data/ailang/contents/serveapi/protocol?ref=<tag> --jq '.[].sha'`), so no SDK-free JSON-RPC method dispatch exists; MCP dispatch still lives only in `serveapi/mcp_handler.go` behind `modelcontextprotocol/go-sdk`. Measurement posted on #885 (comment 5825242979) and sent to `mission-control` (`inbox_1790300220936_68a23ac6`). **Second obligation, measured:** `protocol.CallerSurface` enforces `^[a-zA-Z0-9_-]{1,64}$`, which rejects World stable IDs (`tools.echo` and `world/recovery-transition/v1` refused; `effect-paged` accepted as the control), so the MCP surface needs a naming mapping or an upstream grammar change before `tools/list` can equal the A2A `skills[].id` set. **Re-check the predicate at each Gate 1** (a new upstream tag whose `serveapi/protocol` blobs differ). · ~1d after unblock · filed iter-187.

109. **w-pkgproj-crosscheck-has-no-in-process-bound** · clause-2 · **THE LAST PRODUCTION SUBPROCESS IN THIS REPO LAUNCHED WITHOUT A CONTEXT.** Measured iter-188 at `1a1d929`: `git grep -n "exec.CommandContext\|exec.Command(" -- '*.go' | grep -v _test` → 4 of 5 production launch sites are `CommandContext`-bounded (`host/archive/archive.go:470`, `host/broker/handlers.go:93`, `host/capsule/capsule.go:169`, `host/replay/replay.go:327`); the fifth, `host/pkgproj/pkgproj.go` `CrossCheck` (`ailang publish --dry-run`), is bare `exec.Command` and relies solely on step 7's shell `run_bounded 120`. Row 27 bounded its own new sibling (`QueryInterface`: named 30 s timeout, `WaitDelay`, capped output, overflow cancels) and deliberately left this one alone because it pre-dates the row. **THE ITEM:** give `CrossCheck` the same in-process bound, reconciling the tension its own comment records ("a new caller must bring its own bound" vs. no cross-language constant ordering); reuse `QueryInterface`'s pattern; hang/descendant/overflow doubles as in row 27's T14–T16/T20. Descendant cleanup stays row 24's. · ~0.3d · gated on nothing · **filed iter-188 from the row-27 design's §11 O5.**

110. **w-ready-packet-golden-contenthash-drifts-from-the-published-0-1-0** · clause-7 · **THE GOLDEN IS LABELLED 0.1.0, BUT A TIER-1 FLOOR RAISE MOVES ITS `contentHash` AWAY FROM THE BYTES THE REGISTRY SERVES FOR 0.1.0.** Surfaced by the row-27 designer (design §11 O4); pre-existing and orthogonal to row 27, which added `interfaceHashV2` precisely because v2 does NOT move on a contract-only or body-only raise (measured V6). **THE ITEM:** measure it first — diff the committed golden's `contentHash`/`tarballSHA256` against the served `world/core@0.1.0` metadata (`0c8c6061…`/`44fc9fab…`) at HEAD and at each S8 floor-raise commit — then decide whether the golden should describe "the next publishable tree" (and be relabelled) or "the published 0.1.0" (and floor raises must bump a version). The decision may be ratification-class; price the arms before asking. · ~0.3d · gated on nothing · **filed iter-188.**

111. **[ARM A's SEQUENCING GATE MET 2026-09-26 (iter-194): row 23's context threading landed at `b9ecabe`.]** **w-daemon-lock-blocked-read-status-and-deadline-enforcement** · clause-2 · **A LOCK-BLOCKED DAEMON READ ANSWERS 500 INTERNAL AT THE BUSY WINDOW, NOT 503 AT THE DEADLINE, AND RESIDUAL (i)'S NAMED OWNER `w-daemon-late-read-503` HAS NO ROW AND NO DOC.** Surfaced by row 22's design (iter-189, O1). Measured through the shipped handler: with `readDeadline` 10 s, `BusyTimeout()` 2 s and a raw `BEGIN EXCLUSIVE` holder, `GET /v1/head` returns `500 {"class":"Internal"}` in 2.047 s. `timedOut` (`handlers.go:310-315`) is false for `SQLITE_BUSY` under a live context. Separately, `grep -nE '^[0-9]+\. \*\*w-daemon-late-read'` finds no row, while the control matches row 22. **THE ITEM:** own (i) the late-completing read that answers 200 after its deadline (`LIMITATION(w-daemon-late-read-503)`), (ii) the 500-vs-503 classification of a lock-blocked read, and optionally (iii) arm A, per-request `busy_timeout` capping. Arm A was measured feasible in row 22's doc §2, but a skipped restore poisons the writer's only connection (pool reads 37 against a configured 2000), so it must be sequenced after row 23. Also fix F1, the overclaim at `daemon.go:505-507` ("the same proven bound applies" for the projection's `MaxWait`). · ~1d · NEEDS A DESIGN DOC · gated on row 23 for arm A only · filed iter-189.
...
115. **w-capsule-overflow-kill-eperm-flake** · clause-2 · **`host/capsule` `TestOrdinaryOverflowCarriesNoESRCH` FAILS ~1% ON macOS WITH EPERM FROM THE PROCESS-GROUP KILL.** Measured iter-196 by the row-106 designer (design V34): **3/320** under stress on BOTH the pristine `capsule.go` at `535330d` and the prototype, so it predates row 106. Suspected mechanism (UNMEASURED): macOS returns EPERM rather than ESRCH when only a zombie remains in the group, which row 24's ESRCH handling does not cover. The controller did NOT reproduce it (0 fires in 4 full-suite runs this iteration: controller ×2, evaluator ×2), so the count is the designer's, unverified first-party. **THE ITEM:** reproduce with a bounded stress loop, confirm or refute the zombie-group mechanism, then either treat EPERM on an already-reaped group like ESRCH or tighten the test. · ~0.3d · gated on nothing · filed iter-196 (clause-2; groom position 7 bucket).

116. **[HARNESS] ticket:pi-runner:quota-429-reported-as-empty-worktree** · no clause (harness; fleet-owned) · **THE PI RUNNER REPORTS A PROVIDER QUOTA WALL AS `empty_worktree`.** Measured iter-196: the kimi designer run hit the Ollama Cloud weekly limit (`429` ×4 in `message_end.errorMessage`, `stopReason=error`) and `mission_pi_run.sh` returned verdict `empty_worktree` rc=10, the same verdict as a model that did nothing. Only reading the NDJSON classified it as capacity (standing rule 8), not a lane/model failure. Filed to the fleet loop as a ticket (`inbox_1790449840991_68ec78e8`, blocking none); NOT loop work here (Gate-2 admissibility). Unpark on the fleet's `harness-resolved` reply. · filed iter-196.
...
22. **[LANDED 2026-09-25 (iter-189) — PR #148 squash `e34416f`, remote CI green on the merge (Gate 3b SHA-pinned). Judged PASS 97/100, then r2 99/100, zero blocking (sonnet, own worktree). Arm B: `daemon.New` refuses to start unless the opened store's effective `BusyTimeout()` is strictly below `readDeadline`, and a test reds on a reorder in either direction. Arm A (per-request capping) was measured feasible and deferred; the successor is row 111. Doc `implemented/w-daemon-lock-wait-not-deadline-bound.md`]** w-daemon-lock-wait-not-deadline-bound** · clause-2 · **THE READ DEADLINE DOES NOT GOVERN LOCK
...
23. **[PLUMBING TRANCHE LANDED 2026-09-26 (iter-194) — PR #150 squash `b9ecabe`, remote CI green on the merge (Gate 3b SHA-pinned); judged PASS 98/100, zero blocking (sonnet, own worktree). All 11 reads inherit the caller's context; pins are an empty map; `TestProductionContextRoots` pins 15 named roots; `TestProductionGoSurface` guards host/cmd. Doc `implemented/w-store-deadline-free-residue-owner.md`. ROW STAYS OPEN as the owner of the POLICY tranche (strict guard + `Commit`); `D-WORLD-37` RULED A (attended 2026-09-26) — the policy tranche is now routable, and its design returns the concrete bound table to Mark before shipping.]** **w-store-deadline-free-residue-owner** · clause-2 · **ITEM 18's RATCHET NAMES A FOLLOW-ON ITEM
...
24. **[LANDED 2026-09-26 (iter-193) — PR #149 squash `24df816`, remote CI green on the merge (Gate 3b SHA-pinned), linux merge gate V34/V35 banked from the CI `-v` log. Judged PASS 98/100, zero blocking (sonnet, own worktree). Group-wide overflow kill with the kill error joined behind the typed error (ESRCH filtered), a deadline pipe close for a setsid escapee, and `host/procbound`: a bounded direct-child wait with an exact admission reservation. Test-side signals go through the `host/proctest` pid guard. Controller drills 19/19 + 5/5 + 22/22. Residuals are rows 112 and 113. Design iter-192 (orphan), plan and execution iter-193. Doc `implemented/w-host-subprocess-cleanup-boundary.md`]** w-host-subprocess-cleanup-boundary** · clause-2 · **THE OVERFLOW KILL IS NOT PROCESS-GROUP-WIDE
...
27. **[LANDED 2026-09-25 (iter-188) — PR #147 squash `73e06ae`, remote CI green on the merge (Gate 3b SHA-pinned); judged PASS 97/100 zero blocking (sonnet, own worktree); arm (c) with UPSTREAM's own `InterfaceHashV2` (pinned v0.41.0 `pkg quality --json --no-run`), carried through ready packet / package gate / world-publish fence / reconcile (now four digests); re-break test `TestInterfaceV2MovesWhenAnExportedADTGainsAConstructor`; §8.3 corrected; no A/B was owed — arm (a) would conflict every reconcile of the published 0.1.0; doc `implemented/w-interface-hash-covers-the-interface.md`]** **w-interface-hash-does-not-cover-the-interface** · clause-1 · **THE FIELD NAMED `interfaceHash` DOES NOT CHANGE WHEN THE INTERFACE CHANGES, AND THIS MISSION HAS NOW MEASURED THAT TWICE, TWO MONTHS APART, WITH THE SAME HASH LITERAL.** Filed 2026-08-21 (iter-103) off `PE.A`, which is the first change in this repo's history to add a CONSTRUCTOR to an already-exported ADT — the one stimulus that makes the gap visible rather than theoretical.
...
35. **[LANDED 2026-09-24 (iter-184) — PR #143 squash `9574d08`, remote CI green on the merge (Gate 3b SHA-pinned); designed together with its sibling in `design_docs/implemented/w-workbench-timeline-seam.md`; judged PASS 95/100 + r2 100/100 zero blocking (sonnet, own worktree)]** **w-workbench-view-model-fields-render-nowhere** · clause-5 · **THREE `EntryView` FIELDS THE
...
38. **[LANDED 2026-09-24 (iter-184) — PR #143 squash `9574d08`, remote CI green on the merge (Gate 3b SHA-pinned); designed together with its sibling in `design_docs/implemented/w-workbench-timeline-seam.md`; judged PASS 95/100 + r2 100/100 zero blocking (sonnet, own worktree)]** **w-workbench-timeline-pagination-is-unwired-at-both-ends** · clause-5 · **THE TIMELINE'S
...
39. **[LANDED 2026-09-24 (iter-181) — PR #141 squash `a036062`, remote CI green on the merge (Gate 3b SHA-pinned); judged THREE times independent: iter-180 PASS 95/100 @ `cd687ad`, iter-181 confirmation PASS 97/100 @ `584efe6` + round-2 PASS 100/100 on the bench-fix delta `c6bfe63` — zero blocking anywhere; landed under the attended `D-WORLD-36` ruling (fix the harness), the first pi-controller land in this repo]** **w-session-authority** · clause-3 · **THE REPO HAS NO INBOUND CREDENTIAL -> SESSION
...
40. **[LANDED 2026-09-25 (iter-187) — CARD HALF COMPLETE; INVOCATION SPLIT TO ROWS 106+107]** PR [#146](https://github.com/sunholo-data/ailang-world/pull/146) → squash [`8bb8502`](https://github.com/sunholo-data/ailang-world/commit/8bb8502), remote CI green on the MERGE (Gate 3b SHA-pinned: present 2 / completed 2 / success 2, 1 push run success). Session-scoped A2A agent card (`GET /.well-known/agent.json`) + fail-closed `/a2a/` in `host/projection`, P6.A-CTX (`authority.ResolveContext`) and P6.D (ailang v0.33.2 `serveapi/protocol`, one package-path allowlist line). Revised against row 39's landed contract and DESCOPED by controller SPLIT on three measurements: no server-side invocation coordinator exists (→ row 106), the transition registry is never populated in production (→ row 107), and `protocol.CallerSurface`'s MCP name grammar rejects World stable IDs (→ row 108). Quorum r1 BLOCKED 3/3 (astra upheld: context-free resolve on a single-connection store → P6.A-CTX), r2 BLOCKED 3/3 on one surface → narrow-refinement carve-out. Judged sonnet **PASS 93/100** + r2 **98/100**, zero blocking. Doc + plan in [`implemented/`](implemented/w-a2a-session-projection.md). **Prior head text follows.** ~~**[UNBLOCKED 2026-09-24 (iter-181): row 39's adapter contract is landed on `dev` at `a036062` — the interface it needs now exists]**~~ **w-a2a-session-projection** · clause-6 · The A2A half of the old
...
50. **[LANDED 2026-09-05 (iter-155) AS A CONSEQUENCE OF ROW 59, exactly as `D-WORLD-31` directed — the fixture migration was bought ONCE, inside row 59's sprint, and this row closes without a sprint of its own. Its doc `w-shell-assignment-parser-drops-an-indented-assignment.md` is marked SUPERSEDED in `d353ef1`. The attended ruling's load-bearing condition — *"this hold is contingent on row 59 actually being taken next"* — was MET: row 59 was iteration 155's pick.]** Was: **[`D-WORLD-31` RESOLVED 2026-09-03 (Mark, attended) — NEITHER OPTION AS OFFERED. THE ROW HOLDS
    AT ZERO COST AND NOW CLOSES AS A *CONSEQUENCE* OF ROW 59, NOT BY A SPRINT OF ITS OWN.** The
    ruling: shipping ratified rule A would spend 0.1d to ADD a fresh instance of the very class row
    59 exists to remove (a `KNOWN_BAD` assignment inside `if false; then … fi` that the helper reads
    green while bash leaves the name UNSET), so option B's declarative fixture — the general cure
    for both — is bought ONCE inside row 59. **DONE 2026-09-04 (iter-154):** the fold-in landed in
    `design_docs/planned/w-load-bearing-criteria-need-a-mutation-not-a-grep.md`, which is
    quorum-cleared and carries row 50's closure as its `AC2`/`D5`; this row's own doc
    `w-shell-assignment-parser-drops-an-indented-assignment.md` is **SUPERSEDED**, not funded.
    **THE RULING'S CONDITION IS MET** — *"this hold is contingent on row 59 actually being taken
    next"* — row 59 WAS the next pick and is now the queue head as a sprint. If that sprint slips,
    the ruling's fallback stands: come back and ship A rather than leaving both parked, and say so
    in that iteration's report. This row is **no longer `needs-human-review`** and asks Mark
    nothing. Prior row text follows.]** ~~[RE-PARKED `needs-human-review` 2026-09-01 (iter-146) ON THE NEW `D-WORLD-31` — UNPARKED BY `D-WORLD-29` (Mark, attended 2026-09-01), REVISED TO THE RATIFIED DIRECTION, AND THEN BLOCKED AT A FULL-STRENGTH ROUND 3 BY A DEFECT THE RULING'S OWN RATIONALE DOES NOT COVER. THE RULING PICKED **A**, WHICH REVERSES THE DESIGN DOC AND RESTORES THIS ROW'S OWN ORIGINAL PRESCRIPTION — AND IT EXPLICITLY AUTHORISES THE TEXT RETRACTION BELOW, WHICH IS THE FIRST TIME THIS LOOP HAS BEEN GIVEN LEAVE TO EDIT RATIFIED QUEUE TEXT.** Ruling, verbatim: *"ANSWERED — A — ACCEPT the indented assignment. One whitespace-tolerant scan: trim leading spaces/tabs, ignore comments, extract NAME=\"...\" regardless of indentation, require len(values) == 1. B's stated rationale (\"an indented assignment is a conditional/nested shadow\") is measured FALSE, and defence-in-depth bought with a false premise is not worth a loud red on a benign re-indent. AMEND queue row 50 in the same iteration to drop the sentence carrying that premise — it is ratified text the loop could not edit itself, and this ruling authorises exactly that edit."* Provenance checked first-party under the shared skill's ATTENDED LEDGER EDITS contract: `git log -1 -S'| D-WORLD-29 | RESOLVED'` on this charter returns an ATTENDED (non-fleet) author, **not** the fleet account `sunholo-voight-kampff`, so it is a human answer and not a self-resolution.
...
56. **[LANDED 2026-09-02 — ROW COMPLETE, AND TAGGED LATE BY ITERATION 151 BECAUSE THE ITERATION
    ...
    **zero** charter rows, **zero** log entries, **zero** STATUS stamps and **zero** dashboard
    ...
    `19:26:21Z`. **Its cause was RECOVERED before this iteration closed, from the one
    channel the reboot could not wipe:** the driver posted its own crash notice to `#107` at
    `2026-09-02T19:27:22Z` — `rc=143`, a deliberate watchdog kill, **61 seconds after the merge**.
    Stall rather than hard timeout by elimination (~60 min of a 6 h `HARD_TIMEOUT`), on the
    pre-`e308577` CPU-only idleness arm the driver's own comment now records as false-premised.
    See the iteration-151 STATUS stamp; the reason it was nearly missed is row 73. Prior row text
...
57. **[AMENDED 2026-09-03 (iter-152) — TRACKING ONLY NOW: THE ROW'S CAUSAL CLAIM IS REFUTED BY
    MEASUREMENT, ITS ONE EXPLICITLY-UNMEASURED CLAIM IS SETTLED IN THE SAFE DIRECTION, AND WHAT THIS
    ROW CALLS ONE DEFECT IS IN FACT TWO WITH DIFFERENT FIXES.** All of the below measured first-party
    at `sunholo-data/ailang` `origin/dev` (both files clean against origin at read time), with
    controls, and it corrects an issue **World itself filed**.
    **REFUTED — there is no typed sub-query, and one is not expressible.** The row asserts *"the
    typed sub-query finds zero `approval_request` rows and reports `✓ No pending approval
    requests`"*, which reads as: fix the type and the false green goes away. It does not.
    (i) `printApprovalsInboxPending` (`cmd/ailang/coordinator_pending_spine.go`) queries
    `InboxListOptions{Inbox:"approvals", UnreadOnly:true, Limit:50}` — **no type filter** — and that
    is proven EMPIRICALLY rather than by reading, because it **listed a row whose `message_type` is
    `notification`** (my own iter-152 spine post, `inbox_1788448377647_d836d0e8`).
    (ii) `messaging.InboxListOptions` (`internal/messaging/inbox.go:82-93`) declares **10** filter
    fields (`Inbox`, `Status`, `UnreadOnly`, `FromAgent`, `Limit`, `IncludeRead`, `Collapsed`,
    `DupOf`, `StartDate`, `EndDate`) and **none** filters on message type, so the sub-query this row
    describes cannot be written against the API at all.
    (iii) The green line is a verdict about a **different data source**: `coordinator_list.go:137`
    prints it when `store.ListPendingApprovals(ctx)` is empty, and that is
    `FROM approval_requests WHERE status = 'pending'`
    (`internal/coordinator/store_sqlite_approvals.go:88`) — local SQLite, a different table, and on
    a `storage=gcp` node a different backend from the Firestore inbox above it. That file references
    `inbox_messages` **0** times (control: `approval_requests` **16**).
    (iv) **TWO-ARM CONTROL, and it is the load-bearing one** (rule 3d — my result came back in the
    direction I predicted, so it needed a negative arm): `ailang coordinator pending` with **0**
    unread inbox rows and with **1** printed a **BYTE-IDENTICAL** `✓ No pending approval requests`.
    The verdict is provably invariant to the inbox contents; the only delta between arms is the
    spine block appearing above it.
    (v) Nothing repo-wide compares `message_type` to `approval_request`; the sole hit is a **writer**
    (`internal/coordinator/daemon_tasks_exec_run.go:612`).
    **⇒ Setting `message_type=approval_request` on the send path would leave the false green
    byte-identical.** Filed upstream as the correction on
    [ailang#984](https://github.com/sunholo-data/ailang/issues/984) (0 comments before mine, so
    nothing had been built on the bad premise), and the real cause filed separately as
    [ailang#1036](https://github.com/sunholo-data/ailang/issues/1036).
    **DEFECT 1, RESTATED CORRECTLY — `--type` is not ignored, it is MISFILED.**
    `cmd/ailang/messages_send.go:42` binds `--type` to `Category`; `:132` **hardcodes**
    `MessageType: messaging.InboxTypeNotification`; `:137` sets `Category: category`. So the value
    IS stored, in the wrong field. Empirically, across all **18** rows in the `approvals` inbox the
    split is total by author class: **12** mission-loop rows (`mission-v1` ×5, `mission-world` ×4,
    `mission-motoko` ×2, `audit-r1` ×1) are `type=notification`/`category=approval_request`; **6**
    `coordinator` rows are `type=approval_request`/`category=` empty. Reproduced live on my own send.
    (The row's original 6+6-of-12 census still holds shape at 12+6-of-18.)
    **AND ITS REAL BLAST RADIUS, NEWLY MEASURED — the misfiling is NOT cosmetic, it just does not
    hurt where this row said.** `message_type` is read by live consumers:
    `cmd/ailang/messages_activity.go:59` aggregates `ByType[m.MessageType]`, and `ailang messages
    activity` live reports `by type: 29 notification, 1 completion` with **ZERO `approval_request`**
    while simultaneously listing `1 approvals` under busiest inboxes — fleet approval traffic is
    invisible to that view; `internal/coordinator/agent_registry.go:56` routes inbound templates via
    `TemplateByMessageType`, so a mission-authored approval can never match an `approval_request`
    template; `backstop_sweep.go:157,202` and `message_adapter.go:51` both key `Kind` off
    `MessageType`.
    **THE ROW'S ONE UNMEASURED CLAIM IS NOW SETTLED, AND IN THE SAFE DIRECTION.** The row says
    whether the Discord "🔔 Approval needed" push filters on type is *"NOT established and must be
    measured before this row claims it"* — correct discipline, and here is the measurement:
    `internal/daemon/messageNotification` (`handlers.go:104`) references `MessageType` **0** times
    (controls: `ToInbox` **7** in the same file, `MessageType` **106** repo-wide, fresh absent
    literal **0**), switches on `m.ToInbox`, and `humanTriageInbox` accepts `approvals`; it is LIVE
    from `internal/daemon/daemon.go:204` (a caller search, not a call-site grep — it is not
    test-only). **So mission-authored approvals DO reach Discord despite the wrong type — the ask is
    not lost from the human channel**, which lowers this whole class from "asks are invisible" to
    "asks are mis-typed in aggregation and routing".
    **ONE MORE DEFECT FOUND IN PASSING:** `coordinator_pending_spine.go:9`'s own doc comment claims
    it *"lists unread approval_request messages"* — a filter the code does not have. Carried in
    `#1036`.
    **DISPOSITION — World's side is DONE and this row is now tracking-only.** The fix is in
    `sunholo-data/ailang` (CLI) and, for the misfiling, arguably in the shared mission-control
    skill's Gate-5 approvals snippet, which every mission on this rig runs and which World cannot
    edit (it resolves into the V1 checkout). Per the frozen-core rule World does not work around it
    locally: the deliverable was the measurement plus the upstream issues plus the cross-mission
    note (`mission-control`, `inbox_1788448675286_e0797635`). Re-check `#984`/`#1036` at pick time
    rather than transcribing this row — the predicate is an upstream disposition, i.e. exactly the
    external-party class Gate 1 requires be RUN, not read. · ~0.2d spent · gated on upstream ·
    surfaced iter-139, corrected iter-152. **Prior row text follows and STANDS except where
    contradicted above.**
    ~~57-original.~~ **w-approvals-spine-prints-a-green-no-pending-under-the-row-it-just-listed** · clause-2 ·
...
58. **[LANDED 2026-09-03 (iter-153) — THE MECHANISM IS *CONCURRENCY*, NOT SPEED, AND THE DISPOSITION IS ATTRIBUTION RATHER THAN A BIGGER NUMBER.** PR [#115](https://github.com/sunholo-data/ailang-world/pull/115) → squash [`12b8c87`](https://github.com/sunholo-data/ailang-world/commit/12b8c87), Gate 3b GREEN on the MERGE commit (`present=3 == expected=3`, `not_green=0`, `runs_total=1 event=push`, parent control `checks=3`). The `--version` probe costs **47–52 ms** warm and **1211–1294 ms** on a FIRST exec at a fresh path, and macOS serializes that assessment **globally**: 8 concurrent first execs return at 1289…**8871** ms and 12 at 1255…**13691** ms, linear at ~1.13 s each. `probeTimeout` is a PER-PROBE bound, so it is crossed at **N ≥ 9** regardless of single-probe speed — which is why the suite is green package-by-package, flaky under `go test ./...`, and never red on Linux CI. **Two candidate causes REFUTED by the same measurements:** CPU contention (iter-141's axis) moves it to only 1322/1373/1385 ms under 16 spinners, and the per-invocation Observatory cleanup over a 553 MB DB is paid by the ~50 ms warm arm. **`probeTimeout` was deliberately NOT raised** — the cost scales with concurrency, which the probe cannot observe, so a constant only moves the cliff (rule 3m). The deliverable is the row's own: `archive.EnvironmentFailure` + `archive.AttributeFailure` label the deadline an ENVIRONMENT failure at 8 call sites; nothing is skipped and nothing suppressed, the test still FAILS and now says which of the two things went wrong. 6 mutants, 6 RED, each landed by sha256 with `go vet` rc=0 read first and restored byte-identical — **M6 (drop `probeVersion`'s `%w`) is killed by the SHIPPED-path arm ALONE**, which is what proves that arm is not redundant with the error literals in the test file. **Also fixed in the same file:** `host/archive` resolved the pin from a hardcoded `/tmp/ailang-v0300/ailang`, dead on EVERY machine since iter-151 moved it, so `TestArchivePinnedInterpreter` was a silent SKIP with no red anywhere; it now reads `AILANG_BIN` like its two sibling packages, which is what `ci.yml`'s own comment already asked for — measured SKIP → PASS (0.95 s), and it now runs in CI too. Prior row text follows.]** ~~ **[AMENDED 2026-08-31 (iter-141) — THE HEADLINE IS WRONG AS FILED: THE GATE IS **FLAKY**, NOT DETERMINISTICALLY RED, AND THAT CHANGES THE FIX.** Four runs of `scripts/verify_go.sh` on this rig within about 90 minutes, on trees whose only difference is one test file: iter-140 **rc=1 with FOUR** failing tests; this iteration's Gate-2 base run **rc=1 with ONE** (`TestHandlerTimeoutKillsTheWholeProcessGroup`, and note it failed on a DIFFERENT mechanism — its own non-vacuity markers reporting `exec_started=false forked=false`, not the archived-interpreter `--version` timeout row 58 names); the `opus` planner's base run **rc=0 with an EMPTY failing set**; the controller's post-change run **rc=0 with an EMPTY failing set**. Two red, two green, three different failing sets. **The planner refuted a fact the controller had handed it under a VERIFIED-BY-ME label** — which is the loop working (Gate-2 rule (d)) — and the correction matters because row 58 as filed prescribes repairing every sprint AC around a *deterministic* base red. A flaky gate needs a different remedy: an AC written as a SET comparison against a recorded base set is still right, but the row must ALSO say that the base set is not stable across runs, so the honest criterion is `S_after \ S_known_flaky == ∅` with `host/verifygate` `ok` in both legs, and a red must be re-run unloaded before it is attributed to anything. The mechanism analysis (macOS first-exec provenance assessment, 96 ms pinned vs 1336 ms cold, plus a per-invocation cleanup over a 513 MB Observatory DB) STANDS as a real rig cost and is the likeliest driver of the variance; what does not stand is *"rc=1 on pristine dev"* stated as a property. **The instrument-failure floor the row asks for is now MORE clearly the right deliverable, not less** — a timing-sensitive test that reds under load and passes alone is exactly a test that should name the timeout an ENVIRONMENT failure. Prior row text follows.** ~~**w-verify-go-is-red-at-pristine-base-on-the-rig-while-ci-is-green** · clause-2 · **THE LOOP'S OWN LOCAL GATE `scripts/verify_go.sh` IS `rc=1` ON UNTOUCHED `dev`, SO EVERY SPRINT'S `verify_go.sh rc=0` ACCEPTANCE CRITERION IS BROKEN AS WRITTEN — IT CAN ONLY BE WAVED THROUGH OR BLAMED ON THE SPRINT.** Measured iter-140 on a pristine worktree at `dev` = `9c0ad0b`, in **TWO arms** — once under concurrent load and once alone, because the controller had itself introduced a load confound by running two full suites plus a designer session in parallel. **The confound is REFUTED: the failing set is IDENTICAL in both arms**, four tests, `diff` silent — `TestEpisodeLiveReplayThreeArmsAndEvidence` and `TestHandlerTimeoutKillsTheWholeProcessGroup` (`host/broker`), `TestF1PinnedInterpreterHashMismatchRefusedBeforeExec` (`host/capsule`), `TestFixtureEpisodeReplaysBitForBit` (`host/replay`); `host/verifygate` is `ok` in both. **Three of the four share ONE mechanism** — `cannot obtain --version from archived interpreter … timed out after 10s`. **Mechanism isolated, not guessed:** the same binary answers `--version` in **96 ms** at its pinned path, **1336 ms** on FIRST exec of a freshly-copied instance at a new temp path, and **372 ms** on a cached second exec — macOS first-exec code-signing/provenance assessment (`com.apple.provenance` xattr present; `spctl -a -t exec` rejects) — on top of a per-invocation Observatory retention cleanup over a **513 MB** DB that every `ailang` invocation on this rig now pays. **This is RIG-LOCAL, not a repo defect, and the discriminator is the fleet control group:** dev CI is **GREEN on this same commit**, 2/2 checks, on Linux runners with neither Gatekeeper nor a 513 MB observatory. **Why it is a row and not a shrug:** `verify_go.sh` is the gate this mission cites in every sprint's acceptance criteria and every STATUS stamp, and iter-139 recorded it `rc=0` roughly five hours earlier — so the regression is in the rig's state, is recent, and will silently convert into either a false *"the sprint broke it"* or a habit of ignoring the gate. **THE ITEM:** decide the disposition — raise the archived-interpreter `--version` deadline above the first-exec assessment cost, pre-warm or ad-hoc-sign the archived copy, or bound the observatory cleanup — and, whichever lands, give the three replay-family tests an instrument-failure floor that names the timeout as an **ENVIRONMENT** failure rather than a replay failure, so a rig cost can never again wear a correctness defect's clothes. · ~0.3d · gated on nothing · surfaced iter-140 (controller-measured).~~

59. **[LANDED 2026-09-05 (iter-155), CI green 3/3 on the merge commit `d353ef1`] — PR [#116](https://github.com/sunholo-data/ailang-world/pull/116), rebase-merged so the three milestone commits survive on `dev` (`fb8bc29` MS1 fixture+parser+five readers repointed+runtime test, `5d84209` MS2 fixture-shape gate, `d353ef1` MS3 prose rule + row-50 supersession). Evaluator `sonnet` **PASS 96/100**, round 1, **zero BLOCKING**; the judge built its OWN sensitivity drills (weakening `BADCHARS` and the name allow-list) rather than replaying the executor's mutants, and exactly the corresponding subtests went red. **FOUR DEFECTS IN THE QUORUM-CLEARED DOC, ALL RAISED BY THE PLANNER AND ALL REPRODUCED FIRST-PARTY BEFORE ACTING (rule 3f), landed as `5928453` BEFORE any code:** **V-20** the parser was STILL a bash 3.2 syntax error — iter-154's `V-19` fixed the RECORD regex and left the WHITELIST regex inline one call site over, and the inline form is rc=2 on the GOOD value, so M11/M12/M13 would each have read "rejected" while the parser rejected everything (guard the helper, miss the call site, one iteration later); **V-21** AC1 and AC4 were VACUOUS AT BASE — `go test -run '<a test that does not exist>'` is **rc=0** `[no tests to run]`, indistinguishable by exit code from the existing-test control, so the two ACs whose stated baseline was "green only after the fixture lands" were green before anything was written (this row's own defect wearing `go test -run`'s clothes); **V-22** a FIFTH reader at `toolchain_pin_gate_test.go:315`, invisible to V-2 because V-2 anchored on the function NAME rather than on the fact it claimed — deleting the data lines would have redded the MS1 boundary; **V-23** M7's substring can never appear literally. Two NON-BLOCKING evaluator findings recorded, not waved through: AC4's "regardless of failure mode" is oversold by one notch (the canonical substring fires in 1 of 4 subtests under M10; the gate still reds), and the doc's Timeline says "add the `source`" where its Solution Design correctly says it never sources. Verify gate green BOTH legs out of the executor's sandbox; controller's own M1 drill discharged by sha256 with a pristine control either side. Doc → `implemented/`.** Was: DESIGNED + QUORUM-CLEARED 2026-09-04 (iter-154).
    Doc `design_docs/planned/w-load-bearing-criteria-need-a-mutation-not-a-grep.md` (620 lines),
    designer `pi:ollama/deepseek-v4-flash:0731-cloud` (3 runs, typed verdict `ok` each, flat-rate
    `$0.00`). **Row 50's option-B fixture migration is FOLDED IN per the attended ruling
    `D-WORLD-31`** — the doc is the cure for BOTH rows, and row 50 closes as a consequence of this
    sprint rather than by one of its own. **TWO FULL-STRENGTH QUORUM ROUNDS** (3/3 present both
    times, `.synthesis.absent_reviewers` `[]` cross-checked): R1 **blocked**, 3 rejects,
    `metered=$0.08615`; R2 **blocked**, `gemini-3-1-pro` **PASS** (first pass), `gpt5-6-sol` and
    `oc-glm-5-2` reject, `metered=$0.10906`. Cleared by the **CONTROLLER CARVE-OUT** — a bounded
    2nd revision applying both survivors' VERBATIM fixes, neither of which reversed the design's
    direction. **THE TWO OBJECTIONS THAT EARNED THEIR MONEY, both confirmed first-party rather
    than forwarded (rule 3f):** `gpt5-6-sol` showed the "data-only by construction" fixture is
    data-only to the STATIC SCAN and CODE to bash — measured, `KNOWN_BAD="$(touch …/PWNED)"`,
    `PATH="/nonsense"` and a benign line ALL match the doc's own grammar (`grep -cE` = **3**),
    sourcing created the sentinel and clobbered `PATH`, negative control **0** — so the design
    would have shipped arbitrary code execution behind a gate that reads the file as data; the fix
    is a bounded fail-loud parser (name allow-list, inert value class, `printf -v`, refusal on
    unknown/duplicate/malformed, all-three-present check). `oc-glm-5-2` showed `AC2` discharged
    *"row 50's defect is provably gone"* with a `grep -cE` of the fixture's own shape — **the doc
    shipping the defect it exists to kill** — now deleted and pointed at mutations M1/M2/M8.
    **`M9` DISCHARGED FIRST-PARTY** (not asserted): the canary assertion wrapped in `if false { … }`,
    mutant landed by sha256 `a23cfa79419ae691` → `6ddd8ca5209a3d37`, `go vet` rc=0 read BEFORE any
    test result, test **rc=1** `--- FAIL`=1 carrying `assertion if-stmt count=0, want exactly 1`,
    restored byte-identical, pristine control green either side. **AND `V-19`, WHICH NO REVIEWER
    ASKED FOR:** the prescribed parser with the regex INLINE is a **bash 3.2 syntax error** on this
    rig's `3.2.57` — the exact version the `launchd drivers (bash 3.2)` CI job pins — and all four
    fixtures returned **rc=2** INCLUDING the good one, so M11/M12/M13 would each have read
    "rejected" vacuously. With the regex in a variable: good rc=0, M11 rc=1 with the sentinel NOT
    created, M13 rc=1, M1 rc=1. **NEXT: `sprint-planner`.** Prior row text follows.]**
    ~~**w-static-grep-cannot-prove-an-assertion-is-live**~~ · clause-2 · **THE ROW-51 DESIGN PROVES ITS
...
60. **[LANDED 2026-09-05 (iter-156), CI green 3/3 on the merge commit `3417088`] — PR [#117](https://github.com/sunholo-data/ailang-world/pull/117) -> squash [`3417088`](https://github.com/sunholo-data/ailang-world/commit/3417088). **THE ROW NAMED ONE CALL SITE AND THERE WERE THREE, AND THE PREDECESSOR NEEDLE BOUND LESS THAN IT READ.** Disposition: row 60's **option (a)** — relax the needles to bind the operand ORDER and each operand's DERIVATION, and add a rename arm to the green-control set; option (b) is NOT also taken, per the row's own "do not do both". **THE CLAIM, REPRODUCED FIRST-PARTY BEFORE ANY CODE (rule 3f):** the P1 block extracted and run standalone gives byte-identical verdicts on all four arms under a consistent `ROOT_FLOOR` rename (above-floor `rc=0`; below-floor `rc=1` *is BELOW the root module floor*; malformed `rc=1` *cannot order toolchain tokens*), `bash -n` rc=0 — and `P1d`'s literal count went 1 -> 0, redding CI. **FINDING 1 — THREE CALL SITES, EACH INVISIBLE UNTIL THE ONE BEFORE IT WAS FIXED.** Relaxing `P1d` left the rename redding on **`P2`**'s `GOTOOLCHAIN="$ACTIVE_GO" go run -race .`; relaxing that left it redding on the **deny-list anchor** `case "$ACTIVE_GO" in`. This repo's own named *guard the helper, miss the call site* shape, three deep in one function, and it was found only because the green control was run as a real mutation rather than asserted. Enumerated afterwards: both identifiers across `host/verifygate/*.go` = 4 hits, 3 comments, **0** remaining executable pins; negative control on an invented identifier fired. **FINDING 2 — THE PREDECESSOR WAS FAIL-OPEN ON A DEFECT IT IS DOCUMENTED AS THE SOLE KILLER FOR.** Nothing tied `$ROOT_FLOOR` to the go.mod floor read, so the inversion `P1d` exists to catch is reachable by swapping the two ASSIGNMENTS instead of the call. Measured: **the old literal count stays 1 (GREEN)** while the gate compares floor >= active. Row 48's `V21` records `P1d` as the sole killer for `M14`/`M16` in the entire sprint. **FINDING 3 — M17 IS NARROWER THAN IT READS AND A DRILL-ONLY ARM DECAYS.** The set is split out as `p1NeedleSet(t, src)`, a function of the file's TEXT, and the green arm runs THAT rather than the binding helpers, so a needle added later is covered without anyone widening the arm. Non-vacuity: re-introducing a spelling pin makes the green arm the **SOLE** detector with the production test still passing. **NET STRICTLY STRONGER**, with two assertions the literals could not express: `$ACTIVE_GO` must not be shadowed inside the block, and the toolchain the floor gate vets must be the same variable the race control runs under. **FILE-LEVEL DRILL, 8 mutants**, each landed in `verify_go.sh` by sha256, `bash -n` and `go vet` rc=0 read BEFORE any test result, each restored byte-identical, pristine control green either side: consistent rename **GREEN** (the row's point; predecessor literal 0 = the old red); call-operand swap RED; **reassignment swap RED with the predecessor literal at 1** — the old needle's green; self-comparison RED; `GOTOOLCHAIN` dropped RED; `GOTOOLCHAIN` underived RED; P1 block deleted RED; floor hardcoded RED. **SENSITIVITY DRILL — AND THE CONTROLLER'S FIRST COMMIT FAILED IT.** Neutering 4 of the 7 conjuncts left **every arm green**, because the arms asserted only `err != nil` and a neighbouring conjunct caught the same mutant: a conjunct with no sole killer is one that can be deleted without a red. Fixed by `assertRed`, which requires the mutant to be rejected AND rejected for the STATED reason — and that immediately **corrected two attributions the controller had predicted wrong** (both swap arms are killed by the floor-derivation check, not by the active-derivation and shadow checks). Two arms added for the conjuncts still uncovered. Re-run: **7 of 7 conjuncts now have a sole killer.** Gate green BOTH legs out of the drill: `verify_ail.sh` rc=0 (11 identities, 40 named tests, 9/9 world-package steps, `compiler pinned by exact bytes: AILANG v0.30.0 on Darwin/arm64`); `go build && go vet && go test ./... -count=1` rc=0, **19** ok, **0** FAIL.** Was: surfaced iter-145 (evaluator-found).
61. **[LANDED 2026-09-05 (iter-157), CI green 3/3 on the merge commit `8b600fd`] — PR [#118](https://github.com/sunholo-data/ailang-world/pull/118) -> squash [`8b600fd`](https://github.com/sunholo-data/ailang-world/commit/8b600fd). DISPOSITION: STRUCTURAL, AND THE ROW'S OTHER OPTION IS
    MEASURED FAIL-OPEN.** The verdict is now consumed by `if go_version_ge "$ACTIVE_GO"
    "$ROOT_FLOOR"; then` DIRECTLY — no intermediate variable, nothing between the comparison and
    the branch — and every arm of the attributing `case $?` exits, so a dataflow break can change
    WHICH refusal is reported but cannot produce a success. **The row understated the blast
    radius:** the inserted `floor_rc=0` opens **all three** refusal branches, not just the
    below-floor one the row named — the malformed-token instrument floor goes with it, printing
    `✓ toolchain floor gate: devel >= root module floor go1.26.6`. **The row's option (a) as
    literally offered does NOT work:** dropping the variable and reading `case "$?"` is fail-OPEN
    *even unmutated*, because the intervening `set -e` is itself a successful command that resets
    `$?` — measured, below-floor input, no mutation, rc=0. Only the `if`/`else` form is
    fail-closed, and it stays fail-closed under insertion (rc=1 in the else branch with the
    attribution shifted; rc=1 unchanged in the then branch). **Two new conjuncts, each with a
    proven SOLE killer:** the call must be the `if` condition (killer: a `|| true` disjunction,
    which leaves every needle-visible byte intact) and the gate must contain no `<var>=$?`
    (killer: re-laundering the verdict after `fi`); neutering either reds exactly its own arm with
    the pinned message and nothing else. **Regression proof on the production path:** the row-61
    mutant class landed in the real `scripts/verify_go.sh` was `bash -n` clean, `go vet` clean and
    **GREEN at base**; it is now **RED** naming `verdict is no longer consumed directly by the
    branch (queue row 61)`. Row 48's declared residual widened from "rewrite" to any dataflow
    break including an insertion, and it now states what remains: a comparator-body rewrite,
    covered by the `exit 0`/`exit 1`/`exit 2` contract needle and V15's battery, not by the
    consumption-shape binding. Original row text follows. · **A
...
62. **[LANDED 2026-09-06 (iter-158), CI green 3/3 on the merge commit `dcf534f`] — PR [#119](https://github.com/sunholo-data/ailang-world/pull/119) -> squash [`dcf534f`](https://github.com/sunholo-data/ailang-world/commit/dcf534f). DISPOSITION: THE SCAN NOW READS THE
    STEP'S OWN KEY AND ITS VALUE, AND THE ROW'S OWN PROPOSED REMEDY WAS MEASURED FAIL-OPEN.** Both filed
    shapes were reproduced first-party at the exact lines the row named (`ci.yml:175`, `ci.yml:177`) with the
    unrelated-step control at rc=0, and both are now green. **The row proposed matching the key at "the step's
    own key indentation"; a step's FIRST key rides the block-sequence dash two columns left of every sibling
    key, so `- continue-on-error: true` evaded that rule** — drill arm N1 neuters exactly that case and its sole
    killer is `true_riding_the_block-sequence_dash_is_refused`. Values: only a literal `false` (any case,
    quoted, or with a trailing comment) is an accepted opt-out; `true`, an empty value and a `${{ }}` expression
    are refusals, and a YAML flow-mapping step is an instrument FAILURE rather than a green — row 62 is about
    false POSITIVES and must not be widened into a fail-open. 7-neutering sensitivity drill, every conjunct
    killed, four sole. **DECLARED RESIDUAL:** the scan is still text-level, so a `continue-on-error` injected by
    a composite action, a reusable workflow, or a YAML anchor/merge key is outside it; the original row-44
    residuals (a never-true step-level `if:`, a computed assignment) are unchanged. **ORIGINAL ROW:**
    `w-flag-scan-false-positives-on-an-explicit-false-and-on-benign-script-text` · clause-2 ·
...
63. **[LANDED 2026-09-06 (iter-159), CI green 3/3 on the merge commit `2115172`] — PR [#121](https://github.com/sunholo-data/ailang-world/pull/121) -> squash [`2115172`](https://github.com/sunholo-data/ailang-world/commit/2115172). DISPOSITION: BOTH REFUSALS ARE **REACHABLE**, SO BOTH GOT AN ARM RATHER THAN THE UNREACHABILITY NOTE THE ROW OFFERED AS AN ALTERNATIVE — AND THE HALF THE ROW LEFT UNFIRED IS THE WORSE ONE: `stepCol < 0` IS FIRED BY A PURE RE-STYLE WHOSE PARSED DOCUMENT IS **DEEP-EQUAL** TO THE PRISTINE `ci.yml`, I.E. A LEGITIMATE INPUT ACTIONS RUNS IDENTICALLY, WHILE `anchor < 0` NEEDS A WORKFLOW WITH NO `steps` KEY AT ALL. Measured at base: BOTH branches could be deleted outright with `go test ./host/verifygate/` still rc=0 (positive control — neutering the continue-on-error value check reds 5 arms), so they were guards, not gates. The derivation is now `stepBlockAnchors`, refusal messages byte-identical, carrying three fixture arms each a proven SOLE killer. DECLARED RESIDUAL, MEASURED: discarding the refusal at the call site is backstopped by `stepCol != expectedStepCol`, so it is a MIS-ATTRIBUTION, not a fail-open.] w-locator-derivation-refusals-are-unpinned-and-undeclared** · clause-2 · **THE ROW-52
...
64. [**LANDED 2026-09-06 (iter-160), PR124, merge `bf15c73`, CI GREEN3/3**] **w-fleet-residual-net-shares-phase-1-pathspec** · clause-2 · **THE ROW-54 ARM REPORTS EVERY
...
65. **w-go-build-is-not-a-compile-fence-for-a-test-file** · **[PARKED — needs-human-review, iter161, D-WORLD-33]** — [rejected design](planned/w-go-test-compile-fence.md), [two-round evidence](verification/world-iter161/README.md). Proposed prose lint misses ordinary Markdown; record boundaries and companion association remain unresolved. No planner/executor/evaluator ran and no guard shipped. REDESIGN or DEFER; default DEFER. Prior discovery follows unchanged. · clause-2 · **EVERY "THE MUTANT BUILDS" ASSERTION IN THIS REPO THAT USES `go build ./...` IS VACUOUS WHENEVER THE MUTANT LIVES IN A `_test.go` — AND THE MUTANTS ALWAYS DO, BECAUSE THIS MISSION'S GATES ARE TESTS.** Found by the `opus` planner of row 55 as its refutation R-1, **reproduced first-party by the controller** rather than inherited. `go build` does not compile test files, so a mutation drill that fences on it cannot tell "the mutant is sound" from "the mutant does not compile" — which is the exact third-fact-wearing-the-same-exit-code class the mutation discipline exists to close. **Measured at `234d9da`, mutant LANDED by sha256 and restored byte-identical either side:** appending `func zzBroken() { var x int = "not an int"; _ = x }` to `host/verifygate/dispatch_lever_gate_test.go` leaves `go build ./...` at **rc=0**, while `go test -count=1 -run '^$' ./host/verifygate/` reds at **rc=1** naming the type error, and `go vet ./host/verifygate/` also reds at **rc=1**. So two working fences exist and the one in use is the broken one. **THE ITEM:** sweep every place this repo asserts a mutant builds and replace the fence — `git grep -l` finds the pattern in `CLAUDE.md`'s verify-gate prose and in a number of implemented sprint plans; row 55's own plan already uses the `-run '^$'` form, so the corrected shape is in-tree to copy. **Scope, stated honestly:** `verify_go.sh` itself is NOT affected — it also runs `go test ./... -count=1`, so CI has no hole. This is an **AC-instrument** defect, not a CI defect, and it must not be re-scoped as the latter. **ALSO PROPOSED UPSTREAM:** the shared `mission-control` SKILL.md prescribes exactly this fence ("assert `go build ./...` … rc=0 on the mutated tree"), so the gap is fleet-wide and not World's alone; World cannot edit that file and has proposed it to V1/Mark. **Instance 1** — pre-registered here so the second is recognisable rather than rediscovered. · ~0.1d · gated on nothing · surfaced iter-149 (planner-found, controller-reproduced, declared out of scope by the row it was found in).

66. **w-flow-key-quote-trim-is-uncovered** · **[LANDED 2026-09-07 (iter-169) — REDUCED BY A CONTROLLER SPLIT; the refusal-contract remainder is row 87]** · clause-2 · **A QUOTED KEY INSIDE A FLOW MAPPING — `on: {"workflow_dispatch": }` — IS VALID YAML DECLARING THE LEVER, AND NOTHING PINS THE TRIM THAT MAKES IT MATCH.** Found by the `sonnet` evaluator of row 55 as non-blocking finding 2. Removing `strings.Trim(…, "'\"")` at `dispatch_lever_gate_test.go:162` leaves the full package **rc=0** (compile fence rc=0, `AILANG_BIN`-set package run rc=0), so the shape would go undetected as a declared lever. **The failure is a false RED, not a false green** — consistent with the row's declared fail-loud bias — and the shape sits outside row 55's three declared shapes, which is why it is a row rather than a blocking finding. Note the symmetry worth recording: this is the same defect class row 55 just closed (a valid lever-declaring form the scan cannot see), one nesting level deeper, and row 55's own arm set does not reach it. **THE ITEM:** add the arm, or declare the shape unsupported **in the code** and let it reach `errUnhandledOnForm` loudly — either is acceptable, an undeclared silent brittleness is not. · ~0.1d · gated on nothing · surfaced iter-149 (evaluator-found, out of the found row's declared scope).


82. **w-mutation-drill-restore-target-is-HEAD-not-your-baseline** · clause-2 · **THE SHARED SKILL'S MUTATION-DRILL RECIPE SAYS "RESTORE BYTE-IDENTICAL" AND NEVER SAYS *TO WHAT* — SO THE OBVIOUS MECHANISM, `git checkout -- <file>`, RESTORES TO **HEAD**, AND WHEN THE SUBJECT UNDER MUTATION IS YOUR OWN UNCOMMITTED WORK THAT SILENTLY DESTROYS IT.** Measured first-party this iteration: a drill harness mutating `verify_go.sh` while the *fix* sat uncommitted in `toolchain_pin_gate_test.go` used `git checkout --` as its restore step; on the arm that mutated the test file itself, the restore reverted ~250 lines of finished, verified work to `origin/dev` and the harness's next line printed a plausible per-arm verdict. **The direction is what makes it worth a row: a drill exists to make a destructive edit safe, and its own safety step is the destructive one.** The loop lost the work and had to rebuild it. **It failed loudly only because the harness asserted the post-restore sha256 against a captured baseline** and printed `RESTORE FAILED`; with the near-universal `git checkout -- <f> && echo restored` form it would have been silent, and the drill's remaining arms would have run against a tree missing the subject under test — every one of them reporting on the wrong tree, which is this queue's own recurring shape (*a gate that measures the wrong tree reports the right answer for the wrong reason*, row 81) aimed at the restore rather than at the copy. **THE ITEM:** (a) the drill recipe must name the restore TARGET, not just the property — `cp <backup> <file>` from a backup captured before the first mutation, never `git checkout --`, because the baseline of a drill is *the tree you are testing*, which is only sometimes HEAD; (b) the post-restore sha256 assertion against that captured baseline is **mandatory, not optional**, and it is the sole reason this was survivable; (c) prefer committing the subject before drilling it, which makes HEAD and the baseline the same tree and removes the trap entirely. `.claude/skills/mission-control/SKILL.md` is the V1 checkout's file and World may not edit it (`world-cannot-edit-shared-skill`), so the recipe half would be a proposal to Mark + V1 — but this is a **FIRST INSTANCE**, below the shared skill's own >=2-friction bar, so it is **RECORDED HERE AND NOT FILED UPSTREAM** (iter-108's precedent; filing below the bar is a defect this loop has already paid for once, iter-106). The row exists so a second instance is recognised rather than rediscovered. The repo half — a documented drill helper that captures a baseline and asserts against it — is available now and needs no fleet action. **INSTANCE 2 ARRIVED AT ITERATION 170, AND IT CAME FROM A DIRECTION THIS ROW DID NOT ANTICIPATE — THE RESTORE WAS NOT IN A DRILL, AND THE WORK IT DESTROYED WAS THE *CONTROLLER'S*, NOT THE EXECUTOR'S.** The `pi:deepseek` executor, running under a directive that explicitly forbade git write operations beyond commit-per-milestone, reverted four `git mv`s the controller had STAGED in the shared sprint worktree before the run began (the row-68 `planned/` → `implemented/` moves). Measured: `git status --porcelain` showed the four `R ` entries before the executor started and none after, with the files back in `planned/`; the sprint's own `git diff --stat origin/dev..HEAD` was otherwise clean, so nothing leaked in the other direction. Cost was zero — caught by a `git status` read before the record commit, and the moves were simply redone — but it was luck of timing, not a guard. **What instance 2 adds:** this row's remedy (a) is addressed to whoever WRITES a drill; the hazard is actually the shared TREE. A sub-agent that resets or checks out anything in a worktree the controller is also using can destroy controller state, and no rule anywhere tells it not to — the skill's isolation rule gives the EVALUATOR its own worktree precisely for this reason, and stops there. **THE >=2-FRICTION BAR FOR A SHARED-SKILL PROPOSAL IS NOW MET** (it was explicitly not met at iter-156): the executor-facing half — *the executor's directive must forbid any git write that is not its own commit, and the controller must not leave staged state in a tree it hands to a sub-agent* — is PROPOSED to Mark + V1, since World may not edit that file. · ~0.2d · gated on the fleet for the recipe half · surfaced iter-156 (controller-measured, first instance); **instance 2 iter-170 (controller-measured; bar met, filed upstream)**.

83. **w-a-queue-rows-proposed-remedy-is-a-claim-too-and-nobody-checks-it** · clause-2 · **THE LOOP
    LIVE-REPROS A ROW'S *DEFECT* AND TAKES ITS *FIX* ON TRUST — AND THE FIX IS THE HALF A LATER
    ITERATION WILL ACTUALLY EXECUTE.** Gate 2's ghost discipline and rule 3f are both pointed at
    the row's CLAIM: does the bug exist, at HEAD, measured. Nothing is pointed at the row's
    PRESCRIPTION, though a row that names a disposition is exactly the row a controller will
    implement without re-deriving it — the row already did the thinking, it cites a measurement,
    and re-litigating it feels like waste. **Measured, iteration 157, first instance:** row 61
    named two dispositions and called the first *"the cheapest honest fix … branch directly on the
    call"*. Written literally — `go_version_ge "$ACTIVE_GO" "$ROOT_FLOOR"` followed by
    `case "$?" in` — it is **fail-OPEN on below-floor input with no mutation at all** (`rc=0`),
    because the `set -e` restoring errexit sits between the comparison and the read and is itself
    a successful command that resets `$?`. A controller who implemented the row as written would
    have shipped a gate strictly worse than the one it replaced, with a commit message citing the
    row as authority, and every static needle green. **The tell is that the remedy reads as
    settled work**: the defect paragraph carries `Measured:` and a restore sha, the disposition
    paragraph carries neither, and the visual weight of the first launders the second. Note the
    asymmetry that makes this worth a row rather than a caution — an unverified DEFECT costs an
    iteration on a ghost and is caught the moment you try to reproduce it; an unverified REMEDY is
    caught only *after* it ships, by whatever it was supposed to guard. **THE ITEM:** decide
    whether this is a charter guardrail (a row proposing a disposition must carry the same
    evidence standard as a row proposing a defect: the fix demonstrated, or explicitly marked
    UNVERIFIED) or a Gate-2 controller habit — and if the former, sweep the open rows that name a
    disposition and mark which of them have had it measured. · ~0.2d · gated on nothing · surfaced
    iter-157 (controller-found while implementing row 61; first instance, so it may not spend the
    Gate-5 skill edit).
84. **w-mission-pi-run-verdict-is-blind-to-committed-work** · clause-2 · **THE pi RUNNER'S ONE
    LOAD-BEARING ASSERTION MEASURES THE WORKING TREE, AND THE SKILL'S EXECUTOR CONTRACT MANDATES
    COMMITTING — SO A *WELL-BEHAVED* EXECUTOR SCORES `empty_worktree`, THE RUNNER'S OWN NAME FOR
    THE FALSE-GREEN CLASS, AND THE PRESCRIBED RESPONSE IS TO THROW THE WORK AWAY.** Measured
    first-party this iteration on a complete, independently-evaluated sprint:
    `scripts/mission_pi_run.sh` (V1 checkout, absolute path — absent here, row 69's class)
    returned `{"verdict":"empty_worktree","rc":10,"worktree_changed_files":0}` after 1,593 s and
    94 tool executions, while the same worktree carried **seven commits, 10 files and 2,892
    insertions** against `origin/dev`. The mechanism is one line —
    `mission_pi_run.sh:233`, `DIFF_LINES=$(git -C "$WORKDIR" status --porcelain | wc -l)` — and
    the runner's own header says the worktree diff *"is the only load-bearing"* assertion,
    because `stopReason` is evadable in both directions. It is right that a process signal cannot
    prove work happened; it picked the one artifact a compliant executor is required to empty.
    **The direction is what makes this severe.** The pi recipe's step 3 says any non-zero verdict
    falls back to the next chain link, so obeying the rulebook here means re-running a finished
    sprint on a fresher lane and discarding a judged one — a false NEGATIVE that costs an entire
    iteration and is invisible afterwards, since the fallback run's own record looks normal. It is
    also self-concealing in the ordinary case: an executor that leaves work uncommitted (the
    failure mode) reads `ok`, and one that commits per milestone (the contract) reads `empty`.
    Every previous pi lane run on this rig either died before committing or was a designer
    authoring loose files, which is why five iterations of pi lane experience never surfaced it.
    **THE ITEM:** measure the commit range, not the tree — `git -C "$WORKDIR" diff --stat
    origin/dev..HEAD` or `git log --oneline origin/dev..HEAD`, or take the union with
...
85. **w-charter-status-rotation-rule-body-is-stranded-in-the-archive** · clause-2 · The charter's
    `## STATUS (rotation rule)` heading survives at `design_docs/world-mission.md:795` while the
    rule TEXT it introduces sits in `design_docs/world-mission-status-archive.md` lines 18–19 —
    i.e. a past rotation carried the body out with the stamps and left the heading behind. The
    consequence is that the charter states a rotation rule and does not say what it is, so every
    Gate-4 STATUS rotation is performed from the controller's reading of neighbouring stamps
    rather than from a rule anyone can cite; the shared skill's own mass-deletion guard exists
    because this step has already destroyed the charter's queue once. Surfaced by the iteration-168
    executor while rotating stamps 160–164 and deliberately NOT fixed there (it is charter prose,
    outside that sprint's `design_docs/**`-scoped remit and past one backlog item). **THE ITEM:**
    restore the rule body under its heading in the charter, leave the archived copy alone as
    history, and add an assertion that the heading is followed by non-heading content so the next
    rotation cannot re-strand it. · ~0.1d · gated on nothing · surfaced iter-168
    (executor-measured, first instance).

86. **w-two-iterations-have-no-log-heading-and-no-index-row-anywhere** · clause-2 · Iterations
    **64** and **144** appear in neither `world-mission-log.md` nor `world-mission-log-archive.md`
    (no `## 64`, no `## 144`) and have no row in `world-mission-index.md`. Gate 2's first
    instruction is to grep the index before picking, precisely so the loop does not redo work, and
    the index is REGENERATED from the log — so a missing log entry is a permanent hole in the one
    artifact that answers *"has this been tried?"*, and it reads identically to an iteration that
    never fired. This is the trace the shared skill's died-mid-flight rule tells you to look for
    ("record the orphaned iteration inside your own log entry and credit it — otherwise the log
    silently skips a number and no later reader can tell whether that iteration ran, crashed, or
    never fired at all"), applied to two numbers nobody has walked back. Found by the iteration-168
    executor while adding index rows 162–167, and not chased there for the same scope reason as
    row 85. **THE ITEM:** attribute both numbers from the driver log
    (`/tmp/ailang-mission-world.log`) and any surviving worktree/PR traces, then either write the
    missing entries or record an explicit `## 64 — NO ITERATION` / `## 144 — …` placeholder stating
...
88. **w-gate1-push-boundary-attribution** · clause-2 · **LOCAL GIT TOPOLOGY CANNOT TELL A
    SEPARATELY-PUSHED ZERO-CHECK TIP FROM AN INTRA-PUSH PARENT THAT SHARED A PUSH WITH A LATER
    MERGE — THE TWO HAVE IDENTICAL TOPOLOGY.** Carved out of row 70 by a controller SPLIT at
    iteration 170, after quorum round 2 blocked with BOTH reviewers present and BOTH objections
    landing on this one surface while row 70's own deliverable drew none. `gpt5-6-sol`'s objection,
    verbatim and now this row's problem statement: *"A zero-check commit whose immediate child is a
    merge may be either a separately pushed tip or an intra-push parent included in the same push as
    that merge; both have identical local Git topology. V16 proves only that `e308577` is a merge
    parent, not that it was a push tip. Determinism of local topology does not establish
    correctness, and the rejected Events API does not supply the missing provenance."*
    **TWO SOURCES ARE ALREADY RULED OUT, both measured at iteration 170.** (a) The GitHub **events
    API**: not chronologically ordered, missing the last ~3h of pushes, pagination-limited (HTTP 422
    at page 4), and it has **permanently** lost `e308577`'s push five days on — with `f69873e` found
    in the same call as the positive control. Depending on it would loud-fail Gate 1 on ordinary
    pushes, which is worse than the blindness row 70 set out to fix. (b) **Local topology alone**,
    per the objection above. **`gemini-3-1-pro`'s round-2 fix is inherited verbatim** and any
    implementation must carry it: replace `git rev-list --children --all` with
    `git rev-list --children $base..origin/dev` (the `--all` form leaks children from arbitrary
    local branches, so the verdict would depend on the controller's own git state), and define
    multi-child handling explicitly — *"if a commit has multiple children in the range, it is an
    UNVERIFIED-TIP if ANY child in the range is a merge"*. **Corroborated first-party:** the
    controller's own hand-rolled sweep hit the multi-child case immediately, passing two SHAs to
    `git rev-list --parents -n1` and getting `fatal: ambiguous argument` — gemini's clause 2 is
    required, not hypothetical. **WHAT AN ANSWER NEEDS:** an authoritative, PERSISTED push-boundary
    record. Note the cheap forward-only option — Gate 1 already stamps `~/.ailang/state/mission-world-base`
    every fire, so recording the observed `origin/dev` tip there accumulates exactly that record
    going forward; it cannot recover history, and that limit should be stated rather than designed
    around. **Row 70's shipped instrument does NOT depend on this row** — it deliberately makes no
    tip claim, which is why it could land. · ~0.3d · gated on nothing · surfaced iter-170
    (controller SPLIT; quorum-found).

89. **w-world-pi-sessions-lack-the-session-protocol-gate** · clause-2 · **WORLD'S pi SESSIONS RUN
    WITHOUT THE session-protocol-gate EXTENSION, SO EVERY PI EVALUATOR SPAWN IS TRANSPORT-DEAD AT
    THE HANDSHAKE'S STEP 4 UNTIL THE CONTROLLER REPAIRS THE SCOPE BY HAND.** Measured iter-171:
    the gate ships as an ailang pi extension (fleet repo `.pi/extensions/session-protocol-gate.ts`;
    `ailang pi install` is the Tier-0 global install; Tier-1 is the repo source tree). V1's pin
    worktree carries it project-scoped — V1 sessions ack fine — but the World repo has NO
    `.pi/`, so World worktree sessions expose no `session_protocol_ack`; the iter-171 evaluator's
    first attempt correctly STOPPED per the handshake contract (role transport failure, never a
    judge verdict) and the controller repaired it worktree-scoped
    (`.eval-world-iter171/.pi/extensions/`), which is now the interim Repo Profile bullet. **The
    durable question this row owns:** should the World repo carry `.pi/extensions/` as source
    (Tier-1, like the fleet repo — a drift surface against the fleet's copy that needs its own
    drift-control decision), or should World standardize the worktree-scoped drop permanently,
    or should the fleet be asked to make the gate Tier-0-default on all rigs? The options differ
    in blast radius and none is free. · ~0.2d · gated on nothing · surfaced iter-171 (controller
    retro; evaluator transport failure, first instance).
...
90. **w-census-arms-all-assert-the-total-line-so-no-mutation-localises** · clause-2 · **THE QUEUE-CENSUS SUITE CATCHES EVERY MUTATION AND CAN LOCALISE NONE OF THEM, BECAUSE NEARLY EVERY ARM GREPS THE ONE LINE THE INSTRUMENT PRINTS LAST.** Surfaced by the independent evaluator at iteration 172 (findings F-NB-1/2/3) and **reproduced first-party by the controller before being recorded**: mutating the total line's wording in `scripts/queue_census.sh` (`closed / ` → `closed of `, with a `cp`-captured backup and a sha256-asserted restore — never `git checkout --`, row 82) gives **`58 passed, 10 failed`**, ten failing assertions across **nine** arms (`body-heading`, `closed-vocab`, `enumerate`, `heading-tail`, `open-vocab`, `preamble`, `prose-not-tag`, `report`, `snapshot`, `untagged`), against the design doc's §5 prediction of **`report` only**. The judge measured the same class on three more rows: the decoration-strip mutation reds 4 arms against a predicted 2, `PARKED → closed` reds **14 of 19** against a predicted 3, and the `heading-tail` mutation reds 14 arms while `heading-tail` **itself passes** — i.e. the mutation is detected by every arm except the one named for it. **The direction is safe and that is exactly what makes it durable:** every mutation is caught by MORE arms than predicted, never fewer, so no gate anywhere goes red and the defect is invisible to the whole protocol. But a mutation matrix exists to say *which* assertion defends *which* behaviour — verification rule **3i**, the "kills which mutation" column being the one claim in a sprint nobody checks — and a column that is right about detection and wrong about locality cannot be used to localise a regression, which is the only thing anyone reads it for. It also silently weakens rule 3n's diff-anchored enumeration: an invented mutation that reds nine arms tells you nothing about coverage. **THE ITEM:** decouple the arms from the total line — each arm asserts only the fields its own behaviour owns (`row=`/`tag=`/`class=` for classification arms, `preamble:` for the preamble arm, the floor's message for a floor arm), and exactly ONE arm (`report`) asserts the total line's literal shape. Then re-run the matrix and replace §5's predicted red sets with **measured** ones. Note the honest trade to state in the doc: a shared total-line assertion is *cheap redundancy* and has real value (it is why the fixture's byte defect reddened so loudly in CI), so the fix is to keep one owner for that literal rather than to remove the assertion everywhere. · ~0.2d · gated on nothing · surfaced iter-172 (evaluator-found, controller-reproduced).

91. **w-personal-email-gate-scope-stops-at-the-sprint-artifacts** · clause-2 · **THE ROW-74 GATE SCANS THE SIX MISSION DOCS AND `scripts/` AND NOTHING ELSE, SO A PERSONAL ADDRESS IN A DESIGN DOC, A SPRINT PLAN OR A ROOT `sprint_*.json` LANDS WITH CI GREEN.** Measured iter-175 at `a295291` (design V15): a full-tree scan of all 357 tracked files with the gate's regex and exclusions reads **8** hits — the one personal address row 74 removed, plus **7 non-address false positives** in exactly the excluded classes: `git AT github.com` ×3 (two `design_docs/planned/w-daemon-timeout-test-flake*` docs and `sprint_w-daemon-timeout-test-flake.json` — SSH remote strings) and `toolchain AT v0.0.1-go1.2x.y.darwin` ×4 (`design_docs/implemented/w-defork-pin-redirects-work-repo*`, `w-setup-go-pin-unguarded-sprint-plan.md` — Go toolchain module ids). So widening is not a one-line regex change: it needs its own exclusion decisions (an SSH-remote form, a Go module-id form), its own arms, and a ruling on whether governance/contact files stay out — exactly what the fleet's script header warns (*"Widening this without ruling on those first would just fail the build"*). The design docs are loop-written too (the designer, planner and executor all write there), and this iteration's own doc had to be kept address-free by hand. **THE ITEM:** decide the widened scope (`design_docs/**` + root `sprint_*.json` at least), add the two measured exclusion forms with a sole-killer arm each, keep the exit-2 floor, and prove the widened gate reads 0 on the tree; the token spellings in this row are deliberately broken with ` AT ` because the charter is IN scope. · ~0.2d · gated on nothing · surfaced iter-175 (quorum r2 `gpt5-6-sol` objection, controller-measured).


68. **w-driver-pin-named-world-points-at-the-wrong-repo** · **[LANDED 2026-09-08 (iter-170) — THE FLEET GUARD IS IN AND THE PIN IS BACK ON]** — M2 landed as fleet commit [`8c56c5863`](https://github.com/sunholo-data/ailang/commit/8c56c5863) *"preserve foreign work repository across pinned driver re-exec"* (on the fleet's `origin/dev`, and an ANCESTOR of the pinned ref `48c4a6e49` this rig runs); M1's `AILANG_DRIVER_PIN=0` stopgap is SUPERSEDED by M3, deployed attended 2026-09-07. **Confirmed HOLDING first-party this fire**: pin ACTIVE (`AILANG_DRIVER_PINNED=48c4a6e49`) with `MISSION_WORKDIR`/`pwd` = the World repo, while the pin worktree's origin is still `sunholo-data/ailang` — the wrong repo the loop is no longer in. **AC-F1..AC-F12 = 12/12 PASS**; the discriminator `_pin_same_repo` returns rc=1 for World (different repo → keep) and rc=0 for motoko (same repo → redirect), the `AILANG_DRIVER_MISSION_IS_DE_FORKED` override wins over inference both ways, an origin-less work repo is LOUD (`rc=1 PIN_STATUS=STALE`, `MISSION_WORKDIR` unchanged), SSH-vs-HTTPS normalise equal, and the production call site is a plain call not a command substitution. **⚠ RUN THOSE ACs UNDER `bash`** — the controller's first pass used the login shell (zsh) and read `rc=2` on AC-F4/F5/F11, a pure instrument artifact of running a bash script's functions under zsh; every AC matched under `bash -c`. Docs → `implemented/`. · clause-2 · **THE LATENT CONDITION FIRED.** The iter-150 measurement was the warning: `~/.ailang-driver-pin/world` was a worktree of `sunholo-data/ailang`, not `sunholo-data/ailang-world`, and lacked `design_docs/world-mission.md`. After DE-FORK commit `e92594c` deleted World's driver copy, `dev.ailang.mission-world.plist` began running the FLEET driver at `/Users/voightkampff/dev/sunholo-data/ailang/tools/launchd/mission-control.sh`; its pin then set both `MISSION_WORKDIR` and `pwd` to `/Users/voightkampff/.ailang-driver-pin/world`, whose origin is `https://github.com/sunholo-data/ailang.git`, where `ls design_docs/world-mission.md` returns `No such file or directory`, while reporting `PIN_STATUS=pinned` and drift 0. **THE SPLIT:** M1 is the controller-owned rig-local `AILANG_DRIVER_PIN=0` mitigation; M2 is the FLEET-owned de-fork guard in `tools/launchd/lib/pin-root.sh`; M3 re-enables the pin only after M2 lands. Under ratified `D-WORLD-DRIVER-1`, World never edits `tools/launchd/*`; it supplies evidence and the fleet hand-off in `design_docs/planned/w-defork-pin-redirects-work-repo.md` and `design_docs/planned/w-defork-pin-redirects-work-repo-sprint-plan.md`. · ~0.1d · gated on the fleet M2 · FIRED iter-166 · surfaced iter-150.
...
70. **w-gate-1-cannot-see-an-unverified-commit-that-is-not-head** · **[LANDED 2026-09-08 (iter-170) — REDUCED BY A CONTROLLER SPLIT; the push-boundary attribution remainder is row 88]** — ships `scripts/gate1_range_check.sh` + `scripts/test_gate1_range_check.sh` + 3 generated fixtures, wired into CI's `go-verify` job. It enumerates `mission-base.sh last gate1`..`origin/dev`, excludes the resolved head by SHA equality, reads one check set per commit and reports every non-HEAD zero-check commit as `ZERO-CHECK-CANDIDATE` (exit 1 = *"a controller must look"*, explicitly NOT *"this commit is unverified"*), with loud-fail paths for an empty range (3), an unreachable `gh`, a `gh` timeout and a malformed `total_count` (2). Measured at `ab5af8c`: over the last 40 commits of `origin/dev` it yields 35 VERIFIED / 4 candidates, and on `165b9fd9d..68403eaf5` it surfaces `e308577` — the commit row 70 exists to name. **The attribution half (is a zero-check commit a separately-pushed TIP or an intra-push parent?) is carved out to row 88**: local git topology cannot answer it, and the events API was measured unusable. Evaluator PASS **88/100**, one blocking finding fixed and proven in-iteration. · clause-2 · **A DIRECT-TO-DEV COMMIT WITH ZERO CHECK-RUNS IS INVISIBLE TO EVERY GATE IN THIS LOOP, BECAUSE GATE 1 READS ONLY `origin/dev`'s HEAD.** Measured iter-150: `e308577` — the commit that added a CI job and thereby armed this iteration's red — has `checks=0` (control: its parent `d75b9c3` and the merge both resolve check sets fine, and the endpoint answers abbreviated SHAs correctly). The origin skill has a rule for a HEAD whose check count is a true zero (fire `workflow_dispatch`, do not record a health verdict); it has **nothing** for a zero on a commit that HEAD has since moved past, and that is the likelier shape here because World's dev takes direct fleet pushes between fires. The red surfaced two commits later wearing a merge's clothes, which cost the attribution work in row 67 rather than being caught where it happened. **THE ITEM:** at Gate 1, after the HEAD check-set read, enumerate `dev@{previous fire}..origin/dev` and assert each commit either has a check set or is explained (intra-push, not a tip) — cheap, one API read per commit, and it is the only instrument that would have named `e308577` at the time. Note the honest scope: only a push **tip** gets a run, so the assertion must be about tips, or it manufactures alarms by construction. · ~0.2d · gated on nothing · surfaced iter-150.

71. **[LANDED 2026-09-08 (iter-171), disposition (A)] w-mission-critical-state-lives-in-a-directory-the-os-wipes-on-boot** · clause-2 ·
...
72. **[LANDED 2026-09-08 (iter-172), CI green on the merge `82d7ef7`, evaluator PASS 87/100 zero blocking]** **w-queue-closed-count-is-an-increment-chain-no-instrument-reproduces** · clause-2 · Ships `scripts/queue_census.sh` (anchored first-token classifier that never reads a row body, honest `UNTAGGED` third bucket, mandatory closed/open control pair adjudicated in the same call, seven ordered floors F0–F6, a total line citing its own input's blob OID), `scripts/test_queue_census.sh` (19 arms, literal-line fixtures only so every floor is reachable), a frozen generated row-start fixture, and two CI steps in `go-verify` — the suite and a live-charter census with `--control-closed 1 --control-open 79`. **The count is now a reading, not a chain:** `census: 41 closed / 89 rows (tagged-open 4, untagged 44)` at the pre-record charter, reproduced independently three times before the instrument existed (controller prototype, planner baseline, shipped script). Normalising the 44 UNTAGGED rows is deliberately OUT of scope with a census-proven recipe recorded; five of them (16, 18, 66, 68, 70) are closed but slug-first, and are reported by number rather than guessed. **The first CI push went RED and it was the good kind** — `numbering gap: expected 1..89, missing 65 76`, because the fixture's `substr($0,1,120)` counts BYTES and cut a 3-byte em dash in half on exactly two lines, which mawk drops and the rig's BWK awk does not; the anti-vacuity floor named both missing rows instead of quietly reporting 87. Fixed in the generating command (`git show 8b15153:` + `iconv -c -f UTF-8 -t UTF-8`), recorded as V23. Docs → `implemented/`. · clause-2 ·
...
73. **[LANDED 2026-09-14 (iter-174), CI green on the merge `50103fd`, evaluator PASS 98/100 zero blocking (round 2; round 1 84/100 with one blocking finding fixed at `1ed9775`), fleet proposal sunholo-data/ailang#1160, note inbox_1789370541088_c82428d1]** **w-gate-0-cannot-see-the-drivers-own-crash-notices** · clause-2 · **THE DRIVER ANNOUNCES EVERY
    FAILED ITERATION AS A COMMENT ON THE BOOKKEEPING ISSUE, AND GATE 0's AUTHOR ALLOWLIST FILTERS
    IT OUT — SO THE LOOP IS BLIND, BY CONSTRUCTION, TO NOTICES OF ITS OWN DEATH POSTED IN THE
    CHANNEL IT READS EVERY FIRE.** `mission-control.sh:617` posts *"⚠️ Mission iteration FAILED to
    complete (rc=$RC — timeout or crash)"* to `$GH_ISSUE_FILE`'s issue. Gate 0.6 reads that same
    issue through `mission_directives.sh`, which is allowlisted to `MarkEdmondson1234` — and that
    allowlist is **correct and must not be widened**: it is the only thing stopping arbitrary
    commenters on a public issue from steering the loop. So this is not a bug in the allowlist; it
    is a **missing second read** beside it. **Measured iter-151:** the notice for the row-56 slot
    sat at `2026-09-02T19:27:22Z` on `#107`; iterations 150 and 151 both ran Gate 0 against that
    issue and **neither surfaced it**. It was found only by chasing a comment timestamp while
    moving the watermark — i.e. by accident, at Gate 5. **Compounding it, the notice's own wording
    is false-reassuring in exactly this case:** *"The queue is untouched; the next interval will
    retry"* is TRUE and misleading together — the queue was untouched precisely because the
    iteration had already merged its PR, so **completed** work read as **unstarted**, and the next
    interval did not retry it because there was nothing left to retry. **THE ITEM:** at Gate 0, in
    addition to the allowlisted directive read, run a **second, separate** read for comments
    authored by *this loop's own account* whose body matches the driver's crash-notice signature,
    since the watermark; treat a hit as a mid-flight-death signal that feeds Gate 2's traces
    (which is where it belongs — it is trace (d), the one that is complete by construction rather
    than a search for residue). Scope discipline: this read **grants no authority** — a crash
    notice is evidence that a fire died, never a directive, and it must not be able to unpark
    anything or become a pick on its own. **Mission-independent** — every mission on this rig
    posts the same notice to its own issue and filters it with the same allowlist — and it is this
    repo's own recurring shape aimed at the loop's memory: **a channel filtered for one sender is
    not a channel you have read.** · ~0.2d · gated on nothing · surfaced iter-151
    (controller-measured, and it corrected this iteration's own record).


74. **[LANDED 2026-09-14 (iter-175), CI GREEN ON THE MERGE COMMIT [`1a0d2a7`](https://github.com/sunholo-data/ailang-world/commit/1a0d2a7) via PR [#139](https://github.com/sunholo-data/ailang-world/pull/139) (`present=2 == expected=2`, `not_green=0`); evaluator `sonnet` round 2 PASS 95/100 ZERO BLOCKING after round 1 FAIL 77/100 with ONE blocking finding fixed at `abe805b`]** SHIPPED: `scripts/check_no_personal_email.sh` (World scope `design_docs/*mission*.md` + `scripts/`, the fleet's address regex, EVERY exclusion clause anchored to the whole token, exit-2 floor on an empty scope) + `scripts/test_check_no_personal_email.sh` (10 arms A–J, fixtures assembled at runtime, the real instrument run as a process) + the `scripts/mission_answer.sh` default moved to the GitHub noreply form with `--dry-run` printing the resolved identity (ARM 6) + two `go-verify` CI steps. **Measured: rc=1 naming `scripts/mission_answer.sh` at base `a295291` (the one hit this row named), ✓ rc=0 after; mutation drill M1–M11 every red set as predicted; M10 (un-anchor) killed by arm I alone, M11 (over-tighten) by arm J alone.** THE JUDGE'S BLOCKING FINDING: the r2-quorum text anchored only the reserved-TLD clause, so `attacker-noreply AT <personal domain>` and any personal domain merely CONTAINING `sentry.io` / `gserviceaccount.com` / `users.noreply.github.com` read ✓ — the fleet's own `scripts/check_no_personal_email.sh` (`f0d44915d`) still carries that substring form (proposed to the fleet at Gate 5). The row's "allow-list for functional defaults" was NOT built — the noreply default (the fleet's own `8369877d9`) removes the need. Widening the scan to `design_docs/**` is NEW ROW 91. Prior row text follows. ~~**w-the-personal-email-gate-the-shared-skill-cites-as-enforcement-does-not-exist** · clause-2 ·
    **THE ATTENDED LEDGER EDITS CONTRACT SAYS `make check-no-personal-email` FAILS THE BUILD IF A
    PERSONAL ADDRESS REACHES A TRACKED FILE. THERE IS NO SUCH TARGET — NOT IN THIS REPO AND NOT IN
    V1's — SO THE ONE ENFORCEMENT THE RULE NAMES IS A CLAIM, AND THIS PUBLIC REPO WAS CARRYING THE
    EXACT EXPOSURE THE RULE EXISTS TO PREVENT.** The shared skill's rule (Mark, attended
    2026-09-02) is right about the harm and right about the remedy — *"use that output to COMPARE,
    never to RECORD"*, because ledger rows are pasted verbatim into a PUBLIC bookkeeping issue by
    every report. **Measured iter-152:** `grep -rn 'check-no-personal-email'` returns **0** in
    `ailang-world` (Makefile, `scripts/`, `.github/`) and **0** in the V1 `Makefile`; meanwhile
    `sunholo-data/ailang-world` is `"visibility":"PUBLIC"` and carried the address in **7** doc
    locations — **4** in the charter (three `D-WORLD-28/29/30` evidence cells plus row 50's
    provenance sentence), **1** in the log, **2** in the status archive — every one of them written
    **2026-09-01**, i.e. the day BEFORE the rule existed, which is why no controller was at fault
    and also why nothing caught them. Negative control: a fresh absent literal reads **0** in the
    same call. **Remediated in the same iteration** by pure substitution to
    `an ATTENDED (non-fleet) author` / `commit author` — 7 → **0**, `git diff --stat` a balanced
    `7 insertions(+), 7 deletions(-)` across 3 files, so no line was added or lost, and the verdict
    the rule actually wants ("attended", not "fleet") is preserved in every cell. Control after:
    the replacement literal reads **4** in the charter. Checked and NOT published: the address had
    reached **0** comments on `#107`, so the report channel had not yet carried it. **DELIBERATELY
    LEFT ALONE:** `scripts/mission_answer.sh:69`'s
    `ATT_EMAIL="${MISSION_ATTENDED_EMAIL:-<attended identity>}"` (the literal is in the script, not
    quoted here — see below) is a functional config default,
    not an evidence cell — redacting it would break the script. Whether a public repo should carry
    it as a literal default rather than requiring the env var is a **fleet** question, not a
    World one. **THE ITEM, and it is the half that is NOT done:** the gate the rule cites still
    does not exist, so the redaction above is a one-time cleanup with nothing preventing the next
    one — and the next one will be written by whichever mission next records an attended
    provenance check. Build a real `check-no-personal-email` (address pattern over tracked files,
    with an allow-list for functional defaults like `mission_answer.sh`), wire it into
    `verify_go.sh` and CI, and prove it non-vacuous with a mutation arm that plants an address in
    a doc and asserts the gate REDS — a gate whose only evidence is "it passes today" is exactly
    the vacuous-pass class this mission keeps closing. **Fleet-relevant, not World-local:** all
    four missions run the shared skill that cites this gate, and V1 lacks it too, so the honest
    scope is a proposal to the fleet plus World's own copy; World cannot edit the shared skill.
    Reported on the cross-mission channel iter-152. · ~0.3d · gated on nothing ·
    surfaced iter-152 (controller-measured, first-party in both repos).~~


75. **[CLOSED 2026-09-25 (iter-189) — SOLVED UPSTREAM: `ailang#1037` was closed COMPLETED on 2026-09-13. Measured on the pinned v0.41.0: an unknown positional now exits **rc=1** with `Error: unexpected argument "zzz_nonexistent_inbox_q7" — 'messages list' takes only flags`. The `-inbox` flag-form control on the same binary exits rc=0. World's side (always use `-inbox`) stands]** **w-messages-list-silently-drops-every-flag-after-a-positional-and-exits-0** · clause-2 ·
    **`ailang messages list <anything>` ACCEPTS AN UNKNOWN POSITIONAL, DISCARDS EVERY FLAG THAT
    FOLLOWS IT, EXITS 0, AND PRINTS THE UNFILTERED DEFAULT LISTING — SO A FILTERED QUERY RETURNS A
    STRICT SUPERSET THAT IS INDISTINGUISHABLE FROM A CORRECT ANSWER.** Hit live at **Gate 0 of this
    very iteration**: I typed `ailang messages list approvals --unread`, read rows from
    `controlplane`, `pkg:*` and `aitana-platform`, and only caught it because the inbox names looked
    wrong. **Three arms, measured (`v0.34.0-414-g267a94e92`).** ARM A, the natural wrong form
    (`messages list approvals -unread -json -limit 50`): human-formatted output from every inbox,
    read AND unread — and `-json` is not honoured either, which is the tell that caught it, because
    a JSON parse of the output fails. ARM B, the control proving the filter works
    (`messages list -inbox approvals -unread -json`): `n=1`, `inboxes={approvals:1}`,
    `statuses={unread:1}`. ARM C, a nonsense positional
    (`messages list zzz_nonexistent_inbox_kq82 -json -limit 3`): **rc=0**, default listing, no
    warning. **Cause:** Go's `flag` package stops parsing at the first non-flag argument, so
    everything after the positional lands in `fs.Args()` and the subcommand never inspects it —
    the same parser behaviour row 57's own text names for a *different* command, arriving here as
    a live defect rather than a hypothesis. **Why the direction matters:** the result is a
    SUPERSET, and a superset reads as a healthy inbox, whereas an error or an empty list would be
    noticed at once. **This is a GATE-0 hazard for every mission on this rig** — all four triage
    their inbox with this command every fire, and Gate 0's verdict decides whether a message
    outranks the queue. **Fleet-scoped:** filed upstream as
    [ailang#1037](https://github.com/sunholo-data/ailang/issues/1037) (reject unexpected
    positionals, or bind a positional inbox to `-inbox` and keep parsing); World's side is to use
    the `-inbox` flag form and never the positional. · ~0.1d upstream · gated on upstream ·
    surfaced iter-152 (controller-measured, live at its own Gate 0).
    **AND THE FIRST RECURRENCE CAME FROM THIS ROW ITSELF, INSIDE THE SAME ITERATION.** The
    paragraph above originally quoted `mission_answer.sh:69` **verbatim**, address included — so
    the act of documenting the redaction re-introduced an 8th occurrence into the very file just
    cleaned, and the post-edit control (`grep -rc` over `design_docs/`) is the only thing that
    caught it, one command before the commit. That is *a record is not inert* (Gate 4) and *a
    control you record is a control you spend* arriving together, aimed at a remediation rather
    than at a measurement: **the write-up of a redaction is itself a publication channel.** It is
    also concrete evidence for this row's own thesis — 24 hours of rulebook and a deliberate
    cleanup were not enough to stop one recurrence in one iteration, which is exactly why the
    deliverable is a GATE and not a sweep. Any gate built for this row must therefore run on the
    mission docs themselves, in CI, after the loop writes them — not as a pre-commit habit a
    controller is trusted to remember.

76. **w-verify-go-driver-drift-gate-short-circuits-the-entire-local-go-gate** · clause-2 · **`scripts/verify_go.sh` — the gate every sprint here cites as an acceptance criterion — IS UNREACHABLE ON THE RIG WHENEVER THE FLEET IS AHEAD, AND NOTHING SAYS SO IN THE ACCEPTANCE ROW.** Measured this iteration at pristine `dev` = `d36359d`: `./scripts/verify_go.sh` **rc=1 in 0.99 s** with `FATAL: DRIVER DRIFT vs FLEET (D-WORLD-DRIVER-1)` naming three files whose World-committed blobs differ from fleet HEAD `5e860afeb` — `tools/launchd/derive-planner-lane.sh` (World `cc98265`, 2026-08-18, vs fleet `e065844b2`, 2026-08-28), `tools/launchd/mission-control.sh` (World `e308577` vs fleet `701f86e5b`, both 2026-09-02) and `tools/launchd/test_mission_routing.sh` (World `cc98265`, 2026-08-18, vs fleet `70e453060`, **2026-09-03 — landed today**). **The red is CORRECT and is not the defect**: `CLAUDE.md` says the drift red means *"the fleet must commit"*, never *"absorb it"*, and `D-WORLD-DRIVER-1` ratified that. **The defect is the ORDERING.** The drift arm fatals at `scripts/verify_go.sh:224`; `go build ./...` is at `:443` and `go test ./... -count=1` at `:462`, so a fleet-owned condition **suspends every Go assertion behind it** — the script's own header calls it *"the durable LOCAL mirror of CI's go-verify job"* and in this state it mirrors nothing. There is **no opt-out**: `grep` for a `SKIP_`/`ALLOW_DRIFT`-shaped variable returns **0**; the only escape is `$CI` being set, which is why **CI is green while the rig cannot run the gate at all** (in CI the fleet checkout is absent, so the arm prints `⚠ … driver currency NOT certified here` and continues). The drift is one-way and continual by construction: the fleet lands driver commits on its own cadence, and World's copies only move when a fleet-authored commit reaches this repo — so this is a standing condition, not an incident. **Why it stayed invisible for three iterations:** 150, 151 and 152 all recorded *"verify gate green, both legs"* using `./scripts/verify_ail.sh` + a hand-rolled `go build ./... && go test ./...`, which is the correct pair of assertions reached by a route that skips the drift arm — so the gate the ACs name has not actually been run here since the drift opened, and its unavailability produced no red anywhere. That is rule 3g's hand-picked-subset gap aimed at a controller's *substitute* for a gate rather than at the gate's own command list, and it is the same shape as this iteration's pick: **a rig-owned condition wearing the clothes of a code verdict.** **THE ITEM:** propose to the fleet that the drift arm either (a) run AFTER the Go legs and report both verdicts, or (b) fail with a distinct, non-suspending status so `verify_go.sh` still answers *"is this tree's Go code green?"* — the one question the acceptance rows ask it. World cannot land either: `tools/launchd/*` and the fleet-comparison arm are frozen core. Meanwhile every AC in this repo that says `verify_go.sh rc=0` must say WHICH legs it means, or it is unsatisfiable by any World change. · ~0.2d (the proposal + an AC-wording process fix) · gated on a fleet decision for the code half · surfaced iter-153 (controller-measured).

77. **w-resolve-role-spawn-does-not-implement-the-designer-rotation-it-is-told-to-obey** · clause-2 · **THE SKILL SAYS "RUN THE RESOLVER AND FOLLOW ITS OUTPUT VERBATIM"; THE RESOLVER RETURNS THE ROTATION'S *SEED* AND NEVER ITS *NEXT ENTRY*, SO A CONTROLLER WHO OBEYS BOTH RULES PINS THE DESIGNER TO ONE LANE FOREVER.** Measured this iteration at HEAD `79d80d9`: `tools/launchd/resolve-role-spawn.sh designer` (V1 checkout, absolute path — absent here, row 69's class) returns `recipe claude:claude-fable-5 declared:provider-pin`, i.e. `$MISSION_DESIGNER_MODEL` echoed back, while `~/.ailang/state/mission-world-designer-rotation` held `claude:claude-fable-5` as the LAST-USED value — so the roles table's rotation says the next designer is `pi:ollama/deepseek-v4-flash:0731-cloud` and the resolver says `claude`. **The two instructions are jointly unsatisfiable and only one of them is loud.** The roles table is emphatic that `$MISSION_DESIGNER_MODEL` is *"the rotation SEED, not a fixed pin"*; the spawn-pattern paragraph is equally emphatic that the resolver's output is *"exactly one line … do not second-guess it"*. A controller reading the spawn pattern and skipping the roles-table prose gets a confident answer that silently disables the rotation — and the rotation exists for quota resilience (an all-Anthropic designer has no lane when the bucket dries) and for reviewer independence, so collapsing it is exactly the failure the 2026-08-28 amendment was written to prevent. This iteration followed the ROLES TABLE and routed to deepseek, which worked (3 runs, typed verdict `ok`); it is recorded here because the next controller has a coin-flip and no rule to decide it by. **THE ITEM:** either teach the resolver to read and advance the namespaced rotation pointer, or make the designer an explicit exception in the spawn-pattern paragraph (*"the resolver does not implement the designer rotation — read the pointer yourself"*). World cannot land the first: `tools/launchd/*` is frozen core, so the code half is filed upstream as [`ailang#1042`](https://github.com/sunholo-data/ailang/issues/1042) plus an `ailang messages send mission-control` note; the second half is a shared-skill Gate-5 edit that needs a second recorded friction before it may be spent. · ~0.1d · gated on nothing for the filing, on the fleet for the code · surfaced iter-154 (controller-measured, first instance).

78. **w-mission-pi-run-does-not-wire-the-sandbox-the-pi-recipe-mandates** · clause-2 · **THE SHARED SKILL'S pi RECIPE SPECIFIES TWO `-e` CONTAINMENT EXTENSIONS AND THEN, ONE PARAGRAPH LATER, TELLS YOU TO RUN THE LANE THROUGH A SCRIPT THAT PASSES NEITHER — SO EVERY pi ROLE ON THIS RIG RUNS UNFENCED WHILE THE RULEBOOK SAYS IT IS SANDBOXED.** The recipe's invocation block is explicit — `-e "$REPO/tools/pi-extensions/sandbox/index.ts" -e "$REPO/tools/pi-extensions/worktree-fence.ts"`, with a table naming which tool each layer fences (`bash` via Seatbelt; `write`/`edit` via a path allow-list) and the note that the upstream sandbox extension fences ONLY `bash`, so `write`/`edit` are un-fenced Node `fs` calls without the second flag. The *next* rule then says `rc=0` from pi is not a claim that work happened and to **run the lane through `scripts/mission_pi_run.sh`** and read its typed verdict. Measured first-party at `scripts/mission_pi_run.sh:155` (V1 checkout — absent here): the invocation is `pi --mode json --no-session --model "$MODEL" < "$DIRECTIVE"`, with **no `-e` flag at all** and no `PI_FENCE_ROOT`. Both instructions are correct in isolation and the second silently voids the first: the typed-verdict runner is strictly better on the *liveness* axis the rule was written for, and strictly worse on the *containment* axis nobody re-checked. **Guard the helper, miss the call site**, aimed at this repo's own pi lane. This iteration ran three designer runs through it unfenced; the post-hoc control the skill prescribes was run each time (`git status --short` in the main checkout, **empty**), which is the reason the exposure is recorded rather than realised — but a post-hoc read is not a fence, and it sees writes, never reads. **THE ITEM:** add the two `-e` flags (and `PI_FENCE_ROOT="$WORKDIR"`) to `mission_pi_run.sh`'s invocation, or state in the recipe that the runner is deliberately unfenced and why. `scripts/` in the V1 checkout is fleet-owned from here, so the code half is filed upstream as [`ailang#1043`](https://github.com/sunholo-data/ailang/issues/1043) plus an `ailang messages send mission-control` note; note also that this repo DOES carry its own `tools/pi-extensions/` (sandbox + worktree-fence + its test), so the extensions exist and only the wiring is missing. · ~0.2d · gated on the fleet for the code · surfaced iter-154 (controller-measured, first instance).


79. **[PARKED — DESIGN REVIEW AND USER APPROVAL] w-evidence-applicability** · clause-5 companion, **not a new release requirement** · [design](planned/w-evidence-applicability.md).
    Added at Mark's attended request to queue Astra-vision designs (2026-09-05); this is authorization to document/queue, not to implement. A package-level requirement revision and evidence binding distinguishes historical proof validity from applicability to the currently selected criterion, assumptions and dependency snapshot. Reuses the existing proof validator and grade labels; no new kernel Evidence variant, DecisionPacket schema or proof mint. Additive kind/verdict/scope/applicability presentation requires reconciliation with HUMAN-SURFACE. **Next:** design review/quorum, resolve freeze items, then Mark's approval and normal sprint planning. ~3–5d provisional; package/adapter and production proof prerequisites recorded in the doc. Does not reorder existing release work.

80. **[PARKED — DESIGN REVIEW AND DEPENDENCIES] w-requirement-change-vertical** · clause-5 integration child of item 7, **not another approval inbox or release threshold** · [design](planned/w-requirement-change-vertical.md).
    Added at the same attended request (2026-09-05). One requirement amendment → stale evidence → bounded resident repair → reviewed action → restart → grounded provenance answer. Uses item 79, the existing decision packet and timeout laws, session-authority/projection/inbox work, and the AILANG semantic-repair-packet/lifecycle-pilot proposals. The deterministic inert-handler drill can precede live integration; it cannot count as product completion or the charter's three REAL provenance questions. **Live-path freeze:** prove the current-selection/authority/durable-intent linearization boundary before enabling dispatch; two adjacent calls are not an atomic admission. **Next:** design review/quorum and Mark's approval; follow existing item-7/39/projection dependencies. ~4–7d after prerequisites, provisional. No new outward handler or shared-driver change.

81. **w-snapshot-reconstruction-faithfulness-proof-is-blind-to-the-split-it-sits-next-to** · clause-2 · **THE SHARED SKILL PRESCRIBES A sha256 FAITHFULNESS PROOF FOR MULTI-MILESTONE COMMIT RECONSTRUCTION, AND THAT PROOF PASSES BY CONSTRUCTION EVEN WHEN EVERY MILESTONE HAS COLLAPSED INTO ONE COMMIT — SO THE ONE PROPERTY THE RECONSTRUCTION EXISTS TO CREATE IS THE ONE PROPERTY NOTHING CHECKS.** The codex/pi recipe's multi-milestone clause is explicit and correct on three of its four steps: the executor writes cumulative `.snap/M<k>/` snapshots, the controller "reconstructs commits by copying snapshots over the tree in milestone order, running the relevant test package at EVERY boundary (bisectability)", and proves the reconstruction faithful by "sha256-manifesting the executor's final tree BEFORE starting and `shasum -c` after the last commit — byte-identity or the reconstruction is wrong". **That manifest is over the FINAL TREE.** Bisectability is a property of the SPLIT, and the final tree is identical whether the split is correct or whether commit 1 swallowed everything and commits 2–3 are empty. Measured first-party this iteration: `git add design_docs host` — a directory-scoped stage, not `-A`, so the skill's existing "never `git add -A`" rule (written for the judge-concurrency window) does not reach it — staged whatever sat on disk rather than the snapshot's named files, so **MS1's commit swallowed MS3's two doc files and MS3 committed nothing** (`git show --stat` on MS1 listed `coding-standards.md` and the superseded row-50 doc; MS3 printed *"nothing added to commit"*). The prescribed `shasum -c` over all five files then returned **OK on every one** — a green faithfulness proof for a reconstruction that had destroyed the bisectability the plan explicitly demanded. It was caught only by reading `git show --stat` per commit instead of trusting three successful `git commit` calls. **AND THE REBUILD'S BOUNDARY GATE FAILED IN THE SAME DIRECTION**: the retry passed a space-separated path list as an unquoted `$CORE`, and **zsh does not word-split unquoted parameter expansions**, so all three paths arrived as ONE argument and `cp` failed — yet the boundary gates still printed `vet=0 verifygate=0 bash-n=0`, because with nothing copied they measured the untouched FINAL tree. *A gate that measures the wrong tree reports the right answer for the wrong reason*, which is this queue's own recurring shape aimed at the reconstruction step. **THE ITEM:** (a) stage each milestone by its snapshot's own file LIST (`git add -- "${files[@]}"`), never by directory; (b) restore files NOT in milestone k's snapshot to their BASE content before committing k, or a later milestone's work leaks into an earlier commit; (c) add the assertion the skill is missing — for each k, `git show --stat` must name exactly the files in `.snap/M<k>/`, and the per-boundary gate must be preceded by a proof the copy actually landed (sha256 of a copied file against the snapshot's), so a failed `cp` cannot green; (d) the `shasum -c` final-tree proof stays, but it must be described as proving *content*, never *split*. `.claude/skills/mission-control/SKILL.md` is the V1 checkout's file and World may not edit it (`world-cannot-edit-shared-skill`), so this is filed as a PROPOSAL to Mark + V1 rather than applied — and it is a **first instance**, so it does not yet meet the shared skill's own ≥2-friction bar; the row exists so the second instance is recognised rather than rediscovered. · ~0.2d (this repo's half: a documented reconstruction helper) · gated on the fleet for the skill half · surfaced iter-155 (controller-measured, first instance).
