...
2. [IN-SPRINT — milestones 1–2 of 6 LANDED 2026-07-24 (iter-8/9), CI green]
   **w-world-library-m1** · clause-1 · Sprint AUTHORIZED (Mark, attended, iter-8-prior). Sprint plan
   ...
   via PR #3 → squash `d5b155c`, evaluator PASS 97/100 (generator≠judge). SQLite driver pin deferred
   to M3 (first import). **Remaining: M3** SQLite store (immutable objects, worlds, atomic append-only
   log; pins the pure-Go SQLite driver) → M4 archive+epoch registry → M5 replay+replay-doubling →
   M6 CI Go gate finalize (`verify_go.sh`). → [LANDED] + doc→implemented/ when M6 lands.
