...
11. [**[IN-SPRINT] — `TR.A1` LANDED 2026-08-11 (iter-71), PR #59 → squash `93df1ec`, dev CI GREEN BOTH JOBS on the merge commit, SHA-addressed, `checks=2` = expected 2, `unresolved_incidents=0`. Evaluator `sonnet` **94/100, ZERO BLOCKING**; its one non-blocking finding APPLIED (`747d1b9`). `metered=$0.00`. Planner `opus` (lane fails closed — `derive-planner-lane.sh` is absent here), executor `codex:gpt-5.6-sol`, judge `sonnet`. Sprint plan: `design_docs/planned/w-transition-registry-tra-sprint-plan.md`; mutation transcript `design_docs/verification/w-transition-registry/tra-mutations.md`. **`TR.A` WAS SPLIT AT ITS T4/T5 BOUNDARY** on the planner's measurement (2630 LOC ≈ 1315/day vs this mission's ~1000/day `VL.B` reference). **`TR.A1` = DONE**: `host/store.CompareAndSetRegistryHead` (transactional, no new table, `schema.sql` untouched), the TR-CJSON-1 canonical codec, descriptors/revisions/deterministic ordering, and committed LITERAL goldens. **AC1/AC4/AC10 ACTIVATED** — base-tolerant arms DELETED, exactly 3/2/1, and the evaluator red the gate by renaming a required test. AC2/AC3/AC5 re-measured `count=0` and AC11 `count=1` AFTER the merge, so no later milestone's criteria were red by this one. **`TR.A2` IS `[NEXT]`** — the plan's T5–T8 (`Reader`, eager `Snapshot`, cache, `BuildNext`/`Publish`), closing AC2/AC3. Already measured and scoped; needs no re-design. Then `TR.B`, then `TR.C`.**]
...
12. [**[NEXT-AFTER-11] — filed 2026-08-11 (iter-71), MEASURED not suspected**]
