...
2. [NEXT — SPRINT AUTHORIZED by Mark 2026-07-24, attended (~07:30 CET)]
   **w-world-library-m1** · clause-1 · **Mark's one-time OK GIVEN (option A of the iter-6 ask):
   carve-out first-use APPROVED for the two narrow resolved defects + the §14-forced
   replay-orchestration framing APPROVED. Route: sprint-planner → executor → evaluator
   (generator≠judge). Recorded here (charter = ratified mission state) because the issue-#1
   GO comment is from the bot account and is correctly NOT a directive under the allowlist —
   the attended-charter-record pattern is the same one iter-6 honored for D1.**
