	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if out.Denied != nil {
			...
			case authority.DenialAbsent, authority.DenialUnknown, authority.DenialExpired:
				...
				// an expired one.
				writeAPIError(w, classFor(*out.Denied), msgFor(*out.Denied), http.StatusUnauthorized)
			case authority.DenialMalformed:
				// The header is present but is not a valid Bearer credential.
				writeAPIError(w, classFor(*out.Denied), msgFor(*out.Denied), http.StatusBadRequest)
			}
			return
		}
